...
def cmd_kuchlarim(message):
    ...
    bot.send_message(
        ...
        "Buyumlar o'yinda avtomatik ishlaydi. Kerak bo'lmasa, o'chirib qo'ying.",
        reply_markup=kb,
    )
...
def cmd_start(message):
    ...
    caption = (
        f"🌙 <b>Hunter Mafia</b> botiga xush kelibsiz, <b>{esc(message.from_user.first_name)}</b>!\n\n"
...
def cmd_startgame(message):
    ...
    for uid, p in game["players"].items():
        ...
        if team_of(role) == "mafia":
            ...
            if teammates:
                safe_send(uid, "🕶 Sheriklaringiz: " + ", ".join(mention(u2, n2) for u2, n2 in teammates) + "\n💬 Ular bilan yashirin gaplashish uchun: <code>/jamoa xabar matni</code>")
            else:
        ...
        if role in ("Komissar 🕵️‍♂️", "Serjant 👮‍♂️"):
            ...
            if law_mates:
                safe_send(uid, "🕵️‍♂️👮‍♂️ Qonun jamoasidagi sherigingiz: " + ", ".join(mention(u2, n2) for u2, n2 in law_mates)
        ...
        if game.get("squad_mode") and game.get("squads"):
            ...
            if my_squad:
                ...
                if squad_mates:
                    safe_send(uid, f"{squad_label} jamoangizdagi sheriklaringiz: " + ", ".join(mention(u2, n2) for u2, n2 in squad_mates)
                               + "\n⚠️ Bu guruhda hech qachon oshkor qilinmaydi — faqat siz bilasiz!")
                else:
                    safe_send(uid, f"{squad_label} jamoasida yolg'izsiz.")
        ...
        if partner_id:
            ...
            safe_send(
                uid,
                f"💍 <b>Maxfiy juftlik!</b> Bu o'yinda sherigingiz: <b>{partner_name}</b>.\n"
                f"⚠️ Buni guruhda hech kimga aytmang — agar sizlardan biri halok bo'lsa, "
                f"ikkalangiz ham DARHOL o'yindan chiqasiz va shundagina guruhga fosh bo'lasiz!",
            )
        ...
        if p["role"] == "Yollanma qotil 🔫":
            ...
            if others:
                ...
                safe_send(
                    ...
                    "Maqsadingiz — FAQAT shu odamni yo'q qilish. U halok bo'lishi bilanoq siz g'alaba qozonasiz!",
                )
...
def cb_vote(call):
    ...
    if voter_id not in game["players"] or not game["players"][voter_id]["alive"]:
        ...
        return
    if voter_id in game.get("day_silenced", set()):
        bot.answer_callback_query(call.id, "😴 Kezuvchi 💋 tashrifidan so'ng bugun ovoz bera olmaysiz — uyqu bosgan!", show_alert=True)
        return
...
def check_and_end_game(chat_id):
    ...
    # 💍 Para (couple) rejimi: agar oxirgi tirik qolgan aynan BITTA juftlikning
    # ikkalasi bo'lsa (rollaridan qat'i nazar — biri tinch, biri mafiya bo'lsa
    # ham) — boshqalar hammasi halok bo'lgani uchun o'sha juftlik birgalikda
    # G'OLIB deb e'lon qilinadi.
    if game.get("couple_mode") and game.get("couple_of") and len(alive) == 2:
...
def end_game(chat_id, winners_team, couple_pair=None):
    ...
    if winners_team == "couple":
        ...
        lines.append(
            ...
            f"🎉 Boshqa hamma halok bo'lgani uchun, ular rollaridan qat'i nazar birgalikda g'olib chiqishdi!\n"
        )
...
def cmd_noma_lum_buyruq(message):
    """🙂 Notanish buyruqqa do'stona javob — foydalanuvchi adashib qolmasin."""
    maybe_capture_owner(message.from_user)
    try:
        bot.reply_to(
            message,
            "🤔 Bunday buyruqni topolmadim.\n"
            "Barcha buyruqlar ro'yxati uchun /yordam ni yuboring.",
        )
    except Exception:
        pass
