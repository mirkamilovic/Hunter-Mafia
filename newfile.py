...
FAQ_KB = [
    ...
    {"keywords": ["olmos qanday olinadi", "diamond qanday", "olmos sotib olish"],
     "answer": "💎 Olmosni <code>/birja</code> orqali Telegram Stars evaziga sotib olishingiz yoki Hunter Coin'dan ayirboshlashingiz mumkin."},
    ...
    {"keywords": ["hunter coin", "coin nima", "coin qanday"],
     "answer": "🪙 Hunter Coin — botning ichki ONLAYN VALYUTASI: kursi vaqt bilan tebranib turadi (<code>/kurs</code> orqali kuzating), do'kondan buyum olish uchun ishlatiladi, Olmos/Dollarga ayirboshlanadi (<code>/olmos_coin</code>, <code>/dollar_coin</code>) va boshqa foydalanuvchiga o'tkazsa ham bo'ladi (<code>/coinsovga</code>). Telegram Stars orqali <code>/birja</code> menyusidan sotib olasiz."},
...
def _tf_elite_savings_tip(uid, elite):
    return "💎 ELITE a'zolikni tejamkor olish uchun: Olmosni to'g'ridan-to'g'ri Stars evaziga emas, avval Hunter Coin yig'ib, keyin ayirboshlab ko'ring — ba'zan bu arzonroq tushadi. Aksiyalar/promo-kodlarni ham kuzatib turing."
...
HEROES = {
    "sardor": {
        ...
        "abilities": [
            ...
            {"key": "death_reveal_once", "unlock": 15, "name": "🩸 Qasos nafasi",
             "desc": "ENG YUQORI DARAJA MAHORATI — O'yin davomida BIR MARTA, agar tunda halok bo'lsangiz, "
                     "sizni o'ldirgan kishining ANIQ rolini so'nggi nafasingizda (shaxsiy xabarda) bilib olasiz."},
        ],
    },
    "malika": {
        ...
        "abilities": [
            ...
            {"key": "guard_once", "unlock": 15, "name": "🕊 Himoyachi tumor",
             "desc": "ENG YUQORI DARAJA MAHORATI — O'yin davomida BIR MARTA, istalgan o'yinchini (o'zingizni ham) "
                     "tanlab, shu TUNGA uni har qanday tungi hujumdan TO'LIQ himoya qilasiz."},
        ],
    },
    "temur": {
        ...
        "abilities": [
            ...
            {"key": "duel_reward_bonus", "unlock": 15, "name": "💰 G'olib nafi",
             "desc": "ENG YUQORI DARAJA MAHORATI — Duelda g'olib chiqqaningizda, yutib olgan pulingiz qo'shimcha "
                     "foizga ko'proq bo'ladi.", "base": 0.10, "cap": 0.30},
        ],
    },
    "layli": {
        ...
        "abilities": [
            ...
            {"key": "gift_once", "unlock": 15, "name": "🎁 Sehrli sovg'a",
             "desc": "ENG YUQORI DARAJA MAHORATI — O'yin davomida BIR MARTA, istalgan o'yinchiga (o'zingizga ham) "
                     "tasodifiy $10-40 yoki 1-2 💎 sovg'a qilasiz."},
        ],
    },
    "burgut": {
...
def start_night(chat_id):
    ...
    if NIGHT_SECONDS > NIGHT_REMINDER_LEAD_SECONDS:
        night_no = game["night_count"]
        rt = threading.Timer(NIGHT_SECONDS - NIGHT_REMINDER_LEAD_SECONDS,
...
def cmd_geroy_bashorat(message):
    ...
    if not hero_has_ability(uid, "role_reveal_once"):
        bot.send_message(message.chat.id, "❌ Bu qobiliyat sizda yo'q (faqat Seraphine geroyi, 11+ darajada beradi).")
        return
    if message.chat.type not in ("group", "supergroup") or not message.reply_to_message:
...
def do_geroy_koz(uid):
    ...
    if not hero_has_ability(uid, "night_vision_auto"):
        return False, "❌ Bu qobiliyat sizda yo'q (faqat Seraphine geroyi, 6+ darajada beradi)."
...
def do_geroy_daromad(uid):
    ...
    if not hero_has_ability(uid, "diamond_trickle"):
        return False, "❌ Bu qobiliyat sizda yo'q (faqat Elandriel geroyi, 11+ darajada beradi)."
    chat_id, game = _find_active_game_for(uid)
    if not game:
...
HERO_ABILITY_USAGE_HINT = {
    "survive": "🌙 AVTOMATIK — lekin foydalanish uchun har TUN botning \"🛡 Geroy himoyasini faollashtirish\" tugmasini bosishingiz SHART (bosmasangiz ishlamaydi)!",
    "revive_once": "🌙 O'yin davomida 1 marta — har TUN botning \"🛡 Geroy himoyasini faollashtirish\" tugmasini bosgan bo'lsangiz, halokat arafasida AVTOMATIK ishga tushadi.",
    "vote_shield_once": "☀️ O'yin davomida 1 marta — har KUN botning \"🗳 Xalq himoyasini faollashtirish\" tugmasini bosishingiz SHART.",
    ...
    "night_vision_auto": "👁 QO'LDA — /geroy_koz buyrug'i yoki /geroyim panelidagi tugma orqali istalgan payt.",
...
def cb_menu(call):
    ...
    if action == "roles":
        ...
        # 🔧 Almos tushish ehtimoli kamaytirildi: avval 5% (roll<95 dan tashqari)
        # edi, endi 2% — Olmos kamyobroq va qimmatroq resurs bo'lib qoladi.
        if roll < 98:
...
def open_hc_stars_menu(chat_id, call=None):
    ...
    text = (
        ...
        "⭐️ pastdagi Stars paketlaridan birini tanlang — <b>5 ⭐️ = 1 🪙</b>."
    )
    if call:
        smart_render(call, text, kb)
    else:
...
def open_diamond_shop_menu(chat_id, uid, call=None):
    ...
    kb.add(types.InlineKeyboardButton(f"🪙 10 Coin ➜ {10 * hc_rate} 💎 Olmos almashtirish", callback_data="exchange_to_diamond"))
    kb = with_back_button(kb)
    text = (
        "💎✨ <b>OLMOS DO'KONI</b> ✨💎\n"
        "━━━━━━━━━━━━━━━━━━\n\n"
...
LANG_STRINGS = {
    ...
    "menu_d2c": {"en": "💎➜🪙 Exchange diamonds", "ru": "💎➜🪙 Обмен алмазов", "uz": "💎➜🪙 Olmos ayirboshlash"},
...
def open_dollar_shop(chat_id, uid, call=None):
    ...
    for i in range(0, len(buttons), 2):
        kb.add(*buttons[i:i + 2])
    kb.add(types.InlineKeyboardButton("💵➜🪙 Dollarni Hunter Coinga almashtirish", callback_data="open_dollar_to_coin"))
    u = user_dict(uid)
    kb = with_back_button(kb)
    text = (
        "💵 <b>DOLLAR DO'KONI</b>\n"
...
HC_BASE_DIAMOND_RATE = 2.0     # boshlang'ich: 2 💎 = 1 🪙
HC_BASE_DOLLAR_RATE = 60.0     # boshlang'ich: 60 $ = 1 🪙
HC_RATE_MIN_MULT = 0.5         # kurs bazaviy qiymatning kamida 50% i
HC_RATE_MAX_MULT = 2.0         # kurs bazaviy qiymatning ko'pi bilan 200% i
HC_MARKET_VOLATILITY_PCT = 6   # har tebranishda ±6% gacha o'zgarish
...
def _hc_market_tick():
    ...
    HC_DOLLAR_RATE = round(_nudge(HC_DOLLAR_RATE, HC_BASE_DOLLAR_RATE), 2)
...
def cmd_hc_kurs(message):
    ...
    bot.send_message(
        ...
        "━━━━━━━━━━━━━━━━━━\n\n"
        f"💎 1 🪙 = <b>{HC_DIAMOND_RATE:g}</b> Olmos {arrow_d}\n"
        f"💵 1 🪙 = <b>{HC_DOLLAR_RATE:g}</b> Dollar {arrow_dl}\n\n"
        f"⏱ Kurs har {HC_MARKET_UPDATE_MINUTES} daqiqada o'zgaradi.\n"
        "💱 /olmos_coin  •  /dollar_coin  •  /coinsovga",
    )
...
def open_diamond_to_coin_menu(chat_id, uid, call=None):
    ...
    text = (
        ...
        f"💎 Sizda: {u['diamond']} Olmos\n"
        f"📈 Joriy jonli kurs: <b>{rate} 💎 = 1 🪙</b> {arrow}\n"
        "(kurs vaqti bilan tebranib turadi, /kurs orqali kuzatib boring)\n\n"
        "Qancha ayirboshlashni tanlang:"
    )
    if call:
        smart_render(call, text, kb)
...
def cb_diamond_to_coin(call):
    ...
    rate = hc_diamond_rate_int()
    coin_amount = diamond_amount // rate
    u = user_dict(uid, call.from_user.first_name)
...
USER_COMMANDS = [
    ...
    ("dollar_coin", "💵 Dollar ↔ Coin ayirboshlash"),
...
_HELP_CATEGORIES = [
    ("🎮 O'yinga qo'shilish", ["start", "join", "rolni_tanla", "tark_et", "turnir_royxat"]),
    ...
    ("🛒 Do'kon, bozor va ELITE", ["almazdokon", "dollar_dokon", "olmos_coin", "dollar_coin", "kurs", "sell", "buy",
