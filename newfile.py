...
def cmd_geroyim(message):
    ...
    if not hero:
        bot.send_message(
            message.chat.id,
            "🏆 Sizda hali geroy yo'q. 💎 Olmos do'konidan <b>🏆 Afsonaviy Sandiq</b> (110 💎) sotib oling — "
    ...
    bot.send_message(message.chat.id, text, reply_markup=kb if cost is not None else None)
...
def cb_hero_level_up(call):
    ...
    try:
        next_cost = hero_level_up_cost(new_level)
        text = (
            f"🏆 <b>{hero['name']}</b>\n"
            f"📊 Daraja: {new_level} / {HERO_MAX_LEVEL}\n"
            f"📝 {hero['desc']}\n"
        )
        kb = types.InlineKeyboardMarkup()
        if next_cost is not None:
            text += f"\n⬆️ Keyingi darajaga: {next_cost} 💎"
            kb.add(types.InlineKeyboardButton(f"⬆️ Darajani oshirish ({next_cost} 💎)", callback_data=f"herolvl|{uid}"))
        else:
            text += "\n🌟 Geroyingiz eng yuqori darajada!"
        bot.edit_message_text(text, call.message.chat.id, call.message.message_id, reply_markup=kb if next_cost is not None else None)
    except Exception:
        pass
