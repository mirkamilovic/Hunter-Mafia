...
# 🌙☀️ Tun/kun rasmlari endi lokal fayllardan yuboriladi (URL emas).
# Skript qayerda ishga tushirilsa, shu papka ichida "assets/night.jpg" va
# "assets/day.jpg" fayllari ham birga bo'lishi SHART (botni hostingga
# yuklaganda "assets" papkasini ham albatta ko'chiring).
_ASSETS_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets")
NIGHT_PHOTO = os.path.join(_ASSETS_DIR, "night.jpg")
DAY_PHOTO = os.path.join(_ASSETS_DIR, "day.jpg")
MAIN_PHOTO = os.path.join(_ASSETS_DIR, "day.jpg")


def send_phase_photo(chat_id, path, caption, reply_markup=None):
    """Tun/kun rasmini lokal fayldan yuboradi. Fayl topilmasa, bot yiqilib
    qolmasligi uchun faqat matnli xabar yuboriladi va log'ga yoziladi."""
    try:
        with open(path, "rb") as photo:
            return bot.send_photo(chat_id, photo, caption=caption, reply_markup=reply_markup)
    except Exception as e:
        _logger.warning("Rasm yuborilmadi (%s): %s", path, e)
...
def get_top_players(limit=10):
    ...
    for user_id, name, wins, dollar, charges_json in rows:
        ...
        if ch.get("shadow", 0):
            continue
        entry = (user_id, name, wins, dollar, bool(ch.get("qirol", 0)))
...
NIGHT_FLAVOR_TEXTS = {
    "Qotil 🗡": "🗡 Qotil harakatga tushdi.",
    "Manyak 🔪": "🔪 Manyak nishon qidirmoqda.",
    "Serjant 👮‍♂️": "👮‍♂️ Serjant patrulda.",
    "Advokat ⚖️": "⚖️ Advokat himoya hujjatini tayyorlamoqda.",
    "Fohisha 💋": "💋 Sirli mehmon shaharda yurmoqda.",
    "Terrorist 💣": "🧨 Portlovchi hid sezildi.",
    "Mergan 🏹": "🏹 Mergan nishonga tayyor turibdi.",
    "Varvar 🪓": "🪓 Jangchi qadam tovushi eshitildi.",
    "Sadoqatli yordamchi 🤝": "🤝 Kimdir homiysini kuzatmoqda.",
    "Snayper 🎯": "🎯 Kimdir nishonga ko'z tikdi.",
    "O'g'ri 🥷": "🥷 Yashirin qadamlar — kimdir o'g'irlik qilmoqda.",
    "Sehrgar 🧙‍♂️": "🔮 Sehrgar kuchlarini ishga solmoqda.",
    "Sehrgar yordamchisi 🪄": "🪄 Yordamchi sehrni kuchaytirmoqda.",
    "Arvoh 👻": "👻 Kimdir tunni sirli kuzatmoqda.",
    "Sudya 👨‍⚖️": "⚖️ Sudya ertangi hukmga tayyorlanmoqda.",
    "Provokator 🗣": "🗣 Kimdir janjal rejalashtirmoqda.",
    "General 🎖": "🎖 General taktika tuzmoqda.",
    "Josus 🕵️": "🕵️ Josus kuzatuvda.",
    "Bomj 🧟‍♂️": "🧟‍♂️ Ko'chada bir soya kezib yuribdi.",
    "Arxitektor 📐": "📐 Arxitektor to'siq rejasini chizmoqda.",
    "Telba 🤪": "🤪 Kimdir g'alati harakat qilmoqda.",
    "Qorovul 🔦": "🔦 Qorovul hududini qo'riqlamoqda.",
    "Beshikdagi bola 👶": "👶 Kenja qahramon tinch uxlamoqda.",
}
...
AFK_KICK_JOKES = [
    "😴 <b>{name}</b> 3 tundan beri uxlab qolgan shekilli — bot uni o'yindan mehr bilan chiqarib yubordi!",
    "📴 <b>{name}</b> tarmoqdan uzilgani uchun soyalar dunyosiga ko'chirildi. Xayr! 👋",
    "🛌 <b>{name}</b> uzoq vaqt jim qolgani uchun \"AFK\" deb topildi va o'yindan chetlashtirildi.",
    "🚪 <b>{name}</b> tun bo'yi hech qayerga qo'ng'iroq qilmadi... Demak, u endi o'yinda emas!",
    "👻 <b>{name}</b> shunchalik sokin ediki, hatto bot ham uni unutib qo'ygandi — endi rasman o'yindan tashqarida.",
]
...
SHOP_DOLLAR = {
    ...
    "sumka":    {"name": "💼 Shubhali sumka", "price": 450, "currency": "dollar", "mode": "instant_dollar", "range": (80, 250),
                 "desc": "Ichidan tasodifiy 80$-250$ chiqadi."},
    ...
    "zar":      {"name": "🎲 Sehrli Zarlar", "price": 320, "currency": "dollar", "mode": "instant_dollar", "range": (100, 400),
                 "desc": "Ochilganda tasodifiy $100 dan $400 gacha yutuq keltiradi."},
    ...
    "kun_himoya": {"name": "🛡 Kunduzgi Himoya", "price": 200, "currency": "dollar", "mode": "charge", "charge_key": "day_shield",
                 "desc": "Kunduzi aholi sizni osmoqchi bo'lsa, bu himoya sizni 1 marta (bitta o'yin davomida) qutqarib qoladi."},
}
...
SHOP_DIAMOND = {
    ...
    "tumor":     {"name": "🔮 Sehrli tumor", "price": 10, "currency": "diamond", "mode": "multiplier",
                  "charge_key": "bonus_mult", "mult_value": 2, "desc": "Kunlik bonusingizni 2 barobar qiladi."},
    "quti":      {"name": "📦 Nodir quti", "price": 15, "currency": "diamond", "mode": "instant_dollar", "range": (100, 300),
    ...
    "eliksir":   {"name": "🧪 Hayot Eliksiri", "price": 35, "currency": "diamond", "mode": "instant_dollar", "range": (1500, 1500),
                  "desc": "Barcha jarohatlarni bitirib, +1500$ bonus beradi."},
}
...
SHOP_OSH = {
    "toy":       {"name": "💍 To'y Oshi", "price": 5000, "currency": "dollar", "mode": "gift_voucher", "gift_key": "shield",
                  "desc": "Pul emas — bu oshni birovga 'tortiq' qilasiz! /oshtortiq buyrug'i orqali (u kishining "
                          "xabariga reply qilib) unga 1 ta 🛡 Himoya (shield) sovg'a qilasiz."},
    "choyxona":  {"name": "🍵 Choyxona Oshi", "price": 4500, "currency": "dollar", "mode": "gift_voucher", "gift_key": "diamond5",
                  "desc": "Birovga tortiq qiling — /oshtortiq bilan u kishiga +5 💎 Olmos o'tkazib berasiz."},
    "tandir":    {"name": "🔥 Tandir Oshi", "price": 4000, "currency": "dollar", "mode": "gift_voucher", "gift_key": "dollar300",
                  "desc": "Birovga tortiq qiling — /oshtortiq bilan u kishiga +300$ o'tkazib berasiz."},
    "samarqand": {"name": "🇺🇿 Samarqand Oshi", "price": 3800, "currency": "dollar", "mode": "gift_voucher", "gift_key": "dollar250",
                  "desc": "Birovga tortiq qiling — /oshtortiq bilan u kishiga +250$ o'tkazib berasiz."},
    "buxoro":    {"name": "🏛 Buxorocha Sofi Oshi", "price": 3600, "currency": "dollar", "mode": "gift_voucher", "gift_key": "dollar200",
                  "desc": "Birovga tortiq qiling — /oshtortiq bilan u kishiga +200$ o'tkazib berasiz."},
    "fargona":   {"name": "🏔 Farg'ona Devzira Oshi", "price": 3500, "currency": "dollar", "mode": "gift_voucher", "gift_key": "dollar150",
                  "desc": "Birovga tortiq qiling — /oshtortiq bilan u kishiga +150$ o'tkazib berasiz."},
    "qora":      {"name": "🥷 Qora Mafia Oshi", "price": 3000, "currency": "dollar", "mode": "gift_voucher", "gift_key": "dollar100",
                  "desc": "Birovga tortiq qiling — /oshtortiq bilan u kishiga +100$ o'tkazib berasiz."},
}

# Har bir gift_key nima beradi (miqdor/turi) — /oshtortiq buyrug'i shu yerdan o'qiydi
OSH_GIFT_EFFECTS = {
    "shield": {"kind": "shield", "amount": 1, "text": "🛡 1 ta Himoya (shield)"},
    "diamond5": {"kind": "diamond", "amount": 5, "text": "+5 💎"},
    "dollar300": {"kind": "dollar", "amount": 300, "text": "+300$"},
    "dollar250": {"kind": "dollar", "amount": 250, "text": "+250$"},
    "dollar200": {"kind": "dollar", "amount": 200, "text": "+200$"},
    "dollar150": {"kind": "dollar", "amount": 150, "text": "+150$"},
    "dollar100": {"kind": "dollar", "amount": 100, "text": "+100$"},
}
...
def new_game(chat_id, chat_title):
    return {
        ...
        "poison_marks": set(),
        "timers": [],
        # --- 28 rolning to'liq tungi harakatlari uchun holat ---
        "night_announced": set(),
        "qotil_target": None,
        "manyak_target": None,
        "qorovul_target": None,
        ...
        "mergan_target": None,
        "mergan_used": False,
        # --- kunduzgi yakuniy hukm (verdict) ovoz berish ---
        "verdict_target": None,
        "verdict_votes": {},
        "verdict_open": False,
    }
...
def cmd_start(message):
    ...
    send_phase_photo(message.chat.id, MAIN_PHOTO, caption, reply_markup=get_main_menu(None, message.from_user.id))
...
def start_night(chat_id):
    ...
    game["qotil_target"] = None
    game["manyak_target"] = None
    game["qorovul_target"] = None
    game["fohisha_target"] = None
    game["advokat_target"] = None
    game["ogri_target"] = None
    game["josus_target"] = None
    game["mergan_target"] = None

    send_phase_photo(
        chat_id, NIGHT_PHOTO,
        caption=f"🌙 <b>{game['day_number']}-tun boshlandi</b>\n"
                "Guruhga yozish vaqtincha yopiq. 🔇\n"
                "🎭 Rolli o'yinchilar — botga DM yozib, tanlovingizni qiling.",
    ...
    # 🕵️‍♂️ AFK nazorati — bu tunda kimlarga harakat kutilayotganini qayd qilamiz
    game["acted_this_night"] = set()
    game["night_action_recipients"] = set()
    game.setdefault("afk_streak", {})
    ...
    for uid in mafia_uids:
        ...
        game["night_action_recipients"].add(uid)
    ...
    def _target_kb(kind, exclude_self=True, extra_skip=True):
        kb2 = types.InlineKeyboardMarkup()
        for target_id, p in alive.items():
            if exclude_self and target_id == uid:
                continue
            kb2.add(types.InlineKeyboardButton(p["name"], callback_data=f"na|{chat_id}|{kind}|{target_id}"))
        if extra_skip:
            kb2.add(types.InlineKeyboardButton("🚫 O'tkazib yuborish", callback_data=f"na|{chat_id}|{kind}|skip"))
        return kb2

    qotil_uids = alive_role_holders(game, lambda r: r == "Qotil 🗡")
    for uid in qotil_uids:
        safe_send(uid, "🗡 <b>Bu tun kimni yo'q qilasiz?</b>", _target_kb("qotil"))
        game["night_action_recipients"].add(uid)

    manyak_uids = alive_role_holders(game, lambda r: r == "Manyak 🔪")
    for uid in manyak_uids:
        safe_send(uid, "🔪 <b>Vahshiy ishtahangiz kimga qaratilgan?</b> (Himoyalarni chetlab o'tadi)", _target_kb("manyak"))
        game["night_action_recipients"].add(uid)

    qorovul_uids = alive_role_holders(game, lambda r: r == "Qorovul 🔦")
    for uid in qorovul_uids:
        safe_send(uid, "🔦 <b>Bu tun kimni qo'riqlaysiz?</b>", _target_kb("qorovul", exclude_self=False))
        game["night_action_recipients"].add(uid)

    fohisha_uids = alive_role_holders(game, lambda r: r == "Fohisha 💋")
    for uid in fohisha_uids:
        safe_send(uid, "💋 <b>Bu tun kimning oldiga borib, uni bandlik bilan chalg'itasiz?</b> (uning tungi qobiliyati bloklanadi)", _target_kb("fohisha"))
        game["night_action_recipients"].add(uid)

    advokat_uids = alive_role_holders(game, lambda r: r == "Advokat ⚖️")
    for uid in advokat_uids:
        safe_send(uid, "⚖️ <b>Ertaga kimni ovoz berib osishdan himoya qilasiz?</b> (1 marta, ertangi kun uchun)", _target_kb("advokat"))
        game["night_action_recipients"].add(uid)

    ogri_uids = alive_role_holders(game, lambda r: r == "O'g'ri 🥷")
    for uid in ogri_uids:
        safe_send(uid, "🥷 <b>Kimning cho'ntagini titkilaysiz?</b>", _target_kb("ogri"))
        game["night_action_recipients"].add(uid)

    josus_uids = alive_role_holders(game, lambda r: r == "Josus 🕵️")
    for uid in josus_uids:
        safe_send(uid, "🕵️ <b>Kimning sirlarini yashirincha poylaysiz?</b> (rolini shaxsiy bilib olasiz)", _target_kb("josus"))
        game["night_action_recipients"].add(uid)

    mergan_uids = alive_role_holders(game, lambda r: r == "Mergan 🏹")
    for uid in mergan_uids:
        if not game.get("mergan_used"):
            safe_send(uid, "🏹 <b>Butun o'yin davomida faqat 1 marta ishlatiladigan o'q!</b> Hozir otasizmi?", _target_kb("mergan"))
            game["night_action_recipients"].add(uid)
...
def cb_night_action(call):
    ...
    # 🕵️‍♂️ AFK nazorati — o'yinchi tunda biror tugmani bossa, faol hisoblanadi
    game.setdefault("acted_this_night", set()).add(uid)

    # 🌑 ANONIM "SIZGA QIZIQISH BILDIRILDI" XABARI — qaysi rol ekani va kim ekani
    # oshkor qilinmaydi, faqat nishonga tanlanganini bildiruvchi sirli DM yuboriladi.
    # Har bir (harakat turi, tanlovchi) juftligi uchun faqat oxirgi tanlangan nishonga
    # bir marta yuboriladi (tugmani qayta bosib nishon o'zgartirilsa spam bo'lmasin).
    ANON_INTEREST_KINDS = {
        "mafia", "komtarget", "doctor", "qotil", "manyak", "qorovul",
        "fohisha", "advokat", "ogri", "josus", "mergan",
    }
    if kind in ANON_INTEREST_KINDS and value != "skip":
        try:
            _target_id = int(value)
        except ValueError:
            _target_id = None
        if _target_id and _target_id != uid and _target_id in game["players"]:
            anon_map = game.setdefault("anon_notice_sent", {})
            notice_key = f"{kind}:{uid}"
            if anon_map.get(notice_key) != _target_id:
                anon_map[notice_key] = _target_id
                safe_send(_target_id, "🌑 <i>Bu kecha kimdir sizning rolingizga qiziqib qoldi...</i>")
    ...
    elif kind in ("qotil", "manyak", "qorovul", "fohisha", "advokat", "ogri", "josus", "mergan"):
        target = None if value == "skip" else int(value)
        game[f"{kind}_target"] = target
        if kind == "mergan" and target is not None:
            game["mergan_used"] = True

        flavor = {
            "qotil": ("qotil", "🗡 <i>Yolg'iz qotilning soyasi biror kishining orqasidan sudralib bormoqda...</i>"),
            "manyak": ("manyak", "🔪 <i>Manyak qorong'ulikda o'z qurbonini poylamoqda...</i>"),
            "qorovul": ("qorovul", "🔦 <i>Qorovul o'z nishonini kechasi bo'yi qo'riqlashga chiqdi...</i>"),
            "fohisha": ("fohisha", "💋 <i>Sirli mehmon kimningdir eshigini taqillatdi...</i>"),
            "advokat": ("advokat", "⚖️ <i>Advokat ertangi kun uchun himoya hujjatlarini tayyorlamoqda...</i>"),
            "ogri": ("ogri", "🥷 <i>Yashirin qadamlar — kimningdir cho'ntagi nishonga olindi...</i>"),
            "josus": ("josus", "🕵️ <i>Josus soyada turib, kimningdir sirlarini kuzatmoqda...</i>"),
            "mergan": ("mergan", "🏹 <i>Mergan bu tun o'zining yagona o'qini otishga qaror qildi...</i>"),
        }
        if target is not None:
            ann_key, ann_text = flavor[kind]
            if ann_key not in game["night_announced"]:
                game["night_announced"].add(ann_key)
                bot.send_message(chat_id, ann_text)
        try:
            bot.edit_message_text("✅ Tanlandi! Tun yakunida natija ma'lum bo'ladi.", call.message.chat.id, call.message.message_id)
        except Exception:
            pass
        bot.answer_callback_query(call.id, "Qabul qilindi!")


def apply_kill(game, uid, killed_set, bypass_protection=False, protected_set=None):
    """Bitta o'yinchini o'ldirish urinishi — shield/revive/doktor davolashi hisobga olinadi.
    protected_set beriladigan bo'lsa, himoya tufayli omon qolgan o'yinchilar shu to'plamga qo'shiladi
    (guruhga keyinroq anonim tarzda "kimningdir himoyasi ushladi" deb e'lon qilish uchun)."""
    ...
    if not bypass_protection:
        if game.get("doctor_target") == uid or game.get("qorovul_target") == uid:
            if protected_set is not None:
                protected_set.add(uid)
            return
...
def resolve_night(chat_id):
    with GAME_LOCK:
        ...
        # 🕵️‍♂️ AFK NAZORATI — rolli o'yinchi ketma-ket 3 tun harakat qilmasa,
        # avtomatik o'yindan chiqariladi va guruhga hazil bilan e'lon qilinadi.
        afk_streak = game.setdefault("afk_streak", {})
        recipients = game.get("night_action_recipients", set())
        acted = game.get("acted_this_night", set())
        for r_uid in list(recipients):
            p = game["players"].get(r_uid)
            if not p or not p["alive"]:
                afk_streak.pop(r_uid, None)
                continue
            if r_uid in acted:
                afk_streak[r_uid] = 0
            else:
                afk_streak[r_uid] = afk_streak.get(r_uid, 0) + 1
                if afk_streak[r_uid] >= 3:
                    p["alive"] = False
                    afk_streak[r_uid] = 0
                    joke = random.choice(AFK_KICK_JOKES).format(name=p["name"])
                    bot.send_message(chat_id, joke)
                    # bu o'yinchining shu tunga qoldirgan barcha buyruqlari bekor bo'ladi
                    game["mafia_votes"].pop(r_uid, None)
                    for key in ("komissar_target", "doctor_target", "qotil_target", "manyak_target",
                                "qorovul_target", "fohisha_target", "advokat_target", "ogri_target",
                                "josus_target", "mergan_target"):
                        if game.get(key) == r_uid:
                            game[key] = None
                    if p.get("role") == "Komissar 🕵️‍♂️":
                        game["komissar_action"] = "skip"

        # 📸 Rol egalarining tunga oid DM-natija xabarlari uchun oldindan olingan ro'yxatlar
        # (AFK bilan chetlashtirilganlardan keyin, hali tirik bo'lganlar bo'yicha)
        mafia_uids_snapshot = alive_role_holders(game, lambda r: team_of(r) == "mafia")
        kom_uids_snapshot = alive_role_holders(game, lambda r: r == "Komissar 🕵️‍♂️")
        doc_uids_snapshot = alive_role_holders(game, lambda r: r == "Doktor 👨‍⚕️")
        qotil_uids_snapshot = alive_role_holders(game, lambda r: r == "Qotil 🗡")
        manyak_uids_snapshot = alive_role_holders(game, lambda r: r == "Manyak 🔪")
        qorovul_uids_snapshot = alive_role_holders(game, lambda r: r == "Qorovul 🔦")
        mergan_uids_snapshot = alive_role_holders(game, lambda r: r == "Mergan 🏹")

        # 💋 Fohisha — bloklangan o'yinchining shu tungi harakati bekor qilinadi
        blocked_uid = game.get("fohisha_target")
        if blocked_uid and blocked_uid in game["players"]:
            blocked_role = game["players"][blocked_uid]["role"]
            if blocked_role == "Komissar 🕵️‍♂️":
                game["komissar_action"] = "skip"
            elif blocked_role == "Doktor 👨‍⚕️":
                game["doctor_target"] = "skip"
            elif team_of(blocked_role) == "mafia" and blocked_uid in game["mafia_votes"]:
                game["mafia_votes"][blocked_uid] = "skip"
            elif blocked_role == "Qotil 🗡":
                game["qotil_target"] = None
            elif blocked_role == "Manyak 🔪":
                game["manyak_target"] = None
            elif blocked_role == "Qorovul 🔦":
                game["qorovul_target"] = None
            elif blocked_role == "Advokat ⚖️":
                game["advokat_target"] = None
            elif blocked_role == "O'g'ri 🥷":
                game["ogri_target"] = None
            elif blocked_role == "Josus 🕵️":
                game["josus_target"] = None
            elif blocked_role == "Mergan 🏹":
                game["mergan_target"] = None
        ...
        killed = set()
        protected = set()
        apply_kill(game, mafia_kill_target, killed, bypass_protection=bypass_for_mafia, protected_set=protected)

        if game.get("komissar_action") == "kill" and game.get("komissar_target"):
            apply_kill(game, game["komissar_target"], killed, protected_set=protected)

        # 🧪 Zaharli flakon — /zahar orqali belgilanganlar
        poison_targets = game.pop("poison_marks", set())
        for pt in poison_targets:
            apply_kill(game, pt, killed, protected_set=protected)
        game["poison_marks"] = set()

        # 🗡 Qotil — mustaqil qotil, himoyalar (shield/doktor/qorovul) hisobga olinadi
        qotil_target = game.get("qotil_target")
        if qotil_target:
            visitor_role_of[qotil_target] = visitor_role_of.get(qotil_target, "Qotil 🗡")
            apply_kill(game, qotil_target, killed, protected_set=protected)

        # 🔪 Manyak — shafqatsiz, barcha himoyalarni chetlab o'tadi
        manyak_target = game.get("manyak_target")
        if manyak_target:
            visitor_role_of[manyak_target] = "Manyak 🔪"
            apply_kill(game, manyak_target, killed, bypass_protection=True)

        # 🏹 Mergan — butun o'yinda bir martalik o'q, himoyalarni chetlab o'tadi
        mergan_target = game.get("mergan_target")
        if mergan_target:
            visitor_role_of[mergan_target] = "Mergan 🏹"
            apply_kill(game, mergan_target, killed, bypass_protection=True)

        # 🥷 O'g'ri — o'ldirmaydi, faqat pul o'g'irlaydi (tirik qolganlarda ishlaydi)
        ogri_target = game.get("ogri_target")
        ogri_uid = next(iter(alive_role_holders(game, lambda r: r == "O'g'ri 🥷")), None)
        if ogri_target and ogri_uid and ogri_target not in killed and game["players"].get(ogri_target, {}).get("alive"):
            victim = user_dict(ogri_target)
            steal_amount = min(victim.get("dollar", 0), random.randint(50, 300))
            if steal_amount > 0:
                add_balance(ogri_target, dollar=-steal_amount)
                add_balance(ogri_uid, dollar=steal_amount)
                safe_send(ogri_uid, f"🥷 Siz {game['players'][ogri_target]['name']}dan {steal_amount}$ o'g'irladingiz!")
            else:
                safe_send(ogri_uid, "🥷 Nishoningizning cho'ntagi bo'sh ekan...")

        # 🕵️ Josus — nishonning haqiqiy rolini shaxsiy xabarda biladi (guruhga oshkor qilinmaydi)
        josus_target = game.get("josus_target")
        josus_uid = next(iter(alive_role_holders(game, lambda r: r == "Josus 🕵️")), None)
        if josus_target and josus_uid and josus_target in game["players"]:
            t_role = game["players"][josus_target]["role"]
            t_name = game["players"][josus_target]["name"]
            safe_send(josus_uid, f"🕵️ Josuslik natijasi: <b>{t_name}</b> — <b>{t_role}</b>")

        # 🛡 Himoya ushlab qolgan hujumlar — kim ekanini oshkor qilmay, guruhga anonim e'lon qilinadi
        for _puid in protected:
            if _puid not in killed:
                bot.send_message(chat_id, "🛡 <i>Bu kecha kimningdir himoyasi bir hujumni ushlab qoldi...</i>")
        ...
        # 🕵️‍♂️ Rol egalariga tunning natijasi haqida shaxsiy (DM) xabar — har biri o'z
        # harakati natijasini, nishonning bosiladigan ismi bilan birga ko'radi.
        def _t(uid_):
            p_ = game["players"].get(uid_)
            return mention(uid_, p_["name"]) if p_ else "Noma'lum"

        for m_uid in mafia_uids_snapshot:
            if mafia_kill_target is None:
                safe_send(m_uid, "🕶 Bu tun sheriklaringiz bilan hech kimga hujum qilmadingiz.")
            elif mafia_kill_target in killed:
                safe_send(m_uid, f"🕶 Bu kecha {_t(mafia_kill_target)} ni yo'q qilishga muvaffaq bo'ldingiz!")
            else:
                safe_send(m_uid, f"🕶 {_t(mafia_kill_target)} ga hujum qildingiz, ammo u himoyalangan edi — hujum barbod bo'ldi.")

        if game.get("komissar_action") == "kill" and game.get("komissar_target"):
            k_target = game["komissar_target"]
            for k_uid in kom_uids_snapshot:
                if k_target in killed:
                    safe_send(k_uid, f"🎯 Hukmingiz ijro etildi: {_t(k_target)} yo'q qilindi.")
                else:
                    safe_send(k_uid, f"🎯 {_t(k_target)} ga hujum qildingiz, ammo u himoyalangan edi.")

        doctor_target = game.get("doctor_target")
        for d_uid in doc_uids_snapshot:
            if doctor_target in (None, "skip"):
                safe_send(d_uid, "👨‍⚕️ Bu tun hech kimni davolamadingiz.")
            elif doctor_target in protected:
                safe_send(d_uid, f"👨‍⚕️ Yaxshi xabar — {_t(doctor_target)} ga hujum qilingan edi, siz uni davolab qutqarib qoldingiz!")
            else:
                safe_send(d_uid, f"👨‍⚕️ {_t(doctor_target)} ni davoladingiz, lekin bu kecha unga hech kim hujum qilmagani uchun yordamingiz kerak bo'lmadi.")

        for q_uid in qotil_uids_snapshot:
            if not qotil_target:
                safe_send(q_uid, "🗡 Bu tun hech kimga hujum qilmadingiz.")
            elif qotil_target in killed:
                safe_send(q_uid, f"🗡 {_t(qotil_target)} ni muvaffaqiyatli yo'q qildingiz.")
            else:
                safe_send(q_uid, f"🗡 {_t(qotil_target)} ga hujum qildingiz, ammo u himoyalangan edi.")

        for mn_uid in manyak_uids_snapshot:
            if not manyak_target:
                safe_send(mn_uid, "🔪 Bu tun hech kimga hujum qilmadingiz.")
            elif manyak_target in killed:
                safe_send(mn_uid, f"🔪 {_t(manyak_target)} ni shafqatsizlarcha yo'q qildingiz.")
            else:
                safe_send(mn_uid, f"🔪 {_t(manyak_target)} ga hujum qildingiz.")

        qorovul_target = game.get("qorovul_target")
        for qr_uid in qorovul_uids_snapshot:
            if not qorovul_target:
                safe_send(qr_uid, "🔦 Bu tun hech kimni qo'riqlamadingiz.")
            elif qorovul_target in protected:
                safe_send(qr_uid, f"🔦 {_t(qorovul_target)} ni hujumdan muvaffaqiyatli qo'riqladingiz!")
            else:
                safe_send(qr_uid, f"🔦 {_t(qorovul_target)} ni qo'riqladingiz, bu kecha hujum bo'lmadi.")

        for mg_uid in mergan_uids_snapshot:
            if not mergan_target:
                continue
            if mergan_target in killed:
                safe_send(mg_uid, f"🏹 O'qingiz nishonga tegdi — {_t(mergan_target)} yo'q qilindi!")
            else:
                safe_send(mg_uid, f"🏹 O'qingiz {_t(mergan_target)} ga tegdi, ammo u himoyalangan edi.")
...
# ================================================================================
#  🥘 OSH TORTIQ — do'kondan olingan "osh" buyumlarini boshqa o'yinchiga sovg'a qilish
#  (bu buyumlar endi xaridorga pul bermaydi, faqat "tortiq qilish huquqi" beradi)
# ================================================================================

@bot.message_handler(commands=["oshtortiq"])
def cmd_osh_tortiq(message):
    maybe_capture_owner(message.from_user)
    safe_delete(message)
    if message.chat.type not in ("group", "supergroup"):
        bot.send_message(message.chat.id, "🥘 Bu buyruq faqat guruhda, kimningdir xabariga reply qilib ishlatiladi.")
        return
    if not message.reply_to_message or not message.reply_to_message.from_user:
        bot.send_message(message.chat.id, "🥘 Kimgadir osh tortiq qilish uchun, o'sha kishining xabariga <b>reply</b> qilib /oshtortiq deb yozing.")
        return
    uid = message.from_user.id
    target = message.reply_to_message.from_user
    if target.is_bot:
        bot.send_message(message.chat.id, "⚠️ Botga tortiq qilib bo'lmaydi 😅")
        return
    if target.id == uid:
        bot.send_message(message.chat.id, "⚠️ O'zingizga tortiq qila olmaysiz, osh boshqalar uchun! 😄")
        return
    ch = get_charges(uid)
    vouchers = ch.get("osh_vouchers", [])
    if not vouchers:
        bot.send_message(message.chat.id, "🎒 Sizda hozircha tortiq qiladigan osh yo'q. Do'kondan 🥘 <b>Milliy Osh Menyusi</b> bo'limidan xarid qiling.")
        return
    user_dict(target.id, target.first_name)  # nishon bazada mavjud bo'lishini kafolatlaymiz
    counts = {}
    for gk in vouchers:
        counts[gk] = counts.get(gk, 0) + 1
    kb = types.InlineKeyboardMarkup()
    for gift_key, cnt in counts.items():
        eff = OSH_GIFT_EFFECTS.get(gift_key)
        if not eff:
            continue
        kb.add(types.InlineKeyboardButton(f"{eff['text']} (x{cnt})", callback_data=f"oshgift|{uid}|{target.id}|{gift_key}"))
    bot.send_message(
        message.chat.id,
        f"🥘 {mention(uid, message.from_user.first_name)} → {mention(target.id, target.first_name)} ga qaysi oshni tortiq qilmoqchi?",
        reply_markup=kb,
    )


@bot.callback_query_handler(func=lambda c: c.data.startswith("oshgift|"))
def cb_osh_gift(call):
    maybe_capture_owner(call.from_user)
    _, giver_id_s, target_id_s, gift_key = call.data.split("|")
    giver_id, target_id = int(giver_id_s), int(target_id_s)
    uid = call.from_user.id
    if uid != giver_id:
        bot.answer_callback_query(call.id, "❌ Bu tortiq sizga tegishli emas.", show_alert=True)
        return
    ch = get_charges(uid)
    vouchers = ch.get("osh_vouchers", [])
    if gift_key not in vouchers:
        bot.answer_callback_query(call.id, "❌ Bu tortiq allaqachon ishlatilgan yoki mavjud emas.", show_alert=True)
        return
    vouchers.remove(gift_key)
    ch["osh_vouchers"] = vouchers
    set_charges(uid, ch)

    eff = OSH_GIFT_EFFECTS[gift_key]
    if eff["kind"] == "shield":
        add_shield(target_id, eff["amount"])
    elif eff["kind"] == "dollar":
        add_balance(target_id, dollar=eff["amount"])
    elif eff["kind"] == "diamond":
        add_balance(target_id, diamond=eff["amount"])

    try:
        bot.edit_message_text(f"✅ Tortiq yuborildi: {eff['text']}", call.message.chat.id, call.message.message_id)
    except Exception:
        pass
    bot.answer_callback_query(call.id, "🎁 Tortiq yuborildi!")
    target_name = user_dict(target_id).get("name", "O'yinchi")
    bot.send_message(
        call.message.chat.id,
        f"🥘 <b>{mention(uid, call.from_user.first_name)}</b> {mention(target_id, target_name)}ga osh tortiq qildi: {eff['text']} 🎉",
    )
...
def start_day(chat_id):
    ...
    send_phase_photo(
        ...
        caption=f"☀️ <b>{game['day_number']}-kun boshlandi!</b>\n"
                f"🗣 Kim mafiya? Muhokama qiling — {DAY_DISCUSSION_SECONDS} soniyadan so'ng "
                "ovoz berish tugmalari DM'ga keladi.\n"
                "<i>Ovoz berish faqat botning shaxsiy chatida bo'ladi.</i>",
        reply_markup=_bot_dm_button(),
    )
...
def resolve_day(chat_id):
    with GAME_LOCK:
        ...
        start_verdict_vote(chat_id, top[0])


def start_verdict_vote(chat_id, candidate_id):
    """🗳 Eng ko'p ovoz olgan ishtirokchi bo'yicha yakuniy hukm — guruh 👍/👎 bilan tasdiqlaydi."""
    game = GAMES.get(chat_id)
    if not game:
        return
    game["verdict_target"] = candidate_id
    game["verdict_votes"] = {}
    game["verdict_open"] = True

    name = game["players"][candidate_id]["name"]
    kb = types.InlineKeyboardMarkup()
    kb.add(
        types.InlineKeyboardButton("👍 Ha, osish", callback_data=f"vd|{chat_id}|yes"),
        types.InlineKeyboardButton("👎 Yo'q, osmaslik", callback_data=f"vd|{chat_id}|no"),
    )
    bot.send_message(
        chat_id,
        f"⚖️ <b>Eng ko'p ovoz olgan ishtirokchi:</b> {mention(candidate_id, name)}\n\n"
        f"Siz rostan ham bu ishtirokchini osmoqchimisiz? Tirik ishtirokchilar, ovoz bering! 👇",
        reply_markup=kb,
    )

    t = threading.Timer(VERDICT_SECONDS, lambda: resolve_verdict(chat_id))
    t.daemon = True
    t.start()
    game["timers"].append(t)


@bot.callback_query_handler(func=lambda c: c.data.startswith("vd|"))
def cb_verdict(call):
    maybe_capture_owner(call.from_user)
    ...
    if not game or not game.get("verdict_open"):
        bot.answer_callback_query(call.id, "Ovoz berish yakunlangan.")
        return
    if uid not in game["players"] or not game["players"][uid]["alive"]:
        bot.answer_callback_query(call.id, "Faqat tirik ishtirokchilar ovoz bera oladi.")
        return
    if uid in game["verdict_votes"]:
        bot.answer_callback_query(call.id, "Siz bu ovoz berishda allaqachon ovoz bergansiz.")
        return

    game["verdict_votes"][uid] = (choice == "yes")
    icon = "👍" if choice == "yes" else "👎"
    bot.answer_callback_query(call.id, f"✅ Ovozingiz qabul qilindi: {icon}")


def resolve_verdict(chat_id):
    with GAME_LOCK:
        game = GAMES.get(chat_id)
        if not game or not game.get("verdict_open"):
            return
        game["verdict_open"] = False
        candidate_id = game.get("verdict_target")

        yes = sum(1 for v in game["verdict_votes"].values() if v)
        no = sum(1 for v in game["verdict_votes"].values() if not v)
        bot.send_message(chat_id, f"🗳 <b>Yakuniy hukm natijasi:</b> 👍 {yes}  —  👎 {no}")

        if candidate_id is not None and yes > no:
            finalize_day(chat_id, candidate_id)
        else:
            bot.send_message(chat_id, "🤝 Aholi rahm qildi (yoki ovozlar teng bo'ldi) — bugun hech kim osilmadi.")
            finalize_day(chat_id, None)


def finalize_day(chat_id, hanged):
    """Yakuniy hukmdan so'ng haqiqiy natija: osish, himoya vositalari va keyingi tunga o'tish."""
    game = GAMES.get(chat_id)
    if not game:
        return

    if hanged is not None and hanged in game["players"] and game["players"][hanged]["alive"]:
        p = game["players"][hanged]
        if p.get("smoke_active"):
            p["smoke_active"] = False
            bot.send_message(
                chat_id,
                f"💨 Aholi {mention(hanged, p['name'])}ni osmoqchi bo'ldi, "
                f"lekin 💣 Tutunli bomba tutuni ostida u qochib qutuldi! Bugun hech kim osilmadi.",
            )
        elif game.get("advokat_target") == hanged:
            bot.send_message(
                chat_id,
                f"⚖️ Aholi {mention(hanged, p['name'])}ni osmoqchi bo'ldi, "
                f"lekin ⚖️ Advokat uni himoya qilgan edi — u ozod qilindi! Bugun hech kim osilmadi.",
            )
        elif get_charges(hanged).get("day_shield", 0) > 0 and is_item_active(hanged, "day_shield"):
            use_charge(hanged, "day_shield")
            bot.send_message(
                chat_id,
                f"🛡 Aholi {mention(hanged, p['name'])}ni osmoqchi bo'ldi, "
                f"lekin u 🛡 Kunduzgi Himoya vositasi tufayli omon qoldi! Bugun hech kim osilmadi.",
            )
        else:
            p["alive"] = False
            role = p["role"]
            bot.send_message(
                chat_id,
                f"⚰️ Aholi ovoz berib, {mention(hanged, p['name'])}ni osdi.\n"
                f"Fosh qilingan roli: <b>{role}</b> edi.",
            )
            # 💣 Terrorist — osilsa, o'ziga ovoz bergan bir kishini ham o'zi bilan olib ketadi
            if role == "Terrorist 💣":
                voters = [v for v, t in game.get("votes", {}).items() if t == hanged and v != hanged and v in game["players"] and game["players"][v]["alive"]]
                if voters:
                    victim = random.choice(voters)
                    game["players"][victim]["alive"] = False
                    bot.send_message(
                        chat_id,
                        f"💥 <b>Terrorist portladi!</b> O'zi bilan birga {mention(victim, game['players'][victim]['name'])}ni "
                        f"ham olib ketdi!\nFosh qilingan roli: <b>{game['players'][victim]['role']}</b>",
                    )
                    wait_last_words(chat_id, victim)
...
def cmd_juftim(message):
    ...
    if partner_id:
        ...
        text = f"💍 <b>Oilaviy holat:</b> Nikohda ❤️\n\nSizning juftingiz: {mention(partner_id, partner_name)}"
    else:
...
# Har bir buyumni "ishlatish" uchun qaysi buyruq/usul kerakligini ko'rsatish uchun —
# nomi bo'yicha butun do'kondagi barcha buyumlarni bitta lug'atga yig'amiz.
_ALL_SHOP_ITEMS_BY_NAME = {}
for _cat_key, (_cat_title, _cat_items) in SHOP_CATEGORIES.items():
    for _ik, _iv in _cat_items.items():
        _ALL_SHOP_ITEMS_BY_NAME[_iv["name"]] = _iv

ITEM_USE_COMMAND_HINT = {
    "poison": "/zahar (kimningdir xabariga reply qilib)",
    "gps": "/gps (kimningdir xabariga reply qilib)",
    "radar": "/qayta_tanlash",
...
def show_inventory(chat_id, uid):
    ...
    if not items and voucher_count == 0:
    ...
    for name, n in counts.items():
        suffix = f" — {n} ta" if n > 1 else ""
        lines.append(f"• {name}{suffix}")
        item = _ALL_SHOP_ITEMS_BY_NAME.get(name)
        if not item:
            continue
        key = item.get("charge_key")
        if key in TOGGLABLE_ITEMS and ch.get(key, 0) > 0:
            state = "✅ FAOL" if is_item_active(uid, key) else "⛔ O'CHIQ"
            kb.add(types.InlineKeyboardButton(f"⚙️ {name} — {state}", callback_data=f"toggleitem|{key}"))
        elif key in ITEM_USE_COMMAND_HINT:
            lines[-1] += f"\n  ↳ Ishlatish: {ITEM_USE_COMMAND_HINT[key]}"
    if voucher_count:
        lines.append(f"\n🥘 Tortiq qilinadigan osh: {voucher_count} ta — /oshtortiq (kimningdir xabariga reply qilib)")
    text = "\n".join(lines)
    _, has_toggleable = build_toggle_menu(uid)
    if has_toggleable:
        text += "\n\n⚙️ Tugmalar orqali avtomatik ishlaydigan buyumlaringizni shu yerdan ham yoqib/o'chirib turing."
    bot.send_message(chat_id, text, reply_markup=kb if kb.keyboard else None)
...
def cb_menu(call):
    ...
    if action == "roles":
        ...
        text = (
            ...
            "/oshtortiq (reply) — 🥘 do'kondan olingan oshni birovga tortiq qilish\n"
            "/kuchlarim — avtomatik buyumlaringizni yoqish/o'chirish\n"
...
def cb_buy(call):
    ...
    if mode == "shield":
        ...
        ch = get_charges(uid)
        vouchers = ch.get("osh_vouchers", [])
        vouchers.append(item["gift_key"])
        ch["osh_vouchers"] = vouchers
        set_charges(uid, ch)
        result_note = "🎁 Tortiq tayyor! Kimgadir sovg'a qilish uchun, uning xabariga reply qilib /oshtortiq deb yozing."
