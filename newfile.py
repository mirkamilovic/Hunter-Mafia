...
class _SafeExceptionHandler(telebot.ExceptionHandler):
    ...
    def handle(self, exception):
        ...
        try:
            ...
            # ⏳ Bir xil xatoni juda tez-tez (spam qilib) yubormaslik uchun oddiy "throttle"
            key = f"errnotify_{hash(err_text) % 100000}"
            last = _ERROR_NOTIFY_CACHE.get(key, 0)
            if time.time() - last > 120:  # bir xil xato uchun kamida 2 daqiqa oralig'i
            ...
            pass  # owner'ga xabar yuborishning o'zi xato bersa ham bot ishlashda davom etadi
...
bot = telebot.TeleBot(
    ...
    threaded=True,
    num_threads=16,
    use_class_middlewares=True,  # 🔧 MUHIM TUZATISH: pyTelegramBotAPI'ning ba'zi versiyalarida
    # middleware_handler() dekoratori (pastda RATE_LIMIT uchun ishlatiladi) FAQAT shu
    # parametr True bo'lganda ishlaydi — aks holda bot HAR BIR xabarda
    # "AttributeError: 'TeleBot' object has no attribute 'typed_middleware_handlers'"
    # xatosini berib, amalda BUTUNLAY ishlamay qolgan edi (har bir buyruq — /start,
    # /top va h.k. — xato bilan tugardi). Bu qatorni qo'shish shu muammoni tubdan hal qiladi.
)
...
# ================================================================================
#  🤖 AVTO-FAQ TIZIMI — "SUN'IY INTELLEKT KABI" AVTOMATIK JAVOBLAR
#  ================================================================================
#  Foydalanuvchilar eng ko'p beradigan ~100 ta savolga avtomatik javob beradi.
#  O'zbek tilida 1-2 ta harf o'rni almashib yozilgan/xato terilgan savollarni ham
#  (masalan "balnas" -> "balans", "duel" -> "deul") o'zi anglab, to'g'ri javobni
#  topib beradi — buning uchun so'z darajasidagi FUZZY (taxminiy) moslashtirish
#  ishlatiladi (difflib.SequenceMatcher asosida, tashqi kutubxonasiz).
#
#  ⚠️ Bu tizim mavjud RATE_LIMIT (spam himoyasi) middleware'i BILAN BIRGA
#  ishlaydi — u allaqachon fayl boshida barcha xabarlarni (shu jumladan FAQ
#  javoblarini ham) RATE_LIMIT_WINDOW/RATE_LIMIT_MAX_MSGS chegarasidan
#  o'tkazadi, shuning uchun FAQ handleri UCHUN alohida limit YOZILMAGAN —
#  aynan shu mavjud limitga mos, uni chetlab o'tmasdan ishlaydi.
# ================================================================================

_FAQ_STOPWORDS = {
    "va", "bu", "shu", "u", "men", "sen", "siz", "biz", "ham", "yoki", "lekin",
    "qanday", "nima", "necha", "qachon", "qayerda", "kim", "uchun", "bilan",
    "bo'ladi", "boladi", "qiladi", "qilaman", "bo'lsa", "bolsa", "mi", "chi",
    "a", "e", "yani", "ya'ni", "endi", "hozir", "meni", "mening", "sizni",
}


def _faq_normalize(text):
    """Matnni FAQ solishtirish uchun soddalashtiradi: kichik harf, apostrof
    variantlarini birlashtiradi, tinish belgilarini olib tashlaydi."""
    if not text:
        return ""
    t = text.lower()
    t = t.replace("’", "'").replace("‘", "'").replace("`", "'").replace("ʻ", "'").replace("ʼ", "'")
    t = re.sub(r"[^a-zo'g'shchʻ0-9\s']", " ", t, flags=re.IGNORECASE)
    t = re.sub(r"\s+", " ", t).strip()
    return t


def _faq_word_ratio(a, b):
    """Ikkita so'z orasidagi o'xshashlik darajasi (0..1) — 1-2 harf almashib/xato
    yozilgan bo'lsa ham yuqori ball beradi (masalan 'balnas' vs 'balans')."""
    if not a or not b:
        return 0.0
    if a == b:
        return 1.0
...
def _faq_phrase_score(user_words, phrase):
    """`phrase` (masalan 'balans qancha') so'zlarining nechtasi foydalanuvchi
    xabaridagi so'zlarga (taxminiy, xato terilgan bo'lsa ham) mos kelishini
    hisoblaydi va 0..1 oralig'ida umumiy ball qaytaradi."""
    phrase_words = [w for w in _faq_normalize(phrase).split(" ") if w]
    if not phrase_words:
        return 0.0
    matched = 0.0
    for pw in phrase_words:
        ...
        for uw in user_words:
            if len(pw) <= 2 and pw != uw:
                continue  # juda qisqa so'zlarda faqat aniq mos kelish hisoblanadi
            r = _faq_word_ratio(pw, uw)
            if r > best:
                best = r
        # 0.78+ o'xshashlik — 1-2 harf farqiga (transpozitsiya/typo) yo'l qo'yiladi
        if best >= 0.78:
            matched += 1.0
        elif best >= 0.6:
            matched += 0.5
...
def match_faq_answer(user_text, min_score=0.62):
    """Foydalanuvchi yozgan erkin matnga eng mos FAQ javobini topadi.
    Hech narsa mos kelmasa None qaytaradi (shunda odatdagidek javob berilmaydi,
    bot jim qoladi yoki chaqiruvchi funksiya boshqa mantiqni davom ettiradi)."""
    norm = _faq_normalize(user_text)
    if not norm or len(norm) < 2:
        return None
    # ⚠️ Stop-so'zlarni BUTUNLAY olib tashlamaymiz — ular ba'zi savol qoliplarining
    # (masalan "meni nega bloklashdi") tarkibiy qismi bo'lishi mumkin. Faqat butun
    # xabar FAQAT stop-so'zlardan iborat bo'lgan holatlar uchun zaxira sifatida saqlanadi.
    user_words = [w for w in norm.split(" ") if w]

    # 🔧 Butun-satr (whole_ratio) taqqoslash faqat QISQA xabarlar (1-2 so'z, masalan
    # "balans" yoki "duel qanday") uchun ishlatiladi — uzunroq gaplarda umumiy
    # SequenceMatcher nisbati chalg'ituvchi (masalan bitta umumiy so'z tufayli noto'g'ri
    # FAQ bilan mos kelib qolishi mumkin), shuning uchun uzun xabarlarda faqat so'z
    # darajasidagi aniqroq _faq_phrase_score ishlatiladi.
    use_whole_ratio = len(user_words) <= 2

    best_answer = None
    best_score = 0.0
    for entry in FAQ_DATA:
        for phrase in entry["q"]:
            score = _faq_phrase_score(user_words, phrase)
            if use_whole_ratio:
                whole_ratio = _faq_word_ratio(norm, _faq_normalize(phrase))
...
# --------------------------------------------------------------------------
# 📚 FAQ MA'LUMOTLAR BAZASI — foydalanuvchilar eng ko'p beradigan ~100 savol
# Har bir yozuv: {"q": [bir nechta o'xshash savol/kalit so'z shakli], "a": javob}
# --------------------------------------------------------------------------
FAQ_DATA = [
    {"q": ["bot nima", "hunter mafia nima", "bu bot nima qiladi", "bot haqida"],
     "a": "🤖 Men <b>Hunter Mafia</b> — guruhda o'ynaladigan mafiya-uslubidagi rolli o'yin botiman. Guruhga /join orqali qo'shiling, tun va kun bosqichlarida topishmoqlarni yeching, do'kon, klan, duel va boshqa ko'plab imkoniyatlardan foydalaning!"},
    {"q": ["qanday o'ynayman", "qanday o'ynash mumkin", "oyinni qanday boshlayman", "ishtirok etish uchun nima qilish kerak"],
     "a": "🎮 O'ynash uchun guruhda /join buyrug'i orqali ro'yxatdan o'ting, yetarli o'yinchi to'plangach admin /startgame orqali o'yinni boshlaydi. Sizga shaxsiy (DM) orqali rolingiz yuboriladi."},
    {"q": ["necha kishi kerak", "kamida nechta odam", "minimum oyinchi soni"],
     "a": "👥 O'yin boshlanishi uchun kamida bir nechta o'yinchi ro'yxatdan o'tishi kerak — aniq son guruh sozlamalariga bog'liq, admin /startgame bilan istalgan vaqt majburan ham boshlashi mumkin."},
    {"q": ["tun necha soniya", "tun qancha davom etadi", "night davomiyligi"],
     "a": "🌙 Tun bosqichi 45 soniya davom etadi — shu vaqt ichida tungi rollar o'z harakatlarini (o'ldirish, davolash, tekshirish va h.k.) DM orqali bajarishlari kerak."},
    {"q": ["kun necha soniya", "kunduzi qancha davom etadi", "day davomiyligi", "muhokama vaqti"],
     "a": "☀️ Kunduz bosqichi jami 90 soniya: dastlab 30 soniya erkin muhokama, so'ng ovoz berish tugmalari chiqadi va yana 60 soniya ovoz berish davom etadi."},
    {"q": ["rollar haqida", "qanday rollar bor", "rol ro'yxati"],
     "a": "🎭 O'yinda mafiya, tinch aholi va bir qancha maxsus rollar (Komissar, Doktor, Serjant, Sehrgar va boshqalar) mavjud. Rolingizni bilish uchun o'yin boshlanganda DM'ga qarang."},
    {"q": ["don kim", "don nima qiladi", "don roli"],
     "a": "🕶 Don — mafiyaning boshlig'i. Tunda mafiya jamoasi bilan birga o'ldiriladigan qurbonni tanlaydi."},
    {"q": ["komissar nima qiladi", "komissar roli", "sherif nima qiladi"],
     "a": "🕵️‍♂️ Komissar (Sherif) — tunda bitta o'yinchini tekshirib, uning mafiya yoki tinch aholi ekanini biladi."},
    {"q": ["doktor nima qiladi", "doktor roli", "davolovchi kim"],
     "a": "💉 Doktor — har kecha bitta o'yinchini (o'zini ham) tanlab, uni o'sha tundagi hujumdan himoya qiladi/davolaydi."},
    {"q": ["mafiya kim", "mafiya jamoasi kim"],
     "a": "🕶 Mafiya — tunlari tinch aholidan birini o'ldirishga harakat qiluvchi yashirin jamoa. Ularning maqsadi sonini tinch aholiga tenglashtirish."},
    {"q": ["tinch aholi kim", "town kim", "yaxshilar kim"],
     "a": "🏘 Tinch aholi — kunduzi muhokama va ovoz berish orqali mafiyani aniqlab, ularni o'yindan chiqarishga harakat qiluvchi jamoa."},
    {"q": ["maxfiy missiya nima", "shaxsiy vazifa nima"],
     "a": "🕵️ Har bir o'yinchiga o'yin boshida shaxsiy (DM orqali) maxfiy missiya beriladi — uni bajarsangiz, o'yin oxirida qo'shimcha +25$ mukofot olasiz."},
    {"q": ["random event nima", "kutilmagan hodisa nima"],
     "a": "🎲 Kunduzi taxminan 15% ehtimol bilan kutilmagan random event yuz beradi — bu o'yin jarayoniga qo'shimcha ta'sir qiladigan syujet hodisasi."},
    {"q": ["profil qanday korish", "profilim", "kabinetim", "hisobim qanday"],
     "a": "👤 Profilingizni /profile buyrug'i orqali ko'rishingiz mumkin — u yerda balans, statistika va boshqa ma'lumotlar ko'rsatiladi."},
    {"q": ["balans qancha", "balansim qancha", "pulim qancha", "balans qanday tekshiriladi"],
     "a": "💰 Balansingizni /profile yoki /hisob buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["dollar qanday olinadi", "pul qanday topiladi", "dollar qanday orttiriladi"],
     "a": "💵 Dollar asosan o'yinda ishtirok etish va g'alaba qozonish orqali (har o'yin oxirida avtomatik hisoblanadi), shuningdek kunlik bonus va boshqa faoliyatlar orqali orttiriladi."},
    {"q": ["olmos qanday olinadi", "olmos qanday topiladi", "diamond qanday"],
     "a": "💎 Olmos ELITE sovg'alari, taqdir g'ildiragi (/taqdir), geroy daromadi va boshqa maxsus faoliyatlar orqali qo'lga kiritiladi, shuningdek Telegram Stars orqali ham sotib olish mumkin."},
    {"q": ["coin nima", "koin nima"],
     "a": "🪙 Coin — botning qo'shimcha valyutasi, uni /olmos_coin buyrug'i orqali Olmosga (yoki aksincha) ayirboshlashingiz mumkin."},
    {"q": ["olmos coin ayirboshlash", "coin almashtirish"],
     "a": "🔄 Olmos va Coin o'rtasida ayirboshlash uchun /olmos_coin buyrug'idan foydalaning."},
    {"q": ["dokon qanday ishlaydi", "magazin qayerda", "shop qanday"],
     "a": "🏪 Do'konni /almazdokon (Olmos do'koni) yoki /dollar_dokon (Dollar do'koni) buyrug'i orqali ochishingiz mumkin — u yerda 27 xil buyumdan xarid qilasiz."},
    {"q": ["inventarim", "inventar qayerda", "buyumlarim"],
     "a": "🎒 Sotib olgan buyumlaringizni /inventar buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["kuchlarim nima", "menda qanday effekt bor"],
     "a": "⚡ Hozirda sizda faol bo'lgan barcha buyum/effektlarni /kuchlarim buyrug'i orqali ko'rasiz."},
    {"q": ["reyting qayerda", "top royxat", "eng kuchlilar royxati"],
     "a": "🏆 Umumiy reytingni /top buyrug'i orqali ko'rishingiz mumkin (g'alabalar/balans bo'yicha)."},
    {"q": ["olmos reytingi", "eng kop olmos kimda"],
     "a": "💎 Olmos bo'yicha reytingni /topalmaz buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["dollar reytingi", "eng boy kim"],
     "a": "💵 Dollar bo'yicha reytingni /topdollar buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["ball reytingi kunlik", "kunlik ball top"],
     "a": "📅 Kunlik ball reytingini /balltop_kun buyrug'i orqali ko'rasiz."},
    {"q": ["ball reytingi haftalik", "haftalik ball top"],
     "a": "📆 Haftalik ball reytingini /balltop_hafta buyrug'i orqali ko'rasiz."},
    {"q": ["ball reytingi oylik", "oylik ball top"],
     "a": "🗓 Oylik ball reytingini /balltop_oy buyrug'i orqali ko'rasiz."},
    {"q": ["umumiy ball reytingi", "mutlaq ball top"],
     "a": "🏅 Umumiy (mutlaq) ball reytingini /balltop_mutlaq buyrug'i orqali ko'rasiz."},
    {"q": ["geroylar royxati", "qanday geroylar bor"],
     "a": "🦸 Barcha geroylar ro'yxatini /heros buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["geroyim kim", "mening geroyim"],
     "a": "🦸 Sizga biriktirilgan geroyni /geroyim buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["geroy bashorati", "bashorat nima"],
     "a": "🔮 Geroy bashoratini /geroy_bashorat buyrug'i orqali olishingiz mumkin."},
    {"q": ["geroy kozi", "geroy koz nima"],
     "a": "👁 Geroy ko'zi imkoniyatidan /geroy_koz buyrug'i orqali foydalanasiz."},
    {"q": ["geroy daromadi", "geroydan foyda"],
     "a": "💰 Geroyingizdan kelayotgan daromadni /geroy_daromad buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["elite nima", "hunter elite nima", "vip nima"],
     "a": "👑 HUNTER ELITE — botning premium (VIP) obunasi: 7/15/30 kunlik tariflar mavjud, Olmos yoki Telegram Stars orqali sotib olinadi. ELITE a'zolar shaxsiy laqab (taxallus), ELITE sirli sandiq va boshqa imtiyozlardan foydalanadi. /elite buyrug'i orqali sotib olasiz."},
    {"q": ["elite narxi qancha", "vip narxi", "elite qancha turadi"],
     "a": "👑 ELITE narxlarini (7/15/30 kunlik) /elite buyrug'ini yuborganingizda bot sizga aniq narxlarni Olmos va Stars'da ko'rsatadi."},
    {"q": ["elite sandiq nima", "sirli sandiq"],
     "a": "💎 ELITE Sirli Sandiqni faqat ELITE a'zolar /elite_sandiq buyrug'i orqali ochishlari mumkin — u yerdan tasodifiy sovg'a chiqadi."},
    {"q": ["taqdir gildiragi", "kunlik omad", "kunlik bonus qanday olinadi"],
     "a": "🔮 Kunlik bonusni /taqdir (Taqdir G'ildiragi) buyrug'i orqali har kuni bir marta aylantirib olishingiz mumkin."},
    {"q": ["til qanday ozgartiriladi", "tilni almashtirish", "language change"],
     "a": "🌐 Botning ba'zi matnlari uchun tilni /til buyrug'i orqali tanlashingiz mumkin (Turkcha/English/Rus/Ukrain/Qozoq/O'zbek/Indonez)."},
    {"q": ["taxallus nima", "laqab qanday qoyiladi", "nickname ornatish"],
     "a": "🏷 Shaxsiy laqab (taxallus) faqat ELITE a'zolar uchun mavjud — /taxallus <laqab> buyrug'i orqali o'rnatasiz. Guruhda chaqirilsa, jamoaviy tasdiqlash ovoz berish ochiladi."},
    {"q": ["sovga qanday yuboriladi", "dostimga sovga berish"],
     "a": "🎁 Boshqa foydalanuvchiga Olmos sovg'a qilish uchun /sovga buyrug'idan foydalaning (reply orqali yoki Telegram ID orqali)."},
    {"q": ["oyindan qanday chiqaman", "lobbydan chiqish"],
     "a": "🚪 ELITE a'zolari o'yin lobbysidan istalgan payt /chiqish buyrug'i orqali chiqishlari mumkin."},
    {"q": ["duel nima", "duelga qanday chaqiraman", "jang qilish"],
     "a": "⚔️ Boshqa o'yinchini duelga (reply orqali) /duel buyrug'i bilan chaqirasiz. G'olib tasodifiy qurol va ehtimollik asosida aniqlanadi."},
    {"q": ["duel narxi qancha", "duel uchun pul kerakmi"],
     "a": "⚔️ Duel boshlash uchun ma'lum miqdorda dollar sarflanadi (100$ atrofida) — aniq narx /duel buyrug'ida ko'rsatiladi."},
    {"q": ["duel statistikam", "necha duel yutdim"],
     "a": "📈 Duel statistikangizni /duel_stat buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["duelda qanday golib chiqaman", "duel utish uchun"],
     "a": "⚔️ Duelda g'alaba ehtimolini oshirish uchun do'kondan ⚔️ Olmos Qilich (65% g'alaba) yoki geroy bonuslaridan foydalanishingiz mumkin."},
    {"q": ["nikoh qanday", "turmush qurish", "unashtirish"],
     "a": "💍 Boshqa foydalanuvchiga turmush qurish taklifini /nikoh buyrug'i orqali yuborishingiz mumkin."},
    {"q": ["juftim kim", "erim xotinim kim"],
     "a": "💑 Juftingiz haqida ma'lumotni /juftim buyrug'i orqali ko'rasiz."},
    {"q": ["ajrashish qanday", "nikohni bekor qilish"],
     "a": "💔 Ajrashish uchun /ajrashish buyrug'idan foydalaning."},
    {"q": ["oila haqida malumot", "oilam haqida"],
     "a": "👨‍👩‍👧 Oilangiz haqida to'liq ma'lumotni /oilainfo buyrug'i orqali ko'rasiz."},
    {"q": ["oilalar reytingi", "eng yaxshi oila"],
     "a": "🏆 Oilalar reytingini /oilatop buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["quchoqlash", "opish"],
     "a": "🤗 Juftingizni quchoqlash/o'pish uchun /quchoqla buyrug'idan foydalaning."},
    {"q": ["juftga sovga", "erimga sovga"],
     "a": "🎁 Juftingizga sovg'a berish uchun /sovgla buyrug'idan foydalaning."},
    {"q": ["xiyonat qilish", "hazil xiyonat"],
     "a": "💔 /xiyonat — bu shunchaki hazil-mutoyiba buyrug'i, real oqibati yo'q."},
    {"q": ["reply orqali sovga berish", "givde nima"],
     "a": "🎁 Kimgadir reply qilib /givde buyrug'ini yuborsangiz, unga sovg'a berasiz."},
    {"q": ["pul sovga berish reply", "givdemoney"],
     "a": "💵 Reply orqali birovga pul sovg'a qilish uchun /givdemoney buyrug'idan foydalaning."},
    {"q": ["coin sovga berish reply", "givdecoin"],
     "a": "🪙 Reply orqali birovga Coin sovg'a qilish uchun /givdecoin buyrug'idan foydalaning."},
    {"q": ["klan qanday yaratiladi", "klan ochish", "yangi klan"],
     "a": "🏛 Klan yaratish uchun /klan buyrug'idan foydalaning — bu bo'lim orqali klaningizni tashkil qilasiz (klan litsenziyasi kerak bo'lishi mumkin)."},
    {"q": ["klanim haqida", "mening klanim"],
     "a": "🏛 Klaningiz haqida to'liq ma'lumotni /klanim buyrug'i orqali ko'rasiz."},
    {"q": ["klanga qanday qoshilaman", "klan azosi bolish"],
     "a": "➕ Mavjud klanga qo'shilish uchun /klanga_qoshil buyrug'idan foydalaning."},
    {"q": ["klandan qanday chiqaman", "klanni tark etish"],
     "a": "🚪 Klandan chiqish uchun /klan_tark buyrug'idan foydalaning."},
    {"q": ["klandan azo chetlatish", "klan azosini haydash"],
     "a": "❌ Klan egasi /klan_chetlash buyrug'i orqali a'zoni klandan chiqarishi mumkin."},
    {"q": ["klan orinbosar tayinlash", "orinbosar qoyish"],
     "a": "🥈 Klan egasi /klan_orinbosar buyrug'i orqali o'rinbosar tayinlaydi."},
    {"q": ["klan azo darajasi", "azo levelini oshirish"],
     "a": "⭐ A'zo darajasini /klan_azo_daraja buyrug'i orqali (klan egasi) oshirishingiz mumkin."},
    {"q": ["klan darajasi qancha", "klan levelim"],
     "a": "📈 Klaningiz darajasini /klan_lvl buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["klan nomini ozgartirish", "klan nomi almashtirish"],
     "a": "✏️ Klan nomini o'zgartirish uchun /klan_nomi buyrug'idan foydalaning."},
    {"q": ["klan maqomi nima", "ritsar sehrgar maqomi"],
     "a": "🎖 Klan a'zolarining maqomini (Ritsar/Sehrgar va h.k.) /klan_maqom buyrug'i orqali ko'rish/boshqarish mumkin."},
    {"q": ["klan xazinasi qancha", "klan hazna"],
     "a": "💰 Klan xazinasidagi mablag'ni /klan_hazna buyrug'i orqali ko'rasiz."},
    {"q": ["klan xazinasini taqsimlash", "hazna pulini bolish"],
     "a": "🤝 Klan xazinasini a'zolar orasida taqsimlash uchun /klan_taqsimla buyrug'idan foydalaning."},
    {"q": ["klanlar royxati", "barcha klanlar"],
     "a": "📋 Botdagi barcha klanlar ro'yxatini /klanlar buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["klanlar jangi nima", "klan urushi"],
     "a": "⚔️ Klanlar o'rtasidagi jang tizimi — boshqa klanga jang taklif qilish uchun /klanjang buyrug'idan foydalaning."},
    {"q": ["oyinni tark etish", "oyindan chiqib ketish"],
     "a": "🚪 O'yin davomida tark etish uchun /tark_et buyrug'idan foydalaning — biroq bu holda mag'lub hisoblanasiz va mukofot berilmaydi."},
    {"q": ["turnir royxati", "turnirda kim bor"],
     "a": "📋 Turnir ishtirokchilari ro'yxatini /turnir_royxat buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["promo kod qanday ishlatiladi", "promokod kiritish"],
     "a": "🎟 Promo-kodni /promo <KOD> buyrug'i orqali ishlatishingiz mumkin — sizga dollar/olmos/coin mukofoti beriladi (kod cheklangan marta ishlatilishi mumkin)."},
    {"q": ["mafiya jamoasiga xabar", "jamoa buyrugi"],
     "a": "💬 Tun vaqtida mafiya jamoasiga yashirin xabar yuborish uchun /jamoa buyrug'idan foydalaning (faqat mafiya a'zolari uchun)."},
    {"q": ["qonun jamoasiga xabar", "adolat buyrugi"],
     "a": "💬 Tun vaqtida qonun (Komissar/Serjant) jamoasiga yashirin xabar yuborish uchun /adolat buyrug'idan foydalaning."},
    {"q": ["zahar buyumi", "birovni zaharlash"],
     "a": "☠️ 🧪 Zaharli flakon buyumini sotib olgan bo'lsangiz, /zahar buyrug'i orqali birovni zaharlaysiz — doktor davolamasa, u tun yakunida halok bo'ladi."},
    {"q": ["gps buyumi", "birovning holatini bilish"],
     "a": "📍 📍 GPS Mayak buyumi bo'lsa, /gps buyrug'i orqali birovning tirik yoki o'lganligini bilib olasiz."},
    {"q": ["kompas buyumi", "birovning tarafini bilish"],
     "a": "🧭 🧭 Sirli kompas buyumi bilan /kompas buyrug'i orqali birovning qaysi tarafda (mafiya/tinch aholi/mustaqil) ekanini bilasiz."},
    {"q": ["tutun bombasi", "tutun buyumi"],
     "a": "💨 💨 Tutun bombasi buyumini /tutun buyrug'i orqali ishlatishingiz mumkin."},
    {"q": ["fonar buyumi", "fonar nima qiladi"],
     "a": "🔦 🔦 Fonar buyumini /fonar buyrug'i orqali ishlatasiz."},
    {"q": ["rolni qayta tanlash", "rol almashtirish buyumi"],
     "a": "🔁 📡 Maxfiy radar buyumi bo'lsa, /qayta_tanlash buyrug'i orqali o'z rolingizni qayta tasodifiy tanlaysiz."},
    {"q": ["qora bozorda sotish", "sell buyrugi"],
     "a": "🛒 Inventaringizdagi buyumni Qora bozorda sotuvga qo'yish uchun /sell buyrug'idan foydalaning."},
    {"q": ["qora bozordan sotib olish", "buy buyrugi"],
     "a": "🛍 Qora bozordagi faol e'lonlardan xarid qilish uchun /buy buyrug'idan foydalaning."},
    {"q": ["bozor qanday korinadi", "market royxati"],
     "a": "🏬 Qora bozordagi faol e'lonlarni /market buyrug'i orqali ko'rishingiz mumkin."},
    {"q": ["botni guruhga qoshish", "guruhga qanday qoshaman"],
     "a": "➕ Botni istalgan guruhga oddiy a'zo sifatida qo'shishingiz va admin huquqi berishingiz kifoya — shundan so'ng guruhda o'yin boshlash mumkin bo'ladi."},
    {"q": ["admin bilan boglanish", "egasi bilan aloqa", "shikoyat qayerga"],
     "a": "📩 Bot bo'yicha savol yoki muammo bo'lsa, bot egasi bilan bog'lanishingiz mumkin."},
    {"q": ["bot sekin ishlayapti", "javob bermayapti", "ishlamayapti"],
     "a": "⏳ Ba'zida server yuklamasi tufayli javob biroz kechikishi mumkin — bir necha soniya kutib ko'ring. Agar muammo davom etsa, admin bilan bog'laning."},
    {"q": ["xatolik yuz berdi nima qilaman", "error chiqdi"],
     "a": "⚠️ Xatolik yuz bersa, xabarni qayta yuborib ko'ring. Muammo davom etsa, screenshot bilan admin/bot egasiga murojaat qiling."},
    {"q": ["ban boldim nega", "meni nega bloklashdi"],
     "a": "🚫 Ban odatda qoidabuzarlik (spam, haqorat va h.k.) uchun qo'llaniladi. Sabab haqida bot egasi/admin bilan bog'lanishingiz mumkin."},
    {"q": ["ban qanday olib tashlanadi", "unban qanday"],
     "a": "✅ Blokdan chiqarish faqat admin/bot egasi tomonidan amalga oshiriladi — u bilan bog'laning."},
    {"q": ["vip va elite farqi", "vip elite bir xilmi"],
     "a": "👑 ELITE — botdagi VIP tizimining yangi nomi, eski VIP infratuzilmasi ustiga qurilgan va u bilan to'liq mos: muddatlar bir-biriga qo'shiladi."},
    {"q": ["eng faol oyinchi mukofoti", "faollik ball"],
     "a": "🏅 Har o'yin oxirida eng faol o'yinchiga qo'shimcha +25 ball beriladi (harakatlar soni bo'yicha aniqlanadi)."},
    {"q": ["kim golib deb hisoblanadi", "galaba sharti"],
     "a": "🏆 Mafiya soni tinch aholi juftliklariga tenglashsa — mafiya g'olib; mafiya butunlay yo'q qilinsa — tinch aholi g'olib deb hisoblanadi."},
    {"q": ["ovoz berish qanday ishlaydi", "kunduzi ovoz berish"],
     "a": "🗳 Kunduzgi muhokamadan so'ng ovoz berish tugmalari chiqadi — eng ko'p ovoz to'plagan o'yinchi guruhdan chiqariladi."},
    {"q": ["tunda kim kimni oldiradi", "tungi olim"],
     "a": "🌙 Tunda mafiya jamoasi (Don boshchiligida) qurbonni tanlaydi; agar doktor o'sha kishini davolasa, u tirik qoladi."},
    {"q": ["tez tez yozsam nima boladi", "spam qilsam"],
     "a": "⏳ Juda tez-tez xabar yuborsangiz, spam himoyasi vaqtincha xabaringizni cheklab qo'yishi mumkin — biroz sekinroq yozing."},
    {"q": ["stars orqali tolov", "telegram stars nima"],
     "a": "⭐ Telegram Stars orqali to'lov qilsangiz, muvaffaqiyatli o'tgach ELITE yoki Hunter Coin avtomatik hisobingizga qo'shiladi."},
    {"q": ["boshlangich balans qancha", "yangi royxatdan otganda balans"],
     "a": "🆕 Yangi ro'yxatdan o'tgan foydalanuvchilar 0$ / 0💎 balans bilan boshlaydi — mavjud foydalanuvchilarning balansi o'zgarmaydi."},
    {"q": ["mening statistikam", "necha oyin oynadim"],
     "a": "📊 O'yin statistikangiz (o'ynagan o'yinlar, g'alabalar soni) /profile buyrug'ida ko'rsatiladi."},
    {"q": ["rahmat", "raxmat", "tashakkur"],
     "a": "🙏 Arzimaydi! Yana savolingiz bo'lsa, bemalol yozavering."},
    {"q": ["salom", "assalomu alaykum", "hi", "hello"],
     "a": "👋 Salom! Men Hunter Mafia botiman. Savolingiz bo'lsa yozing, yordam beraman — yoki /start buyrug'i bilan boshlang."},
    {"q": ["yordam kerak", "help", "nima qila olasan"],
     "a": "🆘 Sizga o'yin qoidalari, do'kon, klan, duel va boshqa bo'limlar bo'yicha yordam bera olaman. Aniq savolingizni yozing — masalan \"balansim qancha\" yoki \"klan qanday tuziladi\"."},
]


# ================================================================================
#  🎉 G'OLIBNI TABRIKLASH — HAR SAFAR HAR XIL, "SUN'IY INTELLEKT" USLUBIDA
#  ================================================================================
#  O'yin tugagach g'alaba qozongan o'yinchi(lar)ga xuddi sun'iy intellekt kabi,
#  har gal boshqacha so'z bilan tabrik yuboriladi. Foydalanuvchi ismi ISHLATILGAN
#  har bir joyda mention() orqali HAR DOIM @tag (bosilsa profiliga o'tadigan
#  havola) qilib beriladi — bu talab qat'iy saqlanadi.
# ================================================================================

WINNER_CONGRATS_TEMPLATES = [
    "🎉 Tabriklayman, {mention}! Bu safar strategiyangiz benuqson chiqdi — {reward} sizniki!",
    "🏆 {mention}, ajoyib o'yin edi! Aql bilan harakat qildingiz va g'alaba qozondingiz — {reward} hisobingizga tushdi.",
    "✨ Voy, {mention}! Sizni kuzatib turdim — har bir qadamingiz o'ylangan edi. Zo'r g'alaba, {reward} sizga tegishli!",
    "🥂 {mention}, mana bu chinakam ustalik! G'alabangiz bilan qutlayman, {reward} yutib oldingiz.",
    "🔥 {mention} — bugungi kechaning haqiqiy qahramoni! Menimcha, hech kim sizni tutolmadi. {reward} — mukofotingiz!",
    "🌟 {mention}, tabriklayman! Bunday sovuqqonlik va aql bilan o'ynash har kimning qo'lidan kelmaydi. +{reward}!",
    "🎊 Ishonchim komil edi — {mention} g'olib chiqadi! Zo'r natija, {reward} hisobingizga qo'shildi.",
    "🏅 {mention}, sizni tabriklashdan mamnunman! Bu g'alaba tasodif emas — mahoratingiz natijasi. {reward} sizniki.",
    "💫 {mention} — bugun omad emas, aynan aql-idrok g'alaba qozondi! Mukofot: {reward}.",
    "🎯 To'g'ridan-to'g'ri nishonga urdingiz, {mention}! Chin dildan tabriklayman, {reward} qo'lga kiritdingiz.",
    "🙌 {mention}, zo'r o'ynadingiz! Men sizning har bir harakatingizni tahlil qildim va bu — haqli g'alaba. +{reward}!",
    "👑 Bugungi kechaning qiroli/malikasi — {mention}! Tabriklayman, {reward} hisobingizga yozildi.",
    "🚀 {mention}, siz hammadan bir qadam oldinda edingiz! Ajoyib g'alaba, {reward} sizniki bo'ldi.",
    "🎆 Zo'r yakun, {mention}! Bunday o'yinni ko'rish men uchun ham qiziqarli bo'ldi. Tabriklayman — +{reward}!",
    "🍀 {mention}, aqlingiz va sabringiz o'z samarasini berdi! Chin yurakdan tabriklayman, mukofot {reward}.",
]


def get_ai_winner_congrats(uid, name, reward):
    """🎉 Har safar boshqacha, sun'iy intellekt kabi tabrik matnini qaytaradi
    (SHAXSIY DM uchun — yutuq miqdori ko'rsatiladi). Foydalanuvchi ismi HAR
    DOIM mention() orqali @tag qilib qo'yiladi."""
    template = random.choice(WINNER_CONGRATS_TEMPLATES)
    reward_txt = f"<b>+{reward}$</b>" if reward else "mukofot"
    return template.format(mention=mention(uid, name), reward=reward_txt)


# 🏛 GURUH uchun alohida shablonlar — mavjud dizayn qoidasiga ko'ra guruhga
# yuboriladigan xabarlarda PUL/BALL miqdori ko'rsatilmaydi, shuning uchun bu
# shablonlarda {reward} joy-belgisi ISHLATILMAYDI.
GROUP_WINNER_CONGRATS_TEMPLATES = [
    "🎉 {mention}, tabriklayman! Bugungi kechaning g'olibi — aynan siz!",
    "🏆 Zo'r o'yin, {mention}! Aql va sovuqqonlik bilan g'alabaga erishdingiz.",
    "✨ {mention} — ajoyib strategiya namoyish etdi va yutdi! Tabriklaymiz!",
    "🥂 Bugun {mention} hammadan bir qadam oldinda bo'ldi. Chin dildan tabriklayman!",
    "🔥 {mention} — kechaning haqiqiy qahramoni! Zo'r g'alaba!",
    "🌟 Tabriklayman, {mention}! Bunday o'ynash har kimning qo'lidan kelmaydi.",
    "🎊 {mention} g'olib chiqdi — va bu, albatta, tasodif emas edi!",
    "🏅 {mention}, mahoratingiz bilan hammani hayratda qoldirdingiz. Tabriklayman!",
    "👑 Bugungi kechaning qahramoni — {mention}! Zo'r g'alaba uchun rahmat.",
    "🚀 {mention}, siz g'alabaga chinakam munosib edingiz. Tabriklayman!",
]


def get_group_winner_congrats(uid, name):
    """🎉 Guruhga yuboriladigan, yutuq miqdorisiz, har safar boshqacha tabrik matni."""
    template = random.choice(GROUP_WINNER_CONGRATS_TEMPLATES)
    return template.format(mention=mention(uid, name))
...
def private_message_router(message):
    ...
    for chat_id, game in list(GAMES.items()):
        if message.from_user.id in game.get("last_words_wait", set()):
            ...
            bot.send_message(chat_id, f"🗣 <b>{mention(uid, name)}</b>ning so'nggi so'zi:\n«{message.text}»")
            return
    # 🤖 Avto-FAQ: yuqoridagi holatlarning hech biriga to'g'ri kelmasa (o'yinchi
    # oddiy erkin savol yozgan bo'lsa), sun'iy intellekt kabi eng mos javobni
    # topib beramiz — 100+ ta tayyor savol-javob asosida, xato/harflar almashgan
    # yozuvlarni ham tushunadi. Bu handler ham mavjud RATE_LIMIT middleware
    # chegarasidan (fayl boshidagi RATE_LIMIT_WINDOW/RATE_LIMIT_MAX_MSGS)
    # o'tgan xabarlar uchungina ishga tushadi — alohida chetlab o'tish yo'q.
    faq_answer = match_faq_answer(message.text or "")
    if faq_answer:
        uid = message.from_user.id
        name = (message.from_user.first_name or message.from_user.username or "Do'st")
        bot.send_message(message.chat.id, f"{mention(uid, name)}, {faq_answer}")
    ...
    func=lambda m: (
        ...
        and (
            (m.reply_to_message is not None
             ...
             and BOT_USERNAME
             and m.reply_to_message.from_user.username
             and m.reply_to_message.from_user.username.lower() == BOT_USERNAME.lower())
            or (BOT_USERNAME and f"@{BOT_USERNAME.lower()}" in (m.text or "").lower())
        )
    ),
)
def group_faq_router(message):
    """🤖 Guruhda botga REPLY qilib yoki botni @mention qilib savol yozilsa,
    xuddi shaxsiy chatdagidek FAQ tizimi orqali avtomatik javob beradi.
    Foydalanuvchi ismi har doim @tag (mention) qilingan holda javob beriladi."""
    maybe_capture_owner(message.from_user)
    ...
    if faq_answer:
        uid = message.from_user.id
        name = (message.from_user.first_name or message.from_user.username or "Do'st")
        bot.send_message(message.chat.id, f"{mention(uid, name)}, {faq_answer}", reply_to_message_id=message.message_id)
...
def end_game(chat_id, winners_team):
    ...
    for uid, p in game["players"].items():
        ...
        if left:
            ...
            # 🎉 Har safar har xil, sun'iy intellekt kabi tabrik (DM'da yutuq miqdori bilan)
            outcome_line = get_ai_winner_congrats(uid, p["name"], reward) + points_note
        else:
    ...
    # 🎉 G'oliblarni guruhda ham, xuddi sun'iy intellekt kabi, har safar boshqacha
    # so'zlar bilan alohida tabriklaymiz — ism har doim @tag (mention) qilingan holda.
    if winners:
        congrats_lines = [get_group_winner_congrats(_uid, game["players"][_uid]["name"]) for _uid, _e in winners]
        bot.send_message(chat_id, "\n".join(congrats_lines))
...
def cb_menu(call):
    ...
    if action == "roles":
        ...
        bot.send_message(call.message.chat.id, _text, reply_markup=_top_page_markup("wins", 0, _tp))
        bot.answer_callback_query(call.id)
