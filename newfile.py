...
FAQ_KB = [
    ...
    {"keywords": ["hunter coin", "coin nima", "coin qanday"],
     "answer": "🪙 Hunter Coin — maxsus valyuta, klan litsenziyasi kabi buyumlar shu orqali sotib olinadi. Telegram Stars orqali <code>/birja</code> menyusidan olasiz."},
...
def build_top_text(page=0):
    rows = get_top_players(limit=50)  # endi 10 emas — 50 tagacha, sahifalab ko'rsatiladi
    if not rows:
...
def get_top_by_field(field, limit=50):
...
def build_top_diamond_text(page=0):
...
def build_top_dollar_text(page=0):
...
def _top_page_markup(kind, page, total_pages):
    if total_pages <= 1:
        return None
    kb = types.InlineKeyboardMarkup()
    row = []
    if page > 0:
        row.append(types.InlineKeyboardButton("◀️", callback_data=f"topPage|{kind}|{page - 1}"))
    row.append(types.InlineKeyboardButton(f"{page + 1}/{total_pages}", callback_data="topPage|noop|0"))
...
def cb_top_page(call):
    ...
    try:
        ...
        if not builder:
            ...
            return
        text, total_pages = builder(page)
        kb = _top_page_markup(kind, page, total_pages)
        try:
...
def show_profile(chat_id, uid, name):
    ...
    bot.send_message(chat_id, text, reply_markup=with_back_button())
...
def cb_menu(call):
    ...
    if action == "roles":
        ...
        _text, _tp = build_top_text()
        bot.send_message(call.message.chat.id, _text, reply_markup=with_back_button(_top_page_markup("wins", 0, _tp)))
        bot.answer_callback_query(call.id)
...
def cb_exchange_to_diamond(call):
    try:
        ...
        uid = call.from_user.id
        u = user_dict(uid, call.from_user.first_name)
        # 🔧 XATOLIK TUZATILDI: avval 10 🪙 evaziga 60-100 💎 berilar edi — bu narx
        # to'g'ridan-to'g'ri Stars orqali Olmos sotib olish narxidan (7⭐️=1💎) 10x
        # arzonroq edi va deyarli bepul Olmos manbaiga aylangan. Endi muvozanatli.
        dynamic_diamond_rate = random.randint(6, 10)
...
def cmd_top(message):
    text, tp = build_top_text()
    kb = _top_page_markup("wins", 0, tp)
    if is_owner_channel(message.chat):
...
def cmd_top_diamond(message):
    text, tp = build_top_diamond_text()
    kb = _top_page_markup("diamond", 0, tp)
    if is_owner_channel(message.chat):
        bot.send_message(message.chat.id, text, reply_markup=kb)
        return
    maybe_capture_owner(message.from_user)
...
def cmd_top_dollar(message):
    text, tp = build_top_dollar_text()
    kb = _top_page_markup("dollar", 0, tp)
    if is_owner_channel(message.chat):
        bot.send_message(message.chat.id, text, reply_markup=kb)
        return
    maybe_capture_owner(message.from_user)
...
TUNABLE_SETTINGS = [
    ...
    ("DIAMOND_TO_COIN_RATE", "💎➜🪙 Necha olmos = 1 Hunter Coin", int),
...
_HELP_CATEGORIES = [
    ...
    ("🛒 Do'kon, bozor va ELITE", ["almazdokon", "dollar_dokon", "olmos_coin", "sell", "buy",
