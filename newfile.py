...
FAQ_KB = [
    ...
    {"keywords": ["elite qancha kun", "elite muddati", "elite qancha vaqt", "elite qolgan"],
     "answer": "📅 ELITE muddatingizni /profile orqali yoki /elite menyusining yuqori qismidan ko'rishingiz mumkin — u yerda necha kun qolgani aniq yozilgan."},
    ...
    {"keywords": ["nikoh qanday", "turmush qurish", "uylanish", "sevgilim qilish"],
     "answer": "💍 Nikoh qurish uchun guruhda o'sha kishining xabariga <b>Reply</b> qilib <code>/nikoh</code> deb yozing."},
    {"keywords": ["ajrashish", "nikohni bekor qilish", "ajralish"],
     "answer": "💔 Ajrashish uchun <code>/ajrashish</code> deb yozing."},
    {"keywords": ["juftim kim", "sevgilim kim", "turmush ortogim"],
     "answer": "❤️ Juftingizni bilish uchun <code>/juftim</code> deb yozing."},
    ...
    {"keywords": ["balansim", "pulim qancha", "dollarim qancha", "necha pulim bor"],
     "answer": "💰 Balansingizni <code>/profile</code> yoki <code>/inventar</code> orqali ko'rishingiz mumkin."},
    {"keywords": ["profile nima", "kabinet", "shaxsiy sahifa"],
     "answer": "👤 Shaxsiy kabinetingizni ko'rish uchun <code>/profile</code> deb yozing."},
    ...
    {"keywords": ["sovga qilish", "sovga berish", "givde"],
     "answer": "🎁 Boshqa o'yinchiga sovg'a berish uchun uning xabariga reply qilib <code>/Givde</code>, <code>/GivdeMoney</code> yoki <code>/GivdeCoin</code> dan foydalaning."},
...
def _tf_elite_status(uid, elite):
    if elite:
        return "👑 Siz hozir ELITE a'zosiz! Qolgan muddatni /profile orqali ko'rishingiz mumkin. ELITE imtiyozlaringizdan (laqab, sirli sandiq, lobbydan chiqish) to'liq foydalaning!"
...
def _tf_marriage(uid, elite):
    u = user_dict(uid)
    if u.get("married_to"):
        return "❤️ Siz nikohdasiz! Juftingiz kimligini /juftim orqali ko'rishingiz mumkin."
    return "💍 Siz hozircha yolg'izsiz. Nikoh qurish uchun guruhda yoqtirgan odamingizning xabariga Reply qilib /nikoh deb yozing."
...
def mention(uid, name):
    ...
    try:
        ...
        if elite_until and elite_until > time.time():
            ...
            # 🦁 Faqat BITTA sher emojisi, undan keyin darhol chiroyli qalin "𝓔𝓵𝓲𝓽𝓮" yozuvi,
            # so'ng ismning o'zi keladi: 🦁 𝓔𝓵𝓲𝓽𝓮 Ism (ikkinchi/orqadagi emoji olib tashlandi).
            prefix += lion_tag + " <b>𝓔𝓵𝓲𝓽𝓮</b> "
...
#        💍 Para: FAQAT botda rasmiy nikohdagi (/nikoh) juftliklar qo'shiladi.
...
def cmd_new_team(message):
    _open_special_lobby(message, "squad")


@bot.message_handler(commands=["povtor_team"])
def cmd_povtor_team(message):
...
def cmd_stop_team(message):
    _stop_special_game(message, "squad")


@bot.message_handler(commands=["new_para", "parastart"])
def cmd_parastart(message):
    _open_special_lobby(message, "couple")


@bot.message_handler(commands=["povtor_para"])
def cmd_povtor_para(message):
    _povtor_special_lobby(message, "couple")


@bot.message_handler(commands=["start_para", "paraforce"])
def cmd_paraforce(message):
    _start_special_game(message, "couple")


@bot.message_handler(commands=["stop_para", "parastop"])
def cmd_parastop(message):
    _stop_special_game(message, "couple")
...
PARA_GAME_RULES_TEXT = (
    ...
    "• FAQAT botda RASMIY nikohdagi (/nikoh) juftliklar \"💍 Men va turmush o'rtog'imni "
...
def cb_couple_join(call):
    ...
    if not spouse_id:
        bot.answer_callback_query(call.id, "❌ Bu — Para o'yin, FAQAT rasmiy nikohdagi juftliklar qo'shila oladi. Avval /nikoh orqali oila quring.", show_alert=True)
        return
...
def show_profile(chat_id, uid, name):
    ...
    bot.send_message(chat_id, text, reply_markup=with_back_button(kb))
...
def show_inventory(chat_id, uid):
    ...
    if not items and voucher_count == 0 and not hero:
        bot.send_message(chat_id, "📦 Sizning inventaringiz hozircha bo'sh.\n🛒 Do'kondan buyum sotib oling — /market", reply_markup=with_back_button())
        return
    ...
    bot.send_message(chat_id, "\n".join(lines), reply_markup=with_back_button(kb))
...
def cb_menu(call):
    ...
    # 🖼 "Jonli panel": foydalanuvchi menyuda navigatsiya qilganda, bosilgan
    # eski ekran DARHOL o'chiriladi — chat toza qoladi, faqat bitta joriy
    # ekran ko'rinadi (aynan shu bosqichda, har bir action ichida alohida
    # o'chirish yozib o'tirish shart bo'lmasin deb, bir joyda hal qilingan).
    try:
        bot.delete_message(call.message.chat.id, call.message.message_id)
    except Exception:
        pass
    ...
    if action == "roles":
        ...
        bot.send_message(call.message.chat.id, format_active_effects(uid), reply_markup=with_back_button())
        bot.answer_callback_query(call.id)
        ...
        with db_lock:
            ...
            row = cur.fetchone()
        if row:
            bot.send_message(call.message.chat.id, f"🏛 Sizning klaningiz: <b>{row[0]}</b>", reply_markup=with_back_button())
        else:
            bot.send_message(call.message.chat.id, "Sizda hali klan yo'q. 🏛 Klan litsenziyasini Hunter Coin do'konidan sotib olib <code>/klan Nomi</code> deb yozing.", reply_markup=with_back_button())
        bot.answer_callback_query(call.id)
        ...
        bot.send_message(call.message.chat.id, _text, reply_markup=_kb)
        bot.answer_callback_query(call.id)
        ...
        bot.send_message(call.message.chat.id, "🏅 Qaysi davr bo'yicha reytingni ko'rmoqchisiz?", reply_markup=with_back_button(kb2))
        bot.answer_callback_query(call.id)
        ...
        partner = get_partner_mention(uid)
        if partner:
            bot.send_message(call.message.chat.id, f"💍 <b>Oilaviy holat:</b> Nikohda ❤️\nJuftingiz: {partner}\n\nAjrashish uchun /ajrashish deb yozing.", reply_markup=with_back_button())
        else:
            bot.send_message(call.message.chat.id, "👤 <b>Oilaviy holat:</b> Bo'ydoq.\n\nBoshqa o'yinchi bilan nikoh qurish uchun guruhda uning xabariga <b>Reply</b> qilib /nikoh deb yozing!", reply_markup=with_back_button())
        bot.answer_callback_query(call.id)

    elif action == "duel_info":
        bot.send_message(call.message.chat.id, "⚔️ Boshqa o'yinchi bilan duel o'ynash uchun guruhda uning xabariga <b>Reply</b> qilib /duel deb yozing!", reply_markup=with_back_button())
        bot.answer_callback_query(call.id)
        ...
        bot.send_message(call.message.chat.id, text, reply_markup=with_back_button())
        bot.answer_callback_query(call.id)
        ...
        if u["last_bonus_date"] == today:
            ...
            bot.send_photo(call.message.chat.id, MAIN_PHOTO, caption="🌙 <b>Hunter Mafia</b> — bosh menyu\n\nKerakli bo'limni tanlang 👇", reply_markup=get_main_menu(None, uid))
            return
        ...
        # 🖼 bonus — alert orqali ko'rsatiladi, panel o'chirilgan edi — bosh menyuni qayta chiqaramiz
        bot.send_photo(call.message.chat.id, MAIN_PHOTO, caption="🌙 <b>Hunter Mafia</b> — bosh menyu\n\nKerakli bo'limni tanlang 👇", reply_markup=get_main_menu(None, uid))
...
def open_shop_main(chat_id):
    ...
    bot.send_message(chat_id, "🛒 Do'konga xush kelibsiz! Kerakli bo'limni tanlang:", reply_markup=with_back_button(kb))
...
def cb_admin_elite_price_reset(call):
    ...
    _save_elite_plans()
    text, kb = _elite_price_panel_text_and_kb()
    bot.send_message(call.message.chat.id, "🔁 ELITE narxlari standart holatga qaytarildi.\n\n" + text, reply_markup=kb)
...
def cb_admin_show_help(call):
    ...
    if not is_owner(call.from_user.id):
        ...
        return
    bot.answer_callback_query(call.id)
    bot.send_message(call.message.chat.id, _build_help_text(True))
...
USER_COMMANDS = [
    ...
    ("new_team", "🐺🦅 Yangi Jamoaviy o'yin ro'yxatini ochish (admin)"),
    ("povtor_team", "🔁 Jamoaviy o'yin xabarini qayta yuborish (admin)"),
    ("start_team", "▶️ Jamoaviy o'yinni yakunlab, g'olibni chiqarish (admin)"),
    ("stop_team", "🛑 Jamoaviy o'yinni butunlay bekor qilish (admin)"),
    ("new_para", "💍 Yangi Para o'yin ro'yxatini ochish (admin)"),
    ("povtor_para", "🔁 Para o'yin xabarini qayta yuborish (admin)"),
    ("start_para", "▶️ Para o'yinni boshlash (admin)"),
    ("stop_para", "🛑 Para o'yinni butunlay bekor qilish (admin)"),
...
_HELP_CATEGORIES = [
    ...
    ("👤 Profil va reyting", ["profile", "inventar", "kuchlarim", "hisob", "top", "topalmaz",
                              "topdollar", "balltop_kun", "balltop_hafta", "balltop_oy", "balltop_mutlaq"]),
    ...
    while True:
        try:
            ...
            if not _state_restored:
                ...
                # 🎴 Mini App'ni shaxsiy chatning pastki (chap) tugmasiga o'rnatish — URL
                # haqiqiy hosting manziliga almashtirilgandan keyingina yoqiladi.
                if MINIAPP_URL.startswith("https://") and "SIZNING-USERNAME" not in MINIAPP_URL:
                    try:
                        bot.set_chat_menu_button(
                            menu_button=types.MenuButtonWebApp(text="👤 Profil", web_app=types.WebAppInfo(url=MINIAPP_URL))
                        )
                        _logger.info("🎴 Mini App menyu tugmasi o'rnatildi: %s", MINIAPP_URL)
                    except Exception as _miniapp_err:
                        _logger.warning("Mini App menyu tugmasini o'rnatishda xatolik: %s", _miniapp_err)
