...
def theron_winner_congrats(chat_id, winner_names, team_name):
    """O'yin oxirida g'oliblarni AI orqali shaxsiy tabriklaydi."""
    if not theron_available() or not winner_names:
        return
    try:
        ...
        _logger.warning("Theron g'alaba tabrigida xatolik (muhim emas): %s", e)
...
def cmd_theron(message):
    """🧠 Theron AI bilan suhbat — savolsiz chaqirilsa tanishtiruv, savol bilan
    chaqirilsa (ELITE/limit asosida) shaxsiy AI javobi beradi."""
    try:
        ...
        if not theron_available():
            bot.reply_to(
                ...
                "Hozircha AI xizmati botda sozlanmagan (bot egasi GEMINI_API_KEY ni o'rnatishi kerak). "
                "Boshqa buyruqlar odatdagidek to'liq ishlayveradi."
            )
            return
        ...
        if not allowed:
            bot.reply_to(
                message,
                f"⏳ Bugungi {THERON_NAME} bilan suhbat limitingiz tugadi.\n"
