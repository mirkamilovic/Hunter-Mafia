...
#  💎 VIP SIRLI SANDIQ — /vip_sandiq (faqat HUNTER ELITE uchun, kuniga 1 marta)
#
...
VIP_BOX_PRIZES = [
    (35, "dollar", 80, "💵 VIP kumush sovg'a"),
    (28, "diamond", 2, "💎 VIP olmos sovg'a"),
    (20, "coin", 20, "🪙 VIP coin sovg'a"),
    (12, "dollar", 250, "🏆 VIP oltin sovg'a"),
    (5, "jackpot", 0, "👑✨ VIP AFSONAVIY SANDIQ"),
]


@bot.message_handler(commands=["vip_sandiq", "vipbox"])
def cmd_vip_sandiq(message):
    """💎 VIP SIRLI SANDIQ — FAQAT HUNTER ELITE obunachilari uchun, kuniga 1
    ...
    if not is_elite(uid):
        bot.send_message(
            message.chat.id,
            "🔒 VIP Sirli Sandiq faqat <b>HUNTER ELITE</b> obunachilari uchun ochiq.\n"
    ...
    if get_setting(key) == today:
        bot.send_message(message.chat.id, "⏳ Siz bugungi VIP sandiqni allaqachon ochgansiz. Ertaga qayting!")
        return
    set_setting(key, today)

    sent = bot.send_message(message.chat.id, "💎 <b>VIP Sirli Sandiq ochilmoqda...</b> 🔒➜🔓")
    time.sleep(0.6)

    weights = [w for w, *_ in VIP_BOX_PRIZES]
    _, ptype, amount, label = random.choices(VIP_BOX_PRIZES, weights=weights, k=1)[0]
    mult = luck_mult(uid)

    if ptype == "jackpot":
        j_dollar, j_diamond, j_coin = int(500 * mult), 12, 60
        add_balance(uid, dollar=j_dollar, diamond=j_diamond, coin=j_coin)
        result_text = (
            f"👑✨ <b>AFSONAVIY VIP SANDIQ!!!</b> ✨👑\n"
            f"💵 +{j_dollar}$  💎 +{j_diamond}  🪙 +{j_coin}\n\n"
...
def resolve_night(chat_id):
    with GAME_LOCK:
        ...
        for slug in ("general", "arxitektor", "qorovul", "sadoqat"):
            act = extra.get(slug)
            if act and act["target"] != "skip":
        ...
        if telba_act and telba_act["target"] != "skip":
            if telba_triggered:
                ...
                outcome = "halok bo'ldi" if telba_act["target"] in killed else "himoya tufayli omon qoldi"
                safe_send(telba_act["actor"], f"🤪 Bu safar tasodifiy harakatingiz ishladi! {mention(telba_act['target'], t_name)} {outcome}.")
            else:
                safe_send(telba_act["actor"], "🤪 Bu safar tasodifingiz kelmadi — hech narsa bo'lmadi.")
...
def end_game(chat_id, winners_team):
    ...
    for uid, p in game["players"].items():
        ...
        entry = f"{mention(uid, p['name'])} — <b>{p['role']}</b>" + (f" (+{reward}$)" if reward else " (🚪 tark etgan)")
    ...
    if most_active_uid and most_active_uid in game["players"]:
        lines.append(f"\n🏅 Eng faol o'yinchi: {mention(most_active_uid, game['players'][most_active_uid]['name'])} (+25 ball)")
...
def successful_payment_handler(message):
    ...
    if kind == "hc":
        ...
        # 👑 HUNTER ELITE — tarifli (7/15/30 kun) obuna, Stars orqali to'langanda shu yerga tushadi
...
def cb_admin_panel(call):
    ...
    kb.add(types.InlineKeyboardButton("👑 ELITE (VIP) qo'lda, muddatli berish", callback_data="admin|elite_give_info"))
    bot.send_message(
        call.message.chat.id,
...
# ================================================================================
#  👑 HUNTER ELITE — VIP a'zolik (tarifli), shaxsiy laqab, til sozlamalari,
...
ELITE_PLANS = [
    ...
    {"days": 7,  "diamond": 45,  "stars": 15},
    {"days": 15, "diamond": 85,  "stars": 28},
    {"days": 30, "diamond": 150, "stars": 50},
]
...
def elite_status_line(uid):
    if is_elite(uid):
        ...
        return f"👑 <b>HUNTER ELITE faol</b> — {elite_days_left(uid)} kun qoldi{nick_part}"
    return "👑 HUNTER ELITE: faol emas — /elite orqali oling"


def elite_success_text(days, until):
    return (
        "🎉👑 <b>TABRIKLAYMIZ! HUNTER ELITE FAOLLASHTIRILDI!</b> 👑🎉\n\n"
        f"⏳ Qo'shilgan muddat: <b>{days} kun</b>\n"
        f"📅 Amal qilish muddati: <b>{time.strftime('%d.%m.%Y', time.localtime(until))}</b> gacha\n\n"
        "✨ Endi sizga ochiq:\n"
        "• 👑 Ismingiz yonida oltin ELITE nishoni\n"
...
def open_elite_menu(chat_id, uid):
    ...
    bot.send_message(
        ...
        "💳 Tarifni tanlang (Olmos yoki Telegram Stars orqali to'lang):",
        reply_markup=kb,
    )
...
def cb_elite_buy(call):
    maybe_capture_owner(call.from_user)
    uid = call.from_user.id
    _, days_s, currency = call.data.split("|")
    days = int(days_s)
    plan = next((p for p in ELITE_PLANS if p["days"] == days), None)
    if not plan:
        bot.answer_callback_query(call.id, "❌ Bu tarif topilmadi.", show_alert=True)
        return
    ...
    if currency == "diamond":
        ...
        try:
            bot.send_invoice(
                call.message.chat.id,
                title=f"👑 HUNTER ELITE ({days} kun)",
                description=f"{days} kunlik HUNTER ELITE obunasi ({price} ⭐️ Stars)",
                invoice_payload=f"elitepay_{uid}_{days}",
                provider_token="",   # Telegram Stars uchun bo'sh qoldiriladi
                currency="XTR",
                prices=[types.LabeledPrice(label=f"HUNTER ELITE ({days} kun)", amount=price)],
            )
            bot.answer_callback_query(call.id)
        except Exception:
            bot.answer_callback_query(call.id, "❌ To'lov tizimida xatolik. Keyinroq urinib ko'ring.", show_alert=True)
    else:
        bot.answer_callback_query(call.id, "❌ Noma'lum to'lov turi.", show_alert=True)
...
def cb_admin_elite_give_info(call):
    ...
    bot.send_message(
        call.message.chat.id,
        "👑 <b>HUNTER ELITE (VIP) qo'lda, muddatli berish:</b>\n\n"
...
# ================================================================================
#  📜 BOTFATHER UCHUN 3 XIL BUYRUQLAR MENYUSI (scope bo'yicha avtomatik o'rnatiladi)
#
#   1) GURUH CHATLARI uchun — faqat guruhda kerak bo'ladigan o'yin buyruqlari.
#   2) SHAXSIY CHAT (oddiy foydalanuvchi) uchun — profil/do'kon/ijtimoiy buyruqlar.
#   3) FAQAT BOT EGASI (5588583777) uchun — YUQORIDAGI IKKALASI + barcha owner/admin
#      buyruqlari (boshqa hech kimga bu ro'yxat ko'rinmaydi — Telegram buni
#      BotCommandScopeChat orqali FAQAT shu bitta chat_id'ga bog'lab beradi).
#
#  Bot ishga tushganda set_bot_commands_for_all_scopes() avtomatik chaqiriladi —
#  qo'lda BotFather'da hech narsa sozlashning HOJATI YO'Q, lekin xohlasangiz
#  pastdagi GROUP_COMMANDS / PRIVATE_COMMANDS / OWNER_EXTRA_COMMANDS ro'yxatlarini
#  o'zingiz ham BotFather'ga nusxa ko'chirib qo'yishingiz mumkin.
# ================================================================================

GROUP_COMMANDS = [
    ("newgame", "🎮 Yangi o'yin ochish (admin)"),
    ("povtorgame", "🔁 Yopilgan qo'shilishni qayta ochish"),
    ("join", "➕ O'yinga qo'shilish"),
...
PRIVATE_COMMANDS = [
    ...
    ("vip_sandiq", "💎 VIP Sirli Sandiq (faqat ELITE)"),
    ("jamoa", "💬 Mafiya jamoasiga yashirin xabar"),
    ("adolat", "💬 Qonun jamoasiga yashirin xabar"),
]

# --- Faqat bot egasi (owner) uchun — YUQORIDAGILAR USTIGA qo'shiladigan buyruqlar ---
OWNER_EXTRA_COMMANDS = [
    ("useri", "🔍 Foydalanuvchi profilini qidirish"),
    ("guruhdan_chiq", "🚪 Botni guruhdan chiqarish"),
    ("elite_ber", "👑 ELITE (VIP) qo'lda berish"),
    ...
    ("guruh_unban_hammasi", "✅ Guruhda hammani unban qilish"),
    ("startgame", "▶️ O'yinni majburan boshlash"),
    ("tark_et", "🚪 Botni joriy guruhdan chiqarish"),
    ("zayafka_soni", "📥 A'zolik so'rovlari soni"),
    ("zayafka_qabul", "✅ A'zolik so'rovlarini qabul qilish"),
    ("zayafka_hammasi", "✅ Barcha so'rovlarni qabul qilish"),
    ("zayafka_rad", "❌ A'zolik so'rovlarini rad etish"),
    ("turnir_boshla", "🏆 Yangi turnir ochish (FAQAT OWNER)"),
    ("turnir_royxat", "📋 Turnir ishtirokchilari ro'yxati"),
    ("turnir_yopish", "🔒 Turnir ro'yxatini yopish (FAQAT OWNER)"),
    ("turnir_jang", "⚔️ Turnir jangini boshlash (FAQAT OWNER)"),
    ("turnir_tugat", "🛑 Turnirni bekor qilish (FAQAT OWNER)"),
    ...
    ("mode", "⚙️ Guruh sozlamalari paneli (/setting bilan bir xil)"),
    ("menyu_yangila", "🔄 Buyruqlar menyusini qayta o'rnatish"),
]


def set_bot_commands_for_all_scopes():
    """🤖 Bot ishga tushganda avtomatik chaqiriladi — Telegram'ga 3 xil scope
    bo'yicha 3 xil buyruqlar ro'yxatini yuboradi:
      • Barcha guruh chatlari — faqat GROUP_COMMANDS
      • Barcha shaxsiy chatlar — faqat PRIVATE_COMMANDS
      • Bot egasining shaxsiy chati (5588583777) — PRIVATE_COMMANDS + OWNER_EXTRA_COMMANDS
    ...
    try:
        group_cmds = [types.BotCommand(c, d) for c, d in GROUP_COMMANDS]
        bot.set_my_commands(commands=group_cmds, scope=types.BotCommandScopeAllGroupChats())
    except Exception as e:
        _logger.warning("Guruh buyruqlar menyusini o'rnatishda xatolik: %s", e)

    try:
        private_cmds = [types.BotCommand(c, d) for c, d in PRIVATE_COMMANDS]
        bot.set_my_commands(commands=private_cmds, scope=types.BotCommandScopeAllPrivateChats())
    except Exception as e:
        _logger.warning("Shaxsiy chat buyruqlar menyusini o'rnatishda xatolik: %s", e)

    try:
        # owner uchun: PRIVATE_COMMANDS + OWNER_EXTRA_COMMANDS, dublikatsiz, 100 tadan oshmasin
        seen = set()
        owner_cmds = []
        for c, d in PRIVATE_COMMANDS + OWNER_EXTRA_COMMANDS:
            if c in seen:
                continue
            seen.add(c)
...
def cmd_menyu_yangila(message):
    """🔄 Bot egasi — 3 xil buyruqlar menyusini (guruh/shaxsiy/owner) istalgan
    vaqt qo'lda qayta o'rnatadi (masalan GROUP_COMMANDS/PRIVATE_COMMANDS/
    OWNER_EXTRA_COMMANDS ro'yxatlariga o'zgartirish kiritilgandan keyin)."""
    maybe_capture_owner(message.from_user)
    safe_delete(message)
    if not is_owner(message.from_user.id):
        return
    set_bot_commands_for_all_scopes()
    bot.send_message(message.chat.id, "✅ Barcha 3 xil buyruqlar menyusi (guruh / shaxsiy / owner) qayta o'rnatildi.")
