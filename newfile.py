...
def mention(uid, name):
    ...
    ❗️ MUHIM: bu yerda oddiy unicode emoji EMAS, balki premium_emoji() orqali
    haqiqiy <tg-emoji emoji-id="..."> tegi qo'yiladi — shu sababli guruhda ham,
    shaxsiy chatda ham bir xil (haqiqiy maxsus) ko'rinishda chiqadi."""
    ...
    try:
        ...
        if elite_until and elite_until > time.time():
            ...
            safe_name = f"{premium_emoji('fox')} {safe_name} {premium_emoji('fox_2')}"
        ramka_until = ch.get("ramka_until", 0)
        if ramka_until and ramka_until > time.time():
            safe_name = f"{premium_emoji('frame')}✨ {safe_name} ✨{premium_emoji('frame')}"
    except Exception:
        pass
