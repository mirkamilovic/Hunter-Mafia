...
def mention(uid, name):
    ...
    try:
        ...
        if elite_until and elite_until > time.time():
            ...
            lion_id = CLAN_PORTRAIT_EMOJI.get("logo")
            lion_tag = f'<tg-emoji emoji-id="{lion_id}">🦁</tg-emoji>' if lion_id else "🦁"
            # 🦁 Sher emojisi + endi "𝐄𝐋𝐈𝐓𝐄 💎" (qalin shrift + olmos) — avvalgi
            # kursiv "𝓔𝓵𝓲𝓽𝓮" so'zi va 👑 toj o'rniga.
            prefix += lion_tag + " <b>𝐄𝐋𝐈𝐓𝐄</b> 💎 "
...
# ================================================================================
EMOJI_TO_ID = {
    "🏛️": "5253656287694070722",   # pillar
    "🧰": "5253547530532201410",   # toolbox
    "💍": "5253587611167008718",   # ring  (bor yana: ring_2, wedding_ring_set)
    "🛡️": "5253731101729400237",  # shield (bor yana: shield_2)
    "📜": "5253887094941589781",   # scroll (bor yana: document)
    "🩹": "5253997381111817316",   # bandage
    "🔥": "5253570680405928167",   # flame
    "🎯": "5251516225749558785",   # target (bor yana: Snayper roli)
    "🚩": "5251734152390159229",   # flag_victory (bor yana: clan_banner, silk_flag)
    "💎": "5251496189727124050",   # gems_cluster (bor yana: diamond, elite_badge)
    "🔸": "5253846468845937581",   # bullet
    "📡": "5253972865438492175",   # radar
    "💔": "5253952975444947021",   # crystal_heart
    "🎭": "5253580120744042388",   # mask (bor yana: mask_2)
    "👤": "5251316295021932853",   # shadow_figure (bor yana: person)
    "🗡️": "5254004858649880562",  # dagger
    "👁": "5253662747324882982",   # eye
    "🦊": "5251620945642169057",   # fox / fox_2 (mention()da endi sher ishlatiladi)
    "🎽": "5253611761768111359",   # vest
    "🥽": "5251279323943451259",   # goggles
    "💌": "5253623117661646222",   # love_letter
    "⚗️": "5253655145232769885",  # potion
    "📍": "5253823894497826517",   # gps_beacon
    "🔮": "5253592932631487494",   # amulet (bor yana: crystal_ball)
    "🏦": "5253689492086234676",   # bank_rune (bor yana: vault)
    "⚔️": "5251574663074587729",  # sword (bor yana: duel)
    "💰": "5251467834353034219",   # money
    "🪙": "5253574498631851107",   # coin (bor yana: spinning_coin)
    "📦": "5251575586492561187",   # treasure_chest (bor yana: box)
    "🎰": "5251414052772554556",   # jackpot
    "💳": "5251308731584522091",   # vip_card
    "🎁": "5253706195214050358",   # gift
    "⭐️": "5251515856382371891",  # stars (Telegram Stars — ELITE belgisi EMAS)
    "🛡": "5251674405100106367",   # immunity_charm (bor yana: admin_shield)
    "💼": "5253568275224240880",   # briefcase
    "🎲": "5253887975409886895",   # dice
    "📶": "5251426662796534647",   # signal
    "🏺": "5253955303317227024",   # vase (bor yana: vase_2)
    "👩‍👧": "5251648746965477706", # mother_daughter
    "🏪": "5253530260468706909",   # shop
    "📈": "5251525597368198419",   # chart
    "⚙️": "5251609396475112519",  # gear
    "🔍": "5253956248210022896",   # magnifier
    "🔔": "5251671888249267242",   # bell
    "🔑": "5253534177478877517",   # key_item
    "🔒": "5253517976862237810",   # padlock
    "🗺": "5251462792061429779",   # map
    "🔫": "5253469057184737578",   # gun (bor yana: gun_2)
    "🪖": "5253630311731863374",   # helmet
    "🧭": "5251624768163066301",   # compass
    "🎒": "5251355615447526296",   # inventory_chest
    "❓": "5254003046173682568",   # question
    "⚠️": "5251375114599050954",  # warning
    "🌙": "5253688538603496820",   # moon
    "☀️": "5251569968675332763",  # sun
    "🫡": "5251604517392262503",   # salute
    "📖": "5251562774605112341",   # book
    "😡": "5253569756987960765",   # angry
    "🚀": "5253811460567506961",   # rocket
    "🗳️": "5253749166361845208",  # ballot
    "💥": "5253687546466052125",   # explosion
    "🔓": "5253984844102279619",   # broken_lock
    "⚰️": "5251294738581070469",  # gravestone
    "⏳": "5253668944962690414",   # hourglass
    "⚡️": "5253896878877091357",  # lightning
    "👑": "5251609456604652565",   # crown
    "🖼️": "5253452517265680802",  # frame
    "🐉": "5251539156579953188",   # dragon
    "🏆": "5251284426364597200",   # trophy
    "🎖️": "5253583831595787507",  # medal
...
def cmd_start(message):
    ...
    if payload.startswith("gamble_"):
        try:
            _, game_key, bet_s = payload.split("_", 2)
            ok, msg = _perform_gamble(message.from_user.id, message.from_user.first_name, game_key, int(bet_s))
        except Exception:
            ok, msg = False, "❌ O'yin havolasi noto'g'ri."
        bot.send_message(message.chat.id, msg, reply_markup=with_back_button())
        return
...
# ================================================================================
#  🎰 QIMOR O'YINLARI (Mini App "O'yin" bo'limi uchun) — faqat Dollarda,
#     yutish ehtimoli 30%, minimal tikish $100, yutganda 2x qaytadi.
# ================================================================================

GAMBLE_GAMES = {
    "zar": {"name": "🎲 Baxtli Zar", "emoji": "🎲", "desc": "Zarni oting — 30% ehtimol bilan yutasiz."},
    "karta": {"name": "🃏 Qora Karta", "emoji": "🃏", "desc": "Kartani oching — 30% ehtimol bilan yutasiz."},
    "gildirak": {"name": "🎡 Omad G'ildiragi", "emoji": "🎡", "desc": "G'ildirakni aylantiring — 30% ehtimol bilan yutasiz."},
}
GAMBLE_MIN_BET = 100
GAMBLE_WIN_CHANCE = 0.30
GAMBLE_PAYOUT_MULT = 2  # 🏆 Yutganda tikkan pulning 2 barobari qaytadi (sof foyda = tikkan pul)


def _perform_gamble(uid, first_name, game_key, bet):
    if game_key not in GAMBLE_GAMES:
        return False, "❌ Bunday o'yin topilmadi."
    if bet < GAMBLE_MIN_BET:
        return False, f"❌ Minimal tikish — ${GAMBLE_MIN_BET}."
    u = user_dict(uid, first_name)
    if u["dollar"] < bet:
        return False, "❌ Dollaringiz yetarli emas!"
    add_balance(uid, dollar=-bet)
    won = random.random() < GAMBLE_WIN_CHANCE
    game = GAMBLE_GAMES[game_key]
    if won:
        payout = bet * GAMBLE_PAYOUT_MULT
        add_balance(uid, dollar=payout)
        return True, (f"{game['emoji']} <b>YUTDINGIZ!</b>\n💵 Tikkan: ${bet} ➜ Yutuq: <b>${payout}</b> "
                       f"(sof foyda: +${payout - bet})")
    return True, f"{game['emoji']} <b>Yutqazdingiz.</b>\n💵 -${bet}\nOmad boshqa safar kulib boqar!"


def _gamble_command_factory(game_key):
    def _handler(message):
        ...
        if len(parts) != 2 or not parts[1].isdigit():
            ...
            bot.send_message(
                ...
                f"Foydalanish: <code>/{game_key} &lt;tikish_summasi&gt;</code>\n"
                f"Masalan: <code>/{game_key} 100</code>\n\n"
                f"💵 Minimal tikish: ${GAMBLE_MIN_BET}  •  🎯 Yutish ehtimoli: {int(GAMBLE_WIN_CHANCE*100)}%  •  🏆 Yutuq: 2x",
            )
            return
        ok, msg = _perform_gamble(message.from_user.id, message.from_user.first_name, game_key, int(parts[1]))
        bot.send_message(message.chat.id, msg)
    return _handler


for _gk in GAMBLE_GAMES:
    bot.message_handler(commands=[_gk])(_gamble_command_factory(_gk))
