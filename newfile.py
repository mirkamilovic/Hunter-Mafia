...
HEROES = {
    "sardor": {"name": "⚔️ Draven the Warlord", "kind": "survive", "image": "hero_draven.jpg",
               "desc": "Har kecha hujumdan tasodifiy foizda avtomatik omon qoladi."},
    "malika": {"name": "🔮 Seraphine the Oracle", "kind": "compass", "image": "hero_seraphine.jpg",
               "desc": "Har kuni bepul, cheklovsiz /kompas ishlatish imkoniyati beradi (do'kondan sotib olmasdan)."},
    "temur":  {"name": "🛡️ Magnus the Conqueror", "kind": "duel", "image": "hero_magnus.jpg",
               "desc": "Duellarda g'alaba qozonish ehtimolini oshiradi."},
    "layli":  {"name": "🌙 Elandriel the Enchantress", "kind": "bonus", "image": "hero_elandriel.jpg",
               "desc": "Kunlik bonus miqdorini oshiradi."},
    "burgut": {"name": "🦅 Zephyrion, the Ascendant One", "kind": "all", "image": "hero_zephyrion.jpg",
               "desc": "ENG NODIR geroy (0.05%) — barcha qobiliyatlarning kuchsizroq versiyasini birgalikda beradi."},
}
...
def get_hero(uid):
    with db_lock:
        cur.execute("SELECT hero_key, level FROM heroes WHERE user_id=?", (uid,))
        row = cur.fetchone()
    if not row:
        return None
    return {"key": row[0], "level": row[1], **HEROES.get(row[0], {})}
...
def _hero_power(level, base, per_level, cap):
    """1-lvlda `base`, 15-lvlda taxminan `cap`gacha chiziqli o'sadigan, chegaralangan kuch qiymati."""
    val = base + per_level * (level - 1)
    return min(val, cap)
...
def hero_survive_chance(uid):
    ...
    if not hero:
        return 0.0
    if hero["kind"] == "survive":
        return _hero_power(hero["level"], 0.05, 0.01, 0.20)  # 5% → 20%
    if hero["kind"] == "all":
        return _hero_power(hero["level"], 0.08, 0.012, 0.25)  # 8% → 25% (Burgut, biroz kuchliroq)
    return 0.0
...
def hero_duel_bonus(uid):
    ...
    if hero["kind"] == "duel":
        return _hero_power(hero["level"], 0.05, 0.01, 0.20)  # +5% → +20%
    if hero["kind"] == "all":
        return _hero_power(hero["level"], 0.08, 0.012, 0.25)
    return 0.0
...
def hero_bonus_extra_dollar(uid):
    ...
    if not hero:
        return 0
    if hero["kind"] == "bonus":
        return int(_hero_power(hero["level"], 20, 8, 150))  # +20$ → +150$
    if hero["kind"] == "all":
        return int(_hero_power(hero["level"], 15, 6, 110))
    return 0


def hero_has_free_compass(uid):
    hero = get_hero(uid)
    return bool(hero and hero["kind"] in ("compass", "all"))
...
def apply_kill(game, uid, killed_set, bypass_protection=False):
    ...
    if not bypass_protection:
        ...
        if p["role"] == "Varvar 🪓" and random.random() < 0.3:
            return
        # 🏆 Geroy (Draven / Zephyrion) — lvl asosida hujumdan tasodifiy omon qolish
        ...
        if h_chance > 0 and random.random() < h_chance:
            ...
            safe_send(uid, "🏆 Bu kecha sizga hujum qilishdi, lekin geroyingiz sizni qutqardi!")
            return
...
def end_game(chat_id, winners_team):
    ...
    for uid, p in game["players"].items():
        ...
        if not left:
            add_points(uid, 5 if won else -5, "game_result")
        ...
        # 📬 Har bir o'yinchiga shaxsiy yakuniy xabar
        if left:
            outcome_line = "🚪 Siz o'yinni tark etgansiz — shu sabab bu o'yin uchun <b>mag'lub</b> hisoblanasiz va mukofot berilmaydi."
        elif won:
            outcome_line = f"🎉 <b>G'alaba qozondingiz!</b> +{reward}$ mukofot oldingiz."
        else:
            outcome_line = f"😔 <b>Mag'lub bo'ldingiz.</b> +{reward}$ (ishtirok uchun) oldingiz."
...
def resolve_duel_outcome(a_id, b_id):
    ...
    if a_hero_bonus or b_hero_bonus:
        a_win_chance = max(0.15, min(0.85, 0.5 + a_hero_bonus - b_hero_bonus))
        return (a_id, b_id) if random.random() < a_win_chance else (b_id, a_id)
...
def cmd_turnir_boshla(message):
    """🏆 Guruh admini yoki bot egasi yangi turnir ochadi — ro'yxatga olish boshlanadi."""
    maybe_capture_owner(message.from_user)
    safe_delete(message)
    if message.chat.type not in ("group", "supergroup"):
        bot.send_message(message.chat.id, "🏆 Turnirlar faqat guruhda tashkil qilinadi.")
        return
    if not is_group_admin_or_owner(message):
        bot.send_message(message.chat.id, "⛔ Faqat guruh admini yoki bot egasi turnir boshlashi mumkin.")
        return
    chat_id = message.chat.id
...
def cmd_turnir_yopish(message):
    ...
    if not t or t["status"] != "registration":
        ...
        return
    if not (is_owner(message.from_user.id) or message.from_user.id == t["host_id"] or is_group_admin_or_owner(message)):
        bot.send_message(chat_id, "⛔ Faqat turnir tashkilotchisi yoki admin yopa oladi.")
        return
...
def cmd_turnir_jang(message):
    ...
    if not t or t["status"] != "ongoing":
        ...
        return
    if not (is_owner(message.from_user.id) or message.from_user.id == t["host_id"] or is_group_admin_or_owner(message)):
        bot.send_message(chat_id, "⛔ Faqat turnir tashkilotchisi yoki admin jang boshlashi mumkin.")
        return
    remaining = {uid: name for uid, name in t["participants"].items() if uid not in t["eliminated"]}
    if len(remaining) < 2:
...
def cmd_turnir_tugat(message):
    ...
    if not t:
        ...
        return
    if not (is_owner(message.from_user.id) or message.from_user.id == t["host_id"] or is_group_admin_or_owner(message)):
        bot.send_message(chat_id, "⛔ Faqat turnir tashkilotchisi yoki admin bekor qila oladi.")
        return
...
def cmd_geroy_ber(message):
    ...
    with db_lock:
        cur.execute("INSERT OR REPLACE INTO heroes (user_id, hero_key, level) VALUES (?, ?, 1)", (target.id, hero_key))
        conn.commit()
...
def cb_invitem_detail(call):
    ...
    if charge_key in _INV_TARGET_ACTIONS and left > 0:
        ...
        text += "\n\nℹ️ Bu buyum tegishli buyruq orqali ham ishlatiladi (masalan reply orqali)."
    ...
    if action == "roles":
        ...
        if t and t["status"] != "finished":
            ...
            bot.send_message(
                ...
                "Guruh admini yoki bot egasi <code>/turnir_boshla &lt;nomi&gt;</code> orqali yangi turnir ochadi.\n"
        ...
        text = (
            ...
            "<b>🏆 Musobaqalar (turnirlar):</b>\n"
            "/turnir_boshla &lt;nomi&gt; — yangi turnir ochish (admin)\n"
            "/turnir_royxat — ishtirokchilar ro'yxati\n"
            "/turnir_yopish — ro'yxatni yopib boshlash (admin)\n"
            "/turnir_jang — navbatdagi ⚔️ Turnir Jangini tanlab boshlash (admin)\n"
            "/turnir_tugat — turnirni bekor qilish (admin)\n\n"
...
def cb_buy(call):
    ...
    if mode == "shield":
        ...
        if existing:
            ...
            hero_key = roll_hero()
            with db_lock:
                cur.execute("INSERT OR REPLACE INTO heroes (user_id, hero_key, level) VALUES (?, ?, 1)", (uid, hero_key))
                conn.commit()
            hero = HEROES[hero_key]
