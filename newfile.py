...
AFK_KICK_JOKES = [
    "😴 Kimdir shu qadar chuqur uxlab qolibdiki, o'yin uni o'zi bilan olib keta olmadi...",
    "🛌 Bir soya ikki tundan beri sukut saqlagani uchun shaharni tark etdi.",
    "📴 Kimningdir aloqasi 2 tundan beri yo'q — ehtimol u pitsa buyurtma qilgandir. U endi o'yinda emas.",
    "🚪 Sukunat ikki tunga cho'zilsa, eshik ham ochiladi... Kimdir jimgina chiqib ketdi.",
    "💤 Shahar aholisidan biri shunchalik og'ir uxlab qoldiki, hatto Telegram ham uni unutdi. U o'yindan chetlatildi.",
]
...
SHOP_DOLLAR = {
    ...
    "sumka":    {"name": "💼 Shubhali sumka", "price": 450, "currency": "dollar", "mode": "instant_dollar", "range": (40, 150),
                 "desc": "Ichidan tasodifiy 40$-150$ chiqadi."},
    ...
    "energy":   {"name": "⚡️ Energiya ichimligi", "price": 120, "currency": "dollar", "mode": "instant_dollar", "range": (90, 90),
                 "desc": "O'yin davomida tezkor harakat va +90$ mukofot beradi."},
    ...
    "zar":      {"name": "🎲 Sehrli Zarlar", "price": 320, "currency": "dollar", "mode": "instant_dollar", "range": (50, 280),
                 "desc": "Ochilganda tasodifiy $50 dan $280 gacha yutuq keltiradi."},
...
SHOP_DIAMOND = {
    ...
    "tumor":     {"name": "🔮 Sehrli tumor", "price": 10, "currency": "diamond", "mode": "multiplier",
                  "charge_key": "bonus_mult", "mult_value": 2, "desc": "Kunlik bonusingizni 2 barobar qiladi."},
    "quti":      {"name": "📦 Nodir quti", "price": 15, "currency": "diamond", "mode": "instant_dollar", "range": (80, 220),
    ...
    "vip_olmos": {"name": "🌟 Olmos VIP obuna", "price": 60, "currency": "diamond", "mode": "expiry",
                  "charge_key": "vip_diamond_until", "duration_seconds": 12 * 86400,
                  "desc": "12 kun davomida VIP maqomi (profilda ko'rinadi) — sotib olingan kundan teskari sanoq boshlanadi."},
    # --- YANGI MAHSULOTLAR (qoshimchakod8.py g'oyasi asosida qo'shildi) ---
    ...
    "eliksir":   {"name": "🧪 Hayot Eliksiri", "price": 35, "currency": "diamond", "mode": "instant_dollar", "range": (600, 900),
                  "desc": "Barcha jarohatlarni bitirib, $600-900 oralig'ida bonus beradi."},
}
...
SHOP_COIN = {
    ...
    "vip_coin":    {"name": "🌟 Coin VIP obuna", "price": 180, "currency": "coin", "mode": "expiry",
                    "charge_key": "vip_coin_until", "duration_seconds": 30 * 86400,
                    "desc": "30 kunga (bu do'kondagi eng uzun muddat) VIP maqomi — sotib olingan kundan teskari sanoq boshlanadi."},
}

SHOP_OSH = {
    "toy":       {"name": "💍 To'y Oshi", "price": 5000, "currency": "dollar", "gift_shield": 2, "gift_dollar": 150},
    "choyxona":  {"name": "🍵 Choyxona Oshi", "price": 4500, "currency": "dollar", "gift_shield": 2, "gift_dollar": 120},
    "tandir":    {"name": "🔥 Tandir Oshi", "price": 4000, "currency": "dollar", "gift_shield": 1, "gift_dollar": 120},
    "samarqand": {"name": "🇺🇿 Samarqand Oshi", "price": 3800, "currency": "dollar", "gift_shield": 1, "gift_dollar": 100},
    "buxoro":    {"name": "🏛 Buxorocha Sofi Oshi", "price": 3600, "currency": "dollar", "gift_shield": 1, "gift_dollar": 80},
    "fargona":   {"name": "🏔 Farg'ona Devzira Oshi", "price": 3500, "currency": "dollar", "gift_shield": 1, "gift_dollar": 60},
    "qora":      {"name": "🥷 Qora Mafia Oshi", "price": 3000, "currency": "dollar", "gift_shield": 1, "gift_dollar": 40},
}
...
def cmd_startgame(message):
    ...
    for uid, p in game["players"].items():
        ...
        safe_send(uid, f"🕵️ Maxfiy missiya: <i>{mission}</i>")
...
def start_night(chat_id):
    ...
    for uid in kicked_afk:
        bot.send_message(chat_id, random.choice(AFK_KICK_JOKES))
    ...
    with GAME_LOCK:
        ...
        for slug in ("qotil", "manyak", "mergan"):
            ...
            if act and act["target"] != "skip":
                ...
                apply_kill(game, act["target"], killed)
        ...
        if snayper_act and snayper_act["target"] != "skip":
            ...
            apply_kill(game, snayper_act["target"], killed)
        ...
        if telba_act and telba_act["target"] != "skip":
            ...
            if random.random() < 0.5:
                ...
                apply_kill(game, telba_act["target"], killed)
...
def cmd_kompas(message):
    ...
    if target.id not in game["players"] or not game["players"][target.id]["alive"]:
        return
    if get_charges(uid).get("compass", 0) <= 0:
        safe_send(uid, "❌ Sizda 🧭 Sirli kompas yo'q.")
        return
    use_charge(uid, "compass")
    target_role = game["players"][target.id]["role"]
    safe_send(uid, f"🧭 Kompas natijasi: <b>{target.first_name}</b> — taraf: <b>{team_of(target_role)}</b>")
...
def cmd_osh(message):
    """🥘 Milliy osh — endi o'zingizga emas, GURUHDAGI boshqa bir o'yinchiga
    ziyofat berish uchun: unga bir martalik himoya (shield) va kichik dollar
    sovg'asi qiladi. Foydalanish: kimningdir xabariga reply qilib /osh <turi>."""
    maybe_capture_owner(message.from_user)
    safe_delete(message)
    if not message.reply_to_message:
        lines = ["🥘 <b>Milliy Osh Menyusi</b> — do'stingizga ziyofat bering!\n"]
        for key, item in SHOP_OSH.items():
            lines.append(f"<code>{key}</code> — {item['name']} (${item['price']}) → {item['gift_shield']}🛡 + ${item['gift_dollar']} sovg'a")
    ...
    add_balance(target.id, dollar=item["gift_dollar"])
    bot.send_message(
        message.chat.id,
        f"{item['name']}! {mention(sender.id, sender.first_name)} — {mention(target.id, target.first_name)}ga ziyofat berdi!\n"
        f"🎁 {target.first_name} +{item['gift_shield']}🛡 va +${item['gift_dollar']} sovg'a oldi.",
    )
...
def open_day_voting(chat_id):
    ...
    game["phase_deadline"] = time.time() + DAY_VOTE_SECONDS
    save_games_state()
...
def cmd_duel(message):
    ...
    bot.send_message(
        message.chat.id,
        f"⚔️ {mention(sender.id, sender.first_name)}, {mention(target.id, target.first_name)}ni duelga chorlayapti! Qabul qilasizmi?",
        reply_markup=kb,
    )
...
def format_active_effects(uid):
    ...
    vip_diamond_until = ch.get("vip_diamond_until", 0)
    if vip_diamond_until and vip_diamond_until > time.time():
        rem_days = int((vip_diamond_until - time.time()) // 86400) + 1
        add(True, f"🌟 Olmos VIP obuna: {rem_days} kun qoldi")

    vip_coin_until = ch.get("vip_coin_until", 0)
    if vip_coin_until and vip_coin_until > time.time():
        rem_days = int((vip_coin_until - time.time()) // 86400) + 1
        add(True, f"🌟 Coin VIP obuna: {rem_days} kun qoldi")
...
def show_inventory(chat_id, uid):
    ...
    bot.send_message(chat_id, "📦 <b>SIZNING INVENTARINGIZ</b>\n\nBuyum haqida to'liq ma'lumot va uni yoqish/o'chirish uchun tugmani bosing:", reply_markup=kb)
...
def cb_invitem_detail(call):
    ...
    o'chirish tugmasi yoki qanday ishlatish kerakligi ko'rsatiladi."""
    maybe_capture_owner(call.from_user)
    ...
    kb = None
    if charge_key and charge_key in TOGGLABLE_ITEMS and get_charges(uid).get(charge_key, 0) > 0:
    ...
    elif item.get("mode") in ("charge",) and charge_key:
        left = get_charges(uid).get(charge_key, 0)
        text += f"\n\n📦 Qolgan miqdor: {left} ta. Ishlatish uchun tegishli buyruqni qo'llang (masalan reply orqali)."
