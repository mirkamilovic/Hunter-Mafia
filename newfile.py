...
def hero_game_start_reminder_text(uid):
    ...
    lines = [
        ...
        f"📊 Daraja: <code>{text_progress_bar(level, HERO_MAX_LEVEL)}</code>",
    ]
    ...
    if any(a["key"] == "guard_once" and a["unlocked"] for a in unlocked_abilities):
        lines.append("  🕊 Himoyachi tumor — pastdagi tugma orqali, FAQAT TUNDA (nishonni tanlaysiz, 1 marta)")
        if in_game and game_ig["phase"] == "night" and uid not in game_ig.setdefault("hero_guard_used", set()):
            kb.add(types.InlineKeyboardButton("🕊 Himoya qilish", callback_data=f"herotarget|guard|{uid}"))
    if any(a["key"] == "gift_once" and a["unlocked"] for a in unlocked_abilities):
        lines.append("  🎁 Sehrli sovg'a — pastdagi tugma orqali (nishonni tanlaysiz, 1 marta)")
        if in_game and uid not in game_ig.setdefault("hero_gift_used", set()):
            kb.add(types.InlineKeyboardButton("🎁 Sovg'a qilish", callback_data=f"herotarget|gift|{uid}"))
    if any(a["key"] == "death_reveal_once" and a["unlocked"] for a in unlocked_abilities):
        lines.append("  🩸 Qasos nafasi — AVTOMATIK, faqat halok bo'lgan payt (1 marta)")
    if any(a["key"] == "duel_reward_bonus" and a["unlocked"] for a in unlocked_abilities):
        lines.append("  💰 G'olib nafi — /duel g'alabasida AVTOMATIK qo'shiladi")
