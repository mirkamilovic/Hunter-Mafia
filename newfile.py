    ...
    try:
        ...
        if not question:
            bot.reply_to(
                ...
                "Men botning sun'iy intellekt yordamchisiman — savol bering, maslahat so'rang!\n\n"
                "✍️ Foydalanish: <code>/theron savolingiz</code>\n"
                f"👑 ELITE a'zolar — kuniga {THERON_ELITE_DAILY_LIMIT} tagacha chuqur AI-savol\n"
                f"👤 Oddiy foydalanuvchilar — kuniga {THERON_FREE_DAILY_LIMIT} ta chuqur AI-savol\n"
                "📚 Eng ko'p so'raladigan mavzularga (ELITE, klan, duel, nikoh va h.k.) — LIMITSIZ, DARHOL javob beraman!",
            )
            return
        ...
        if not theron_available():
            bot.reply_to(
                message,
                f"🧠 <b>{THERON_NAME}</b> {THERON_MEANING}\n\n"
                "Bu savolga tayyor javobim yo'q ekan, va chuqurroq AI xizmati hozircha botda "
                "sozlanmagan (bot egasi GEMINI_API_KEY ni o'rnatishi kerak). Boshqa buyruqlar "
                "odatdagidek to'liq ishlayveradi — /yordam orqali ko'ring."
            )
            return
...
def cmd_game(message):
    ...
    with GAME_LOCK:
        ...
        if existing and existing.get("phase") == "waiting":
            ...
            sent = bot.send_message(
                chat_id,
                "🎮 <b>O'yin lobbisi ochiq</b> (hali boshlanmagan)\n\n"
                "Qo'shilish uchun pastdagi tugmani bosing yoki <code>/join</code> deb yozing!\n"
                "Boshlash uchun: <code>/startgame</code>\n\n"
...
def cmd_startgame(message):
    ...
    if game.get("_hero_holders_count"):
        ...
        bot.send_message(
            chat_id,
            "🏆 <i>Diqqat: bu o'yinda geroy(lar) ham ishtirok etmoqda — ular maxsus mahoratlarga ega. "
            "Kimdir mahorat ishlatsa, bu yerda \"🏆 Kimningdir ... mahoratini ishga tushirdi\" deb E'LON QILINADI "
            "(lekin kim ekani sir saqlanadi).</i>",
        )
...
def assign_roles(game):
    ...
    if n <= CORE_ROLE_THRESHOLD:
        ...
        # (aks holda o'yinda mafiya nisbati beqarorlashib ketardi).
        ...
        extra_source = [r for r in ALL_ROLES if r not in CORE_ROLES and r not in MAFIA_ROLES]
        ...
        if partner_id:
            ...
            if couple_key not in game.setdefault("revealed_couples", set()):
                ...
                bot.send_message(
                    ...
                    f"{mention(partner_id, pp['name']) if pp else 'sherigi'} birgalikda "
                    f"maxfiy juftlik ekan — {p['role']} va {pp['role'] if pp else '?'}.",
                )
            if pp and pp.get("alive"):
                ...
                bot.send_message(
                    ...
                    f"{mention(partner_id, pp['name'])} ham 'bog'langan taqdir' qoidasiga ko'ra DARHOL o'yindan chiqdi!",
                )
...
def cmd_coinsovga(message):
    ...
    if message.reply_to_message and len(parts) == 2:
        ...
        bot.send_message(
            message.chat.id,
            "🪙 <b>Hunter Coin sovg'a qilish (o'tkazish)</b>\n\n"
            "Foydalanish:\n"
            "• Kimningdir xabariga REPLY qilib: <code>/coinsovga &lt;miqdor&gt;</code>\n"
            "• Yoki to'g'ridan-to'g'ri: <code>/coinsovga &lt;Telegram_ID&gt; &lt;miqdor&gt;</code>\n\n"
            "❗️ Qabul qiluvchi avval botga kamida bir marta <code>/start</code> bosgan bo'lishi kerak.",
        )
        return
...
def cmd_hc_kurs(message):
    ...
    bot.send_message(
        ...
        f"⏱ Kurs har {HC_MARKET_UPDATE_MINUTES} daqiqada tabiiy ravishda biroz o'zgarib turadi — "
        "xuddi haqiqiy valyuta kabi.\n"
        "💱 Ayirboshlash: /olmos_coin (💎↔🪙)  •  /dollar_coin (💵↔🪙)  •  /coinsovga (🪙 sovg'a)",
    )
