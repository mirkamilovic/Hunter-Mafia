    ...
    try:
        with TEAM_GAMES_LOCK:
            ...
            if winner_team is None:
                ...
                names = ", ".join(mention(u, n) for u, n in tg["alive"][winner_team].items()) or "—"
                bot.send_message(
                    chat_id,
                    f"🏁 <b>Jamoalar urushi tugadi!</b>\n\n"
                    f"🏆 <b>G'olib jamoa: {_team_label(winner_team)}!</b>\n"
                    f"🎉 Omon qolganlar: {names}",
                )
...
def _para_end_game(chat_id, winner_side):
    try:
        with PARA_GAMES_LOCK:
            ...
            names = ", ".join(_para_couple_mention(c) for c in winners) if winners else "—"
            bot.send_message(
                chat_id,
