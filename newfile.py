...
def theron_available():
    """Theron ishga tayyor-tayyormasligini tekshiradi (API kaliti o'rnatilganmi)."""
...
def theron_owner_report_text():
    ...
    prompt = (
        "Sen bot egasining shaxsiy ishchi yordamchisisan. Egangga har kuni shu statistikani hisobot "
        "qilib berasan. Quyidagi FAKT raqamlarni tabiiy gapga aylantirib bering, so'ng 1 ta AMALIY "
        "maslahat qo'shing (masalan promo-kod chiqarish, foydalanuvchilarni qaytarish uchun e'lon, "
        "yangi turnir e'lon qilish va h.k.). Xushmuomalalik bilan, xodim o'z rahbariga hisobot "
        f"berayotgandek boshlang (masalan \"Assalomu alaykum, xo'jayin!\" kabi iliq murojaat bilan).\n\n"
        f"STATISTIKA:\n{facts}"
    )
    result = _theron_raw_call(THERON_BASE_PERSONA, prompt, max_tokens=350, temperature=0.8)
    if result:
        return f"🧠 <b>{THERON_NAME}dan hisobot:</b>\n\n{result}"
    return (
        f"🧠 <b>{THERON_NAME}dan hisobot</b> (AI hozircha mavjud emas, oddiy raqamlar):\n\n{facts}"
    )


def theron_owner_greeting():
    """Owner /start bosganda — Theron o'zini ishchi sifatida tanishtirib,
    hol-ahvol so'raydi va yordam kerakligini so'raydi."""
    prompt = (
        "Botning egasi hozirgina shaxsiy chatga /start bosdi. Uni sodiq va mehnatkash "
        "yordamchi sifatida qisqa, iliq salomlashib kutib ol. O'zingni qisqacha tanishtir "
        "(ismingi va nima uchun shu ishda ekaningni bir og'iz gap bilan), so'ng nima bilan "
        "yordam berishing mumkinligini so'ra (masalan statistikani ko'rish, tavsiya berish). "
        "Juda qisqa bo'lsin — 3-4 gapdan oshmasin."
    )
    return _theron_raw_call(THERON_BASE_PERSONA, prompt, max_tokens=200, temperature=0.9)
...
def theron_answer_user(uid, question, is_elite_user):
    """Elite/oddiy foydalanuvchining /theron savoliga shaxsiylashtirilgan javob."""
    context = theron_build_user_context(uid)
    role_note = (
        "Bu foydalanuvchi ELITE a'zo — unga to'liq, batafsil va samimiy yordam ber, "
        "kerak bo'lsa botning istalgan funksiyasi (klan, duel, do'kon, turnir) bo'yicha chuqur maslahat ber."
        if is_elite_user else
        "Bu foydalanuvchi ELITE emas — javobing QISQA (2-3 gap) bo'lsin. Agar mavzuga mos "
        "bo'lsa, oxirida ELITE a'zolikning ko'proq yordam berishini bir og'iz eslatib o't "
        "(lekin har safar emas, majburlamagan holda)."
    )
    prompt = (
    ...
    history = _THERON_HISTORY.get(uid, [])
    result = _theron_raw_call(THERON_BASE_PERSONA, prompt, history=history, max_tokens=450, temperature=0.85)
    if result:
        hist = _THERON_HISTORY.setdefault(uid, [])
        hist.append({"role": "user", "text": question})
        hist.append({"role": "model", "text": result})
        del hist[:-_THERON_HISTORY_MAX]
    return result


def theron_night_narration(chat_id, killed_names, no_death):
    """Tungi voqealarni AI orqali qisqa, badiiy tarzda hikoya qiladi. Xatolik
    bo'lsa yoki AI mavjud bo'lmasa — jimgina hech narsa qilmaydi (asosiy
    statik e'lon allaqachon yuborilgan bo'ladi, o'yin hech qachon bunga bog'liq
    bo'lmaydi)."""
    if not theron_available():
        return
    try:
        if no_death:
            prompt = ("Bu tun Hunter Mafia o'yinida hech kim o'lmadi. Shu voqeani 1-2 gapda, "
                      "sirli va dramatik uslubda, qo'rqinchli qishloq tunini tasvirlab yoz.")
        else:
            names = ", ".join(killed_names)
            prompt = (f"Bu tun Hunter Mafia o'yinida quyidagi o'yinchilar halok bo'ldi: {names}. "
                      "Shu voqeani 1-2 gapda, sirli va dramatik (lekin haqoratsiz, hurmatli) uslubda "
                      "hikoyachidek tasvirlab yoz. Ism ishlatmasang ham bo'ladi, faqat kayfiyatni ber.")
        text = _theron_raw_call(
            "Sen Hunter Mafia o'yinining sirli hikoyachisisan (Theron degan nomga egasan). "
            "Faqat o'zbek tilida, juda qisqa (1-2 gap), dramatik va badiiy uslubda yozasan. "
            "Hech qachon o'yinchilarni haqorat qilmaysan yoki haqiqiy zo'ravonlikni ulug'lamaysan — "
            "bu FAQAT o'yin, badiiy uslub.",
            prompt, max_tokens=120, temperature=1.0,
        )
        if text:
...
def theron_winner_congrats(chat_id, winner_names, team_name):
    """O'yin oxirida g'oliblarni tabriklaydi — AI mavjud bo'lsa AI orqali
    (har safar boshqacha, yangi matn), AI mavjud bo'lmasa/ishlamasa ham
    HECH QACHON jim qolmaydi: tasodifiy tanlangan, ismlarni albatta o'z
    ichiga olgan zaxira shablon ishlatiladi. Shu orqali tabrik HAR DOIM
    keladi va HAR SAFAR bir xil bo'lib qolmaydi."""
    if not winner_names:
        return
    ...
    if theron_available():
        try:
            prompt = (
                f"Hunter Mafia o'yini tugadi. G'olib jamoa: {team_name}. G'oliblar: {names}. "
                "Ularni qisqa (2-3 gap), samimiy va ilhomlantiruvchi tarzda tabrikla — xuddi jonli "
                "sport sharhlovchisi kabi zavq bilan yoz. Ism(lar)ni albatta tilga ol."
            )
            text = _theron_raw_call(
                "Sen Hunter Mafia o'yinining tabriklovchi hikoyachisisan (Theron). Faqat o'zbek tilida, "
                "qisqa, samimiy va quvnoq uslubda yozasan.",
                prompt, max_tokens=150, temperature=1.0,
            )
        except Exception as e:
            _logger.warning("Theron g'alaba tabrigida AI xatolik (zaxira shablon ishlatiladi): %s", e)
...
def save_games_state():
    ...
    🔧 TUZATILDI (#2): endi asosiy o'yin (GAMES) bilan bir qatorda 🐺🦅 TEAM_GAMES
    va 💍 PARA_GAMES holatlari ham saqlanadi — bot qayta ishga tushsa, ular ENDI
    yo'qolib qolmaydi."""
    try:
        with GAME_LOCK, STATE_LOCK:
            snapshot = {
                ...
                "team_games": {
                    str(chat_id): {
                        "phase": tg["phase"],
                        "teams": {side: {str(u): n for u, n in members.items()} for side, members in tg["teams"].items()},
                        "alive": {side: {str(u): n for u, n in members.items()} for side, members in tg.get("alive", {"wolf": {}, "eagle": {}}).items()},
                        "round": tg.get("round", 0),
                        "night_votes": {side: {str(v): t for v, t in votes.items()} for side, votes in tg.get("night_votes", {"wolf": {}, "eagle": {}}).items()},
                        "msg_id": tg.get("msg_id"),
                    }
                    for chat_id, tg in TEAM_GAMES.items()
                },
                "para_games": {
                    str(chat_id): {
                        "phase": pg["phase"],
                        "couples": {
                            str(cid): {"members": c["members"], "names": c["names"], "alive": c["alive"], "hp": c["hp"], "role": c["role"]}
                            for cid, c in pg["couples"].items()
                        },
                        "member_to_couple": {str(u): cid for u, cid in pg["member_to_couple"].items()},
                        "next_couple_id": pg["next_couple_id"],
                        "msg_id": pg.get("msg_id"),
                        "mafia_votes": {str(v): t for v, t in pg.get("mafia_votes", {}).items()},
                        "protect_target": pg.get("protect_target"),
                        "day_votes": {str(v): t for v, t in pg.get("day_votes", {}).items()},
                        "day_number": pg.get("day_number", 0),
                    }
                    for chat_id, pg in PARA_GAMES.items()
                },
            }
...
def load_games_state():
    ...
    🔧 TUZATILDI (#2): 🐺🦅 TEAM_GAMES va 💍 PARA_GAMES holatlari ham shu yerda
    tiklanadi (avval faqat asosiy GAMES tiklanardi, bular yo'qolib qolardi)."""
    ...
    # --- 🐺🦅 Team Game holatini tiklash ---
    team_restored = 0
    for chat_id_s, tdata in (snapshot.get("team_games") or {}).items():
        try:
            chat_id = int(chat_id_s)
            tg = {
                "phase": tdata["phase"],
                "teams": {side: {int(u): n for u, n in members.items()} for side, members in tdata["teams"].items()},
                "alive": {side: {int(u): n for u, n in members.items()} for side, members in (tdata.get("alive") or {"wolf": {}, "eagle": {}}).items()},
                "round": tdata.get("round", 0),
                "night_votes": {side: {int(v): t for v, t in votes.items()} for side, votes in (tdata.get("night_votes") or {"wolf": {}, "eagle": {}}).items()},
                "msg_id": tdata.get("msg_id"),
                "timers": [],
            }
            TEAM_GAMES[chat_id] = tg
            team_restored += 1
            if tg["phase"] == "night":
                t = threading.Timer(10, lambda cid=chat_id: _team_resolve_night(cid))
                t.daemon = True
                t.start()
                tg["timers"].append(t)
            elif tg["phase"] == "day":
                t = threading.Timer(10, lambda cid=chat_id: _team_start_night(cid))
                t.daemon = True
                t.start()
                tg["timers"].append(t)
            if tg["phase"] in ("night", "day"):
                try:
                    bot.send_message(chat_id, "🔄 <b>Bot qayta ishga tushdi!</b> Jamoalar urushi avtomatik tiklandi — davom etamiz. 🐺🦅")
                except Exception:
                    pass
        except Exception:
            logging.exception(f"⚠️ {chat_id_s} guruhi uchun Team Game holatini tiklashda xatolik.")

    # --- 💍 Para Game holatini tiklash (tun/kun taymerlari ham qayta ishga tushiriladi) ---
    para_restored = 0
    for chat_id_s, pdata in (snapshot.get("para_games") or {}).items():
        try:
            chat_id = int(chat_id_s)
            pg = {
                "phase": pdata["phase"],
                "couples": {
                    int(cid): {"members": c["members"], "names": c["names"], "alive": c["alive"], "hp": c["hp"], "role": c["role"]}
                    for cid, c in pdata["couples"].items()
                },
                "member_to_couple": {int(u): cid for u, cid in pdata["member_to_couple"].items()},
                "next_couple_id": pdata["next_couple_id"],
                "msg_id": pdata.get("msg_id"),
                "mafia_votes": {int(v): t for v, t in pdata.get("mafia_votes", {}).items()},
                "protect_target": pdata.get("protect_target"),
                "day_votes": {int(v): t for v, t in pdata.get("day_votes", {}).items()},
                "timers": [],
                "day_number": pdata.get("day_number", 0),
            }
            PARA_GAMES[chat_id] = pg
            para_restored += 1
            if pg["phase"] == "night":
                t = threading.Timer(10, lambda cid=chat_id: _para_resolve_night(cid))
                t.daemon = True
                t.start()
                pg["timers"].append(t)
            elif pg["phase"] == "day":
                t = threading.Timer(10, lambda cid=chat_id: _para_resolve_day(cid))
                t.daemon = True
                t.start()
                pg["timers"].append(t)
            if pg["phase"] != "waiting":
                try:
                    bot.send_message(chat_id, "🔄 <b>Bot qayta ishga tushdi!</b> Para o'yin avtomatik tiklandi — davom etamiz. 💍")
                except Exception:
                    pass
        except Exception:
            logging.exception(f"⚠️ {chat_id_s} guruhi uchun Para Game holatini tiklashda xatolik.")

    if not games_data and not team_restored and not para_restored:
        return

    logging.info(
        f"🔄 Bot tiklandi: {restored} ta asosiy o'yin, {team_restored} ta Team Game, "
        f"{para_restored} ta Para Game holati qayta yuklandi."
    )
...
#
#  🆕 YANGI, TUSHUNARLI BUYRUQLAR TIZIMI (Para o'yin bilan bir xil naqshda):
#      /new_team    — YANGI ro'yxatga olishni ochadi (🐺 Bo'rilar / 🦅 Burgutlar
#                      tugmalari bilan xabar yuboriladi).
#      /povtor_team — Joriy ro'yxat xabarini QAYTA yuboradi (masalan xabar
#                      guruhda yuqoriga surilib ketgan bo'lsa) — yozilgan
#                      a'zolar SAQLANIB qoladi, hech kim o'chirilmaydi.
#      /start_team  — Ro'yxatni YOPADI va o'yinni YAKUNLAYDI: ko'proq a'zoli
#                      jamoa g'olib bo'ladi (teng bo'lsa — tasodifiy), butun
#                      guruhga tabriknoma yuboriladi.
#      /stop_team   — Jamoaviy o'yinni ISTALGAN PAYTDA, g'olibsiz, BUTUNLAY
#                      BEKOR qiladi (masalan xato bilan ochib yuborilgan bo'lsa).
#
#  🆕 TO'LIQ "JAMOALAR URUSHI" MEXANIKASI (asosiy Hunter Mafia o'yiniga o'xshab,
#  lekin yashirin rol o'rniga IKKI OSHKORA JAMOA bir-biriga qarshi kurashadi):
#   1) Faqat 2 ta tugma: "🐺 Bo'rilar" va "🦅 Burgutlar" — ortiqcha tugma yo'q.
#   2) Barcha 4 buyruq — FAQAT admin/owner uchun (is_authorized). Oddiy
#      foydalanuvchi buyruq bersa — "⛔ Siz admin emassiz" javobi.
#   3) Ro'yxatga olishda tugma bosilganda darhol shu jamoaga yoziladi; ikkala
#      jamoada bir vaqtda bo'la olmaydi — boshqa jamoaga o'tsa, avvalgisidan
#      avtomatik chiqariladi; xabar matnida ikkala jamoaning soni REAL
#      VAQTDA yangilanadi.
#   4) /start_team bosilganda — agar HAR IKKI jamoada kamida
#      TEAM_WAR_MIN_PER_SIDE (=2) ta a'zo bo'lsa, HAQIQIY "Jamoalar urushi"
#      boshlanadi:
#        🌙 Har TUNDA — har bir jamoa a'zolari shaxsiy (DM) xabarda dushman
#           safidan bittasini "yo'q qilish" uchun ovoz beradi (ko'pchilik
#           ovozi bilan tanlanadi, teng bo'lsa — tasodifiy). Ikkala jamoa
#           BIR VAQTDA harakat qiladi.
#        ☀️ Har KUNDA — natija guruhga e'lon qilinadi (kim halok bo'ldi,
#           qaysi jamoa hujum qildi), keyin qisqa tanaffusdan so'ng navbatdagi
#           tun boshlanadi.
#        🏆 G'ALABA: qarshi jamoa a'zolarining barchasi halok bo'lsa — o'sha
#           tomon g'olib. Ikkalasi ham "0" ga tushib qolsa — durrang.
#      Agar biror jamoada 2 tadan kam a'zo bo'lsa (haqiqiy urush uchun
#      yetarli emas) — botning ESKI, SODDA rejimi ishlaydi: darhol ko'proq
#      a'zoli jamoa g'olib e'lon qilinadi.
#   5) To'liq try/except — o'yin yo'qligida yoki takroriy bosishlarda bot
#      hech qachon "qotib qolmaydi" va xato bermaydi.
# ================================================================================

TEAM_WAR_MIN_PER_SIDE = 2   # shundan kam bo'lsa — soddalashtirilgan (darhol natija) rejim ishlaydi
TEAM_NIGHT_SECONDS = 30     # 🌙 har tunda ovoz berish uchun vaqt
TEAM_DAY_SECONDS = 15       # ☀️ natija e'lon qilingandan keyingi muhokama vaqti (asosiy Hunter Mafia'dagi kabi)
TEAM_DAY_VOTE_SECONDS = 45  # 🗳 ochiq kunduzgi ovoz berish vaqti (asosiy Hunter Mafia kunduz ovoz berishi bilan bir xil naqsh)
TEAM_DAY_CONFIRM_SECONDS = 20  # ✅ "Ha/Yo'q" tasdiqlash ovozi uchun vaqt (asosiy o'yindagi osish-tasdiqlash bilan bir xil)

TEAM_GAMES = {}          # chat_id -> {
                         #   "phase": "waiting"/"night"/"day"/"ended",
                         #   "teams": {"wolf": {uid: name}, "eagle": {uid: name}},   # ro'yxatga olingan TO'LIQ tarkib
                         #   "alive": {"wolf": {uid: name}, "eagle": {uid: name}},   # HOZIR tirik a'zolar (urush boshlanganda to'ldiriladi)
                         #   "msg_id": int, "round": int,
                         #   "night_votes": {"wolf": {voter_uid: target_uid}, "eagle": {...}},
                         #   "timers": [...],
                         # }
TEAM_GAMES_LOCK = threading.RLock()


def _team_cancel_timers(tg):
    for t in tg.get("timers", []):
        try:
            t.cancel()
        except Exception:
            pass
...
def _team_new_state():
    return {
        ...
        "msg_id": None,
        "round": 0,
        "night_votes": {"wolf": {}, "eagle": {}},
        "day_votes": {},          # 🗳 kunduz: voter_uid -> target_uid (ochiq, guruh ichida)
        "confirm_votes": {},      # ✅ tasdiqlash bosqichi: voter_uid -> "yes"/"no"
        "pending_target": None,   # kunduzgi ovoz g'olibi (tasdiqlash kutilmoqda)
        "day_sub_phase": None,    # "discussion" -> "voting" -> "confirm"
        "activity": {},           # 🏅 uid -> faollik balli (o'yin oxirida reyting uchun)
        "timers": [],
    }


def _team_bump_activity(tg, uid, amount=1):
    tg["activity"][uid] = tg.get("activity", {}).get(uid, 0) + amount
...
def _team_game_text(tg):
    ...
    if wolf:
        lines.append(", ".join(mention(u, n) for u, n in wolf.items()))
    lines.append(f"\n🦅 <b>Burgutlar:</b> {len(eagle)} ta")
    if eagle:
        lines.append(", ".join(mention(u, n) for u, n in eagle.items()))
    lines.append(
        f"\n\n👇 O'z jamoangizni tanlang (istalgan payt almashtirishingiz mumkin).\n"
        f"💡 Har ikki jamoada kamida {TEAM_WAR_MIN_PER_SIDE} tadan a'zo bo'lsa, "
        f"/start_team bilan HAQIQIY 🌙 tun/☀️ kun urushi boshlanadi!"
    )
    return "\n".join(lines)
...
def _team_already_active_text():
    return ("⚠️ Bu guruhda allaqachon faol Jamoaviy o'yin ro'yxati bor.\n"
            "• Xabarni qayta yuborish uchun: /povtor_team\n"
            "• Ro'yxatni yopib g'olibni chiqarish uchun: /start_team\n"
            "• Butunlay bekor qilish uchun: /stop_team")


@bot.message_handler(commands=["new_team"])
def cmd_new_team(message):
    """🐺🦅 FAQAT admin/owner — YANGI jamoaviy o'yin ro'yxatga olishni ochadi."""
    try:
        maybe_capture_owner(message.from_user)
        safe_delete(message)
        ...
        with TEAM_GAMES_LOCK:
            ...
            if old and old.get("phase") == "waiting":
                bot.send_message(message.chat.id, _team_already_active_text())
                return
            tg = _team_new_state()
            TEAM_GAMES[chat_id] = tg
            sent = bot.send_message(
                message.chat.id,
                _team_game_text(tg),
                reply_markup=_team_game_markup(chat_id),
            )
            tg["msg_id"] = sent.message_id
    except Exception as e:
        _logger.warning("cmd_new_team xatolik: %s", e)
        try:
...
def cmd_povtor_team(message):
    """🔁 FAQAT admin/owner — joriy Jamoaviy o'yin ro'yxati xabarini QAYTA
    yuboradi (masalan xabar guruhda yuqoriga surilib, yo'qolib qolgan bo'lsa).
    ...
    try:
        ...
        with TEAM_GAMES_LOCK:
            ...
            if not tg or tg.get("phase") != "waiting":
                bot.send_message(
                    message.chat.id,
                    "❌ Hozir faol Jamoaviy o'yin ro'yxati yo'q. Avval /new_team bilan oching.",
                )
                return
            if tg.get("msg_id"):
                try:
                    bot.delete_message(chat_id, tg["msg_id"])
                except Exception:
                    try:
                        bot.edit_message_reply_markup(chat_id, tg["msg_id"], reply_markup=None)
                    except Exception:
                        pass
            sent = bot.send_message(
                message.chat.id,
        ...
        _logger.warning("cmd_povtor_team xatolik: %s", e)
        try:
...
# ================================================================================
#  🌙☀️ JAMOALAR URUSHI — tun/kun harakatlari (start_team HAQIQIY urushni
#  boshlashga qaror qilganda ishga tushadi)
# ================================================================================

def _team_enemy(team):
    return "eagle" if team == "wolf" else "wolf"


def _team_label(team):
    return "🐺 Bo'rilar" if team == "wolf" else "🦅 Burgutlar"


def _team_start_night(chat_id):
    """🌙 Har ikki jamoaga (agar ularda tirik a'zo bo'lsa va dushman safida
    nishon qolgan bo'lsa) DM orqali "kimni yo'q qilamiz?" tugmalarini yuboradi."""
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg:
            ...
            tg["phase"] = "night"
            tg["round"] += 1
            tg["night_votes"] = {"wolf": {}, "eagle": {}}

            any_target_sent = False
            for team in ("wolf", "eagle"):
                enemy = _team_enemy(team)
                alive_side = tg["alive"][team]
                alive_enemy = tg["alive"][enemy]
                if not alive_side or not alive_enemy:
                    continue
                kb = types.InlineKeyboardMarkup()
                for t_uid, t_name in alive_enemy.items():
                    kb.add(types.InlineKeyboardButton(t_name, callback_data=f"teamwarvote|{chat_id}|{t_uid}"))
                for m_uid in alive_side:
                    safe_send(
                        m_uid,
                        f"🌙 <b>{tg['round']}-tun</b> — siz {_team_label(team)} safidasiz.\n"
                        f"Dushman ({_team_label(enemy)}) safidan kimni yo'q qilamiz?",
                        kb,
                    )
                    any_target_sent = True

            bot.send_message(
                chat_id,
                f"🌙 <b>{tg['round']}-tun tushdi...</b> Har ikki jamoa maxfiy ravishda "
                f"dushman safidan bittasini yo'q qilishga urinmoqda...",
            )
            send_scene_photo(
                chat_id, NIGHT_PHOTO_LOCAL, NIGHT_PHOTO,
                caption=f"🌙 <b>{tg['round']}-tun.</b> Shahar uxlamoqda... Guruhda yozish taqiqlangan.\n"
                        "Har ikki jamoa a'zolari — botning shaxsiy chatida sizni tanlov kutmoqda. 🤖",
            )

            if not any_target_sent:
                # dushman safida hech kim qolmagan bo'lishi mumkin — darhol tekshiramiz
                if _team_check_win(chat_id):
                    return

            t = threading.Timer(TEAM_NIGHT_SECONDS, lambda: _team_resolve_night(chat_id))
            t.daemon = True
            t.start()
            tg["timers"].append(t)
    except Exception as e:
        _logger.warning("_team_start_night xatolik: %s", e)


@bot.callback_query_handler(func=lambda c: c.data.startswith("teamwarvote|"))
def cb_team_war_vote(call):
    """🌙 Jamoa a'zosi DM'da dushmandan bittasini tanlaganda ovozi yoziladi."""
    try:
        maybe_capture_owner(call.from_user)
        _, chat_id_s, target_uid_s = call.data.split("|")
        chat_id, target_uid = int(chat_id_s), int(target_uid_s)
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg or tg.get("phase") != "night":
                bot.answer_callback_query(call.id, "❌ Hozir tun bosqichi emas.", show_alert=True)
                return
            uid = call.from_user.id
            voter_team = "wolf" if uid in tg["alive"]["wolf"] else ("eagle" if uid in tg["alive"]["eagle"] else None)
            if voter_team is None:
                bot.answer_callback_query(call.id, "❌ Siz bu urushda emassiz yoki halok bo'lgansiz.", show_alert=True)
                return
            enemy = _team_enemy(voter_team)
            if target_uid not in tg["alive"][enemy]:
                bot.answer_callback_query(call.id, "❌ Bu nishon endi mavjud emas.", show_alert=True)
                return
            tg["night_votes"][voter_team][uid] = target_uid
            _team_bump_activity(tg, uid, 2)
        bot.answer_callback_query(call.id, "✅ Ovozingiz qabul qilindi!")
    except Exception as e:
        _logger.warning("cb_team_war_vote xatolik: %s", e)
        try:
            bot.answer_callback_query(call.id, "⚠️ Xatolik yuz berdi.", show_alert=True)
        except Exception:
            pass
...
def _team_check_win(chat_id):
    """G'alaba shartini tekshiradi; o'yin tugagan bo'lsa True qaytaradi."""
    tg = TEAM_GAMES.get(chat_id)
    if not tg:
        return True
    wolf_alive = len(tg["alive"]["wolf"])
    eagle_alive = len(tg["alive"]["eagle"])
    if wolf_alive == 0 and eagle_alive == 0:
        _team_end_game(chat_id, None)
        return True
    if wolf_alive == 0:
        _team_end_game(chat_id, "eagle")
        return True
    if eagle_alive == 0:
        _team_end_game(chat_id, "wolf")
        return True
    return False


def _team_end_game(chat_id, winner_team):
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg:
                return
            _team_cancel_timers(tg)
            activity = tg.get("activity", {})

            if winner_team is None:
                # 🤝 Durrang — g'olib yo'q, lekin baribir faollik reytingini ko'rsatamiz.
                all_uids = list(tg["teams"]["wolf"].keys()) + list(tg["teams"]["eagle"].keys())
                all_uids.sort(key=lambda u: activity.get(u, 0), reverse=True)
                lines = ["🏁 <b>Jamoalar urushi tugadi!</b>\n", "🤝 Ikkala jamoa ham butunlay qirilib bitdi — DURRANG!\n", "🏅 <b>Faollik reytingi:</b>"]
                for i, u in enumerate(all_uids, start=1):
                    team = "wolf" if u in tg["teams"]["wolf"] else "eagle"
                    name = tg["teams"][team][u]
                    lines.append(f"{i}. {mention(u, name)} ({_team_label(team)})")
                bot.send_message(chat_id, "\n".join(lines))
            else:
                loser_team = _team_enemy(winner_team)
                winners = list(tg["teams"][winner_team].items())
                losers = list(tg["teams"][loser_team].items())
                winners.sort(key=lambda t: activity.get(t[0], 0), reverse=True)
                losers.sort(key=lambda t: activity.get(t[0], 0), reverse=True)

                lines = [
                    "🏁 <b>Jamoalar urushi tugadi!</b>\n",
                    f"🏆 <b>G'olib jamoa: {_team_label(winner_team)}!</b>\n",
                    "🏆 <b>G'oliblar (faollikka qarab tartiblangan):</b>",
                ]
                for i, (u, n) in enumerate(winners, start=1):
                    tag = " 🥇" if i == 1 else (" 🥈" if i == 2 else (" 🥉" if i == 3 else ""))
                    lines.append(f"{i}. {mention(u, n)}{tag}")
                lines.append("\n💀 <b>Mag'lublar (faollikka qarab tartiblangan):</b>")
                for i, (u, n) in enumerate(losers, start=1):
                    lines.append(f"{i}. {mention(u, n)}")
                bot.send_message(chat_id, "\n".join(lines))
            TEAM_GAMES.pop(chat_id, None)
    except Exception as e:
        _logger.warning("_team_end_game xatolik: %s", e)


def _team_resolve_night(chat_id):
    """🌙➜☀️ Har ikki jamoaning ovozlarini hisoblaydi, hujumlarni BIR VAQTDA
    (simultaneously) qo'llaydi va natijani e'lon qiladi."""
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg or tg.get("phase") != "night":
            ...
            attacks = {}  # team -> target_uid (o'sha jamoa dushmandan kimni o'ldirmoqchi)
            for team in ("wolf", "eagle"):
                votes = list(tg["night_votes"][team].values())
                if not votes:
                    continue
                tally = {}
                for v in votes:
                ...
                mx = max(tally.values())
                top = [uid for uid, cnt in tally.items() if cnt == mx]
                attacks[team] = random.choice(top)

            result_lines = []
            for team, target_uid in attacks.items():
                enemy = _team_enemy(team)
                victim_name = tg["alive"][enemy].pop(target_uid, None)
                if victim_name:
                    result_lines.append(
                        f"☠️ {_team_label(team)} — {_team_label(enemy)} safidan {mention(target_uid, victim_name)}ni yo'q qildi!"
                    )

            if not result_lines:
                bot.send_message(chat_id, "😴 Bu kecha hech kim hujum qilmadi — tinch tun bo'ldi. (Anonim: hujum qilganlar kimligi ochiqlanmaydi.)")
            else:
                bot.send_message(chat_id, f"☀️ <b>{tg['round']}-tun natijasi:</b>\n\n" + "\n".join(result_lines))

        if _team_check_win(chat_id):
            return
        _team_start_day(chat_id)
    except Exception as e:
        _logger.warning("_team_resolve_night xatolik: %s", e)


def _team_start_day(chat_id):
    """☀️ Kunduz — asosiy Hunter Mafia o'yinidagi kabi avval MUHOKAMA vaqti
    beriladi, so'ng ochiq (guruh ichida, tugmalar bilan) ovoz berish ochiladi."""
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg:
                return
            tg["phase"] = "day"
            tg["day_sub_phase"] = "discussion"
            wolf_alive = tg["alive"]["wolf"]
            eagle_alive = tg["alive"]["eagle"]
            send_scene_photo(
                chat_id, DAY_PHOTO_LOCAL, DAY_PHOTO,
                caption=f"☀️ <b>Kunduz.</b> Hozirgi holat:\n"
                        f"🐺 Bo'rilar: {len(wolf_alive)} ta tirik\n"
                        f"🦅 Burgutlar: {len(eagle_alive)} ta tirik\n\n"
                        f"💬 {TEAM_DAY_SECONDS} soniya muhokama qiling, so'ng ochiq ovoz berish tugmalari chiqadi.",
            )
            t = threading.Timer(TEAM_DAY_SECONDS, lambda: _team_open_day_voting(chat_id))
            t.daemon = True
            t.start()
            tg["timers"].append(t)
    except Exception as e:
        _logger.warning("_team_start_day xatolik: %s", e)


def _team_open_day_voting(chat_id):
    """🗳 Muhokamadan so'ng — asosiy Hunter Mafia'dagi kunduzgi ovoz berish
    bilan bir xil naqshda: guruhga barcha tirik o'yinchilar (ikkala jamoadan)
    ro'yxati tugma sifatida chiqadi, har bir tirik o'yinchi bittasini tanlaydi."""
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg or tg.get("phase") != "day":
                return
            tg["day_sub_phase"] = "voting"
            tg["day_votes"] = {}

            alive_all = []
            for team in ("wolf", "eagle"):
                for uid, name in tg["alive"][team].items():
                    alive_all.append((uid, name, team))

            if len(alive_all) < 2:
                _team_start_night(chat_id)
                return

            kb = types.InlineKeyboardMarkup()
            for uid, name, team in alive_all:
                icon = "🐺" if team == "wolf" else "🦅"
                kb.add(types.InlineKeyboardButton(f"{icon} {name}", callback_data=f"teamdv|{chat_id}|{uid}"))
            kb.add(types.InlineKeyboardButton("🚫 O'tkazib yuborish", callback_data=f"teamdv|{chat_id}|skip"))

            bot.send_message(
                chat_id,
                f"🗳 <b>Ochiq ovoz berish boshlandi!</b> Kimni o'yindan chiqarasiz?\n"
                f"⏳ {TEAM_DAY_VOTE_SECONDS} soniya vaqtingiz bor.",
                reply_markup=kb,
            )

            t = threading.Timer(TEAM_DAY_VOTE_SECONDS, lambda: _team_tally_day_vote(chat_id))
            t.daemon = True
            t.start()
            tg["timers"].append(t)
    except Exception as e:
        _logger.warning("_team_open_day_voting xatolik: %s", e)


@bot.callback_query_handler(func=lambda c: c.data.startswith("teamdv|"))
def cb_team_day_vote(call):
    """🗳 Ochiq kunduzgi ovoz — faqat tirik o'yinchilar (ikkala jamoadan ham)
    ovoz berishi mumkin, har biri bittadan."""
    try:
        maybe_capture_owner(call.from_user)
        ...
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg or tg.get("phase") != "day" or tg.get("day_sub_phase") != "voting":
                bot.answer_callback_query(call.id, "❌ Hozir ovoz berish bosqichi emas.", show_alert=True)
                return
            uid = call.from_user.id
            if uid not in tg["alive"]["wolf"] and uid not in tg["alive"]["eagle"]:
                bot.answer_callback_query(call.id, "❌ Siz bu urushda emassiz yoki halok bo'lgansiz.", show_alert=True)
                return
            if target_s == "skip":
                tg["day_votes"][uid] = "skip"
            else:
                target_uid = int(target_s)
                if target_uid not in tg["alive"]["wolf"] and target_uid not in tg["alive"]["eagle"]:
                    bot.answer_callback_query(call.id, "❌ Bu nishon endi mavjud emas.", show_alert=True)
                    return
                tg["day_votes"][uid] = target_uid
            _team_bump_activity(tg, uid, 2)
        bot.answer_callback_query(call.id, "✅ Ovozingiz qabul qilindi!")
    except Exception as e:
        _logger.warning("cb_team_day_vote xatolik: %s", e)
        try:
            bot.answer_callback_query(call.id, "⚠️ Xatolik yuz berdi.", show_alert=True)
        except Exception:
            pass
...
def _team_tally_day_vote(chat_id):
    """🗳➜✅ Ovozlarni hisoblaydi; g'olib bo'lsa asosiy o'yindagi kabi
    "Ha/Yo'q" tasdiqlash bosqichini ochadi."""
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg or tg.get("phase") != "day":
                return
            votes = [v for v in tg["day_votes"].values() if v != "skip"]
            if not votes:
                bot.send_message(chat_id, "🤷 Hech kim ovoz bermadi — bu kun hech kim chiqarilmadi.")
                tg["day_sub_phase"] = None
                t = threading.Timer(3, lambda: _team_start_night(chat_id))
                t.daemon = True
                t.start()
                tg["timers"].append(t)
                return
            ...
            top = [uid for uid, cnt in tally.items() if cnt == mx]
            if len(top) > 1:
                bot.send_message(chat_id, "🤝 Ovozlar teng taqsimlandi — bu kun hech kim chiqarilmadi.")
                tg["day_sub_phase"] = None
                t = threading.Timer(3, lambda: _team_start_night(chat_id))
                t.daemon = True
                t.start()
                ...
                return
            target_uid = top[0]
            tg["pending_target"] = target_uid
        _team_open_confirm(chat_id, target_uid)
    except Exception as e:
        _logger.warning("_team_tally_day_vote xatolik: %s", e)


def _team_open_confirm(chat_id, target_uid):
    """✅ Asosiy Hunter Mafia'dagi "osish" tasdiqlash bosqichi bilan bir xil:
    eng ko'p ovoz olgan nishonni chiqarish-chiqarmaslik ochiq "Ha/Yo'q" ovoz
    berish orqali yakuniy tasdiqlanadi."""
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg:
                return
            tg["day_sub_phase"] = "confirm"
            tg["confirm_votes"] = {}
            target_team = "wolf" if target_uid in tg["alive"]["wolf"] else "eagle"
            target_name = tg["alive"][target_team].get(target_uid, "O'yinchi")

            kb = types.InlineKeyboardMarkup()
            kb.row(
                types.InlineKeyboardButton("👍 Ha, chiqarish", callback_data=f"teamdvc|{chat_id}|yes"),
                types.InlineKeyboardButton("👎 Yo'q, qoldirish", callback_data=f"teamdvc|{chat_id}|no"),
            )
            bot.send_message(
                chat_id,
                f"⚖️ Eng ko'p ovoz to'plagan nishon: {mention(target_uid, target_name)} "
                f"({_team_label(target_team)}).\n\nUni HAQIQATAN o'yindan chiqaramizmi?",
                reply_markup=kb,
            )
            t = threading.Timer(TEAM_DAY_CONFIRM_SECONDS, lambda: _team_resolve_confirm(chat_id))
            t.daemon = True
            t.start()
            tg["timers"].append(t)
    except Exception as e:
        _logger.warning("_team_open_confirm xatolik: %s", e)


@bot.callback_query_handler(func=lambda c: c.data.startswith("teamdvc|"))
def cb_team_confirm_vote(call):
    try:
        maybe_capture_owner(call.from_user)
        ...
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg or tg.get("day_sub_phase") != "confirm":
                bot.answer_callback_query(call.id, "❌ Hozir tasdiqlash bosqichi emas.", show_alert=True)
                return
            uid = call.from_user.id
            if uid not in tg["alive"]["wolf"] and uid not in tg["alive"]["eagle"]:
                bot.answer_callback_query(call.id, "❌ Siz bu urushda emassiz yoki halok bo'lgansiz.", show_alert=True)
                return
            tg["confirm_votes"][uid] = choice
            _team_bump_activity(tg, uid, 1)
        bot.answer_callback_query(call.id, "✅ Ovozingiz qabul qilindi!")
    except Exception as e:
        _logger.warning("cb_team_confirm_vote xatolik: %s", e)
        try:
            bot.answer_callback_query(call.id, "⚠️ Xatolik yuz berdi.", show_alert=True)
        except Exception:
            pass
...
def _team_resolve_confirm(chat_id):
    """✅➜🌙 Tasdiqlash ovozlarini hisoblaydi, kerak bo'lsa nishonni chiqaradi
    (mention() bilan e'lon qilib), so'ng navbatdagi tunni boshlaydi."""
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg:
                return
            target_uid = tg.get("pending_target")
            yes_n = sum(1 for v in tg["confirm_votes"].values() if v == "yes")
            no_n = sum(1 for v in tg["confirm_votes"].values() if v == "no")
            tg["pending_target"] = None
            tg["day_sub_phase"] = None

            if target_uid is not None and yes_n > no_n:
                target_team = "wolf" if target_uid in tg["alive"]["wolf"] else "eagle"
                target_name = tg["alive"][target_team].pop(target_uid, None)
                if target_name:
                    bot.send_message(
                        chat_id,
                        f"⚖️ Ovoz berish natijasi ({yes_n} 👍 / {no_n} 👎): "
                        f"{mention(target_uid, target_name)} ({_team_label(target_team)}) o'yindan chiqarildi!",
                    )
            else:
                bot.send_message(chat_id, f"⚖️ Ovoz berish natijasi ({yes_n} 👍 / {no_n} 👎): hech kim chiqarilmadi.")

        if _team_check_win(chat_id):
            return
        _team_start_night(chat_id)
    except Exception as e:
        _logger.warning("_team_resolve_confirm xatolik: %s", e)


@bot.message_handler(commands=["start_team"])
def cmd_start_team(message):
    """▶️ FAQAT admin/owner — ro'yxatni YOPADI va o'yinni BOSHLAYDI:
    Har ikki jamoada kamida TEAM_WAR_MIN_PER_SIDE ta a'zo bo'lsa — HAQIQIY
    🌙 tun/☀️ kun "Jamoalar urushi" boshlanadi. Aks holda — ko'proq a'zoli
    jamoa darhol g'olib e'lon qilinadi (soddalashtirilgan yakun)."""
    try:
        maybe_capture_owner(message.from_user)
        safe_delete(message)
        if message.chat.type not in ("group", "supergroup"):
        ...
        with TEAM_GAMES_LOCK:
            ...
            # 🎬 Ikkala jamoada ham yetarlicha a'zo bo'lsa — HAQIQIY urush boshlanadi
            if len(wolf) >= TEAM_WAR_MIN_PER_SIDE and len(eagle) >= TEAM_WAR_MIN_PER_SIDE:
                if tg.get("msg_id"):
                    try:
                        bot.edit_message_reply_markup(chat_id, tg["msg_id"], reply_markup=None)
                    except Exception:
                        pass
                tg["alive"] = {"wolf": dict(wolf), "eagle": dict(eagle)}
                tg["round"] = 0
                bot.send_message(
                    message.chat.id,
                    f"🎬 <b>Jamoalar urushi boshlandi!</b>\n"
                    f"🐺 Bo'rilar: {len(wolf)} ta | 🦅 Burgutlar: {len(eagle)} ta\n\n"
                    f"Har tunda har ikki jamoa shaxsiy xabarda (DM) dushmandan bittasini "
                    f"yo'q qilishga ovoz beradi. Omad tilaymiz! 🍀",
                )
                _team_start_night(chat_id)
                return

            # ⚡️ Aks holda — eski, soddalashtirilgan zudlik bilan yakunlash rejimi
            tg["phase"] = "ended"
            ...
            names = ", ".join(mention(u, n) for u, n in winner.items()) if winner else "—"
            bot.send_message(
                message.chat.id,
                f"🏁 <b>Jamoaviy o'yin yakunlandi!</b>\n"
                f"⚠️ <i>(Haqiqiy urush uchun har ikki jamoada kamida {TEAM_WAR_MIN_PER_SIDE} tadan "
                f"a'zo kerak edi — shu sabab zudlik bilan hisob bo'yicha yakunlandi.)</i>\n\n"
                f"🐺 Bo'rilar: {len(wolf)} ta | 🦅 Burgutlar: {len(eagle)} ta\n\n"
...
def cmd_stop_team(message):
    """🛑 FAQAT admin/owner — Jamoaviy o'yinni (qaysi bosqichda bo'lishidan
    qat'i nazar — ro'yxat, tun yoki kun) g'olibsiz, DARHOL BUTUNLAY bekor
    qiladi va barcha taymerlarni to'xtatadi."""
    try:
        maybe_capture_owner(message.from_user)
        safe_delete(message)
        if message.chat.type not in ("group", "supergroup"):
        ...
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            if not tg:
                bot.send_message(message.chat.id, "❌ Hozir faol Jamoaviy o'yin yo'q.")
                return
            _team_cancel_timers(tg)
            if tg.get("msg_id"):
                try:
                    ...
                    pass
            TEAM_GAMES.pop(chat_id, None)
        bot.send_message(message.chat.id, "🛑 Jamoaviy o'yin bekor qilindi va barcha ro'yxat/urush holati tozalandi.")
    except Exception as e:

        _logger.warning("cmd_stop_team xatolik: %s", e)
        try:
            bot.send_message(message.chat.id, "⚠️ Bekor qilishda xatolik yuz berdi.")
        except Exception:
            pass
...
#
#  🆕 YANGI, TUSHUNARLI BUYRUQLAR TIZIMI (Jamoaviy o'yin bilan bir xil naqshda;
#  eski nomlar /parastart, /paraforce, /parastop ham hamon ishlaydi — pastda
#  ikkala nom ham bitta funksiyaga ulangan, hech narsa buzilmaydi):
#      /new_para    (= /parastart) — YANGI ro'yxatga olishni ochadi.
#      /povtor_para                — Joriy ro'yxat xabarini QAYTA yuboradi —
#                                     yozilgan juftliklar SAQLANIB qoladi.
#      /start_para  (= /paraforce) — Ro'yxatni YOPADI va o'yinni haqiqatan
#                                     BOSHLAYDI: rollarni juftliklarga
#                                     taqsimlaydi, birinchi Tun ochiladi.
#      /stop_para   (= /parastop)  — Para o'yinni istalgan bosqichda,
#                                     DARHOL, BUTUNLAY bekor qiladi.
#
...
#   4) Rollar JUFTLIKKA (ikkalasiga birdan) beriladi — asosiy Hunter Mafia
#      o'yinidagi Don/Komissar/Doktor uchligiga o'xshab: 🕶💍 Juftlik Mafiya
#      (tunda o'ldiradi), 👨‍⚕️💍 Juftlik Doktor (tunda himoya qiladi),
#      🕵️‍♂️💍 Juftlik Komissar (tunda tekshirib, natijani DARHOL popup orqali
#      biladi), qolganlari — 🏠💍 Juftlik Tinch aholi. Tun/Kunduz sikllari ishlaydi.
...
PARA_DAY_CONFIRM_SECONDS = 20  # ✅ asosiy Hunter Mafia'dagi "Ha/Yo'q" tasdiqlash bosqichi bilan bir xil
...
def new_para_game():
    return {
        ...
        "confirm_votes": {},      # ✅ tasdiqlash bosqichi: voter_uid -> "yes"/"no"
        "pending_target": None,   # kunduzgi ovoz g'olibi bo'lgan couple_id (tasdiqlash kutilmoqda)
        "activity": {},           # 🏅 uid -> faollik balli (o'yin oxirida reyting uchun)
        "timers": [],
        "day_number": 0,
    }
...
def _para_bump_activity(pg, uid, amount=1):
    pg["activity"][uid] = pg.get("activity", {}).get(uid, 0) + amount
...
def _para_couple_label(couple):
    """Tugma matni uchun — Telegram tugmalari HTML havolani ko'rsata olmaydi,
    shuning uchun bu yerda oddiy matn qoladi."""
    return f"{couple['names'][0]} 💍 {couple['names'][1]}"


def _para_couple_mention(couple):
    """Guruh xabarlari uchun — asosiy Hunter Mafia'dagi kabi ikkala a'zo ham
    bosilganda profiliga o'tadigan qilib (mention) ko'rsatiladi."""
    u1, u2 = couple["members"][0], couple["members"][1]
    n1, n2 = couple["names"][0], couple["names"][1]
    return f"{mention(u1, n1)} 💍 {mention(u2, n2)}"
...
def _para_game_text(pg):
    ...
    if pg["couples"]:
        for c in pg["couples"].values():
            lines.append(f"• {_para_couple_mention(c)}")
    else:
...
def _para_join_markup(chat_id):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("💍 O'yinga qo'shilish", callback_data=f"parajoin|{chat_id}"))
    return kb


def _para_already_active_text():
    return ("⚠️ Bu guruhda allaqachon faol Para o'yin bor.\n"
            "• Xabarni qayta yuborish uchun: /povtor_para\n"
            "• O'yinni boshlash uchun: /start_para\n"
            "• Butunlay bekor qilish uchun: /stop_para")


@bot.message_handler(commands=["new_para", "parastart"])
def cmd_parastart(message):
    """💍 FAQAT admin/owner — YANGI Para o'yin ro'yxatga olishni ochadi."""
    try:
        maybe_capture_owner(message.from_user)
        safe_delete(message)
        if message.chat.type not in ("group", "supergroup"):
        ...
        with PARA_GAMES_LOCK:
            ...
            if old and old.get("phase") != "ended":
                bot.send_message(message.chat.id, _para_already_active_text())
                return
            pg = new_para_game()
            PARA_GAMES[chat_id] = pg
            sent = bot.send_message(message.chat.id, _para_game_text(pg), reply_markup=_para_join_markup(chat_id))
...
def cmd_povtor_para(message):
    """🔁 FAQAT admin/owner — joriy Para o'yin ro'yxati xabarini QAYTA yuboradi
    (masalan xabar guruhda yuqoriga surilib, yo'qolib qolgan bo'lsa). Yozilgan
    juftliklar hech qayerga yo'qolmaydi — faqat xabar yangidan chiqadi."""
    try:
        maybe_capture_owner(message.from_user)
        safe_delete(message)
        if message.chat.type not in ("group", "supergroup"):
        ...
        with PARA_GAMES_LOCK:
            ...
            if not pg or pg.get("phase") != "waiting":
                bot.send_message(
                    message.chat.id,
                    "❌ Hozir ro'yxatga olish bosqichida Para o'yin yo'q. Avval /new_para bilan oching.",
                )
                return
            if pg.get("msg_id"):
                try:
                    bot.delete_message(chat_id, pg["msg_id"])
                except Exception:
                    try:
                        bot.edit_message_reply_markup(chat_id, pg["msg_id"], reply_markup=None)
                    except Exception:
                        pass
            sent = bot.send_message(message.chat.id, _para_game_text(pg), reply_markup=_para_join_markup(chat_id))
            pg["msg_id"] = sent.message_id
    except Exception as e:
        _logger.warning("cmd_povtor_para xatolik: %s", e)
        try:
            bot.send_message(message.chat.id, "⚠️ Xabarni qayta yuborishda xatolik yuz berdi.")
        except Exception:
            pass
...
def _para_assign_roles(pg):
    """🎭 Rollarni taqsimlaydi — asosiy Hunter Mafia o'yinidagi Don/Komissar/
    Doktor uchligiga o'xshab, PARA o'yinda ham 3 ta asosiy rol bor:
    Juftlik Mafiya, Juftlik Doktor, Juftlik Komissar (+ qolganlar Tinch aholi)."""
    couple_ids = list(pg["couples"].keys())
    random.shuffle(couple_ids)
    n = len(couple_ids)
    mafia_count = max(1, n // 3)
    doctor_count = 1 if n >= 2 else 0
    komissar_count = 1 if n >= 3 else 0
    for i, cid in enumerate(couple_ids):
        if i < mafia_count:
            role = "Juftlik Mafiya 🕶💍"
        elif i < mafia_count + doctor_count:
            role = "Juftlik Doktor 👨‍⚕️💍"
        elif i < mafia_count + doctor_count + komissar_count:
            role = "Juftlik Komissar 🕵️‍♂️💍"
        else:
            role = "Juftlik Tinch aholi 🏠💍"
        pg["couples"][cid]["role"] = role


@bot.message_handler(commands=["start_para", "paraforce"])
def cmd_paraforce(message):
    """▶️ FAQAT admin/owner — ro'yxatni yopib, o'yinni BOSHLAYDI: rollarni
    juftliklarga taqsimlaydi va birinchi Tun bosqichini ochadi. Kamida 2 ta
    juftlik ro'yxatdan o'tgan bo'lishi shart."""
    try:
        maybe_capture_owner(message.from_user)
        safe_delete(message)
        if message.chat.type not in ("group", "supergroup"):
...
def _para_start_night(chat_id):
    try:
        with PARA_GAMES_LOCK:
            ...
            komissar_couples = {cid: c for cid, c in alive.items() if c["role"] == "Juftlik Komissar 🕵️‍♂️💍"}
            ...
            for cid, c in komissar_couples.items():
                kb = types.InlineKeyboardMarkup()
                for t_cid, t_c in alive.items():
                    if t_cid == cid:
                        continue
                    kb.add(types.InlineKeyboardButton(_para_couple_label(t_c), callback_data=f"parainvestigate|{chat_id}|{t_cid}"))
                for m_uid in c["members"]:
                    safe_send(m_uid, f"🕵️‍♂️💍 <b>{pg['day_number']}-tun</b> — qaysi juftlikni tekshiramiz?", reply_markup=kb)

            bot.send_message(chat_id, f"🌙 <b>{pg['day_number']}-tun tushdi...</b> Juftliklar yashirin harakat qilmoqda.")
            send_scene_photo(
                chat_id, NIGHT_PHOTO_LOCAL, NIGHT_PHOTO,
                caption=f"🌙 <b>{pg['day_number']}-tun.</b> Shahar uxlamoqda... Guruhda yozish taqiqlangan.\n"
                        "Rolga ega juftliklar — botning shaxsiy chatida sizni tanlov kutmoqda. 🤖",
            )
...
def cb_para_protect(call):
    try:
        ...
        with PARA_GAMES_LOCK:
            ...
            _para_bump_activity(pg, call.from_user.id, 2)
...
@bot.callback_query_handler(func=lambda c: c.data.startswith("parainvestigate|"))
def cb_para_investigate(call):
    """🕵️‍♂️💍 Juftlik Komissar — tanlangan juftlikning Mafiya ekan-emasligini
    DARHOL (popup orqali, faqat so'ragan kishiga) bildiradi."""
    try:
        maybe_capture_owner(call.from_user)
        _, chat_id_s, target_cid_s = call.data.split("|")
        chat_id, target_cid = int(chat_id_s), int(target_cid_s)
        with PARA_GAMES_LOCK:
            ...
            if voter_cid is None or not pg["couples"][voter_cid]["alive"]:
                bot.answer_callback_query(call.id, "❌ Siz bu o'yinda emassiz yoki halok bo'lgansiz.", show_alert=True)
                return
            if pg["couples"][voter_cid]["role"] != "Juftlik Komissar 🕵️‍♂️💍":
                bot.answer_callback_query(call.id, "❌ Bu tugma faqat Juftlik Komissar uchun.", show_alert=True)
                return
            target = pg["couples"].get(target_cid)
            if not target or not target["alive"]:
                bot.answer_callback_query(call.id, "❌ Bu nishon endi mavjud emas.", show_alert=True)
                return
            is_mafia = target["role"] == "Juftlik Mafiya 🕶💍"
            label = _para_couple_label(target)
            _para_bump_activity(pg, uid, 2)
        verdict = "🕶 MAFIYA ekan!" if is_mafia else "✅ mafiya emas."
        bot.answer_callback_query(call.id, f"🕵️‍♂️💍 Tekshiruv natijasi: {label} — {verdict}", show_alert=True)
    except Exception as e:
        _logger.warning("cb_para_investigate xatolik: %s", e)
        try:
            bot.answer_callback_query(call.id, "⚠️ Xatolik yuz berdi.", show_alert=True)
        except Exception:
            pass
...
def _para_eliminate_couple(chat_id, pg, cid, reason):
    ...
    c["alive"] = False
    c["hp"] = 0
    u1, u2 = c["members"][0], c["members"][1]
    name1, name2 = c["names"]
    bot.send_message(
        chat_id,
        f"💔 {reason}\n"
        f"☠️ {mention(u1, name1)} halok bo'ldi, uning turmush o'rtog'i {mention(u2, name2)} ham "
    ...
    try:
        with PARA_GAMES_LOCK:
            ...
            if not pg:
                return
            _para_cancel_timers(pg)
            activity = pg.get("activity", {})
            if winner_side == "peace":
                winner_couples = [c for c in pg["couples"].values() if c["role"] != "Juftlik Mafiya 🕶💍"]
                loser_couples = [c for c in pg["couples"].values() if c["role"] == "Juftlik Mafiya 🕶💍"]
            ...
            def _individuals(couples):
                out = []
                for c in couples:
                    for u, n in zip(c["members"], c["names"]):
                        out.append((u, n))
                out.sort(key=lambda t: activity.get(t[0], 0), reverse=True)
                return out

            winners = _individuals(winner_couples)
            losers = _individuals(loser_couples)

            lines = [
                "🏁 <b>Para o'yin tugadi!</b>\n",
    ...
    try:
        with PARA_GAMES_LOCK:
            ...
            text = (
                ...
                "\n".join(f"• {_para_couple_mention(c)}" for c in alive.values()) +
                "\n\nKimni o'yindan chiqarasiz? Pastdagi tugmalar orqali ovoz bering:"
            )
            send_scene_photo(chat_id, DAY_PHOTO_LOCAL, DAY_PHOTO, caption=text, reply_markup=kb)
...
def _para_resolve_day(chat_id):
    """🗳➜✅ Ovozlarni hisoblaydi; g'olib chiqqan juftlik bo'lsa, asosiy
    Hunter Mafia'dagi kabi ochiq "Ha/Yo'q" tasdiqlash bosqichini ochadi
    (darhol chiqarib yubormaydi)."""
    try:
        with PARA_GAMES_LOCK:
            pg = PARA_GAMES.get(chat_id)
            if not pg or pg.get("phase") != "day":
            ...
            if not votes:
                bot.send_message(chat_id, "🤷 Hech kim ovoz bermadi — bu kun hech kim chiqarilmadi.")
                t = threading.Timer(3, lambda: _para_start_night(chat_id))
                t.daemon = True
                t.start()
                pg["timers"].append(t)
                return
            ...
            if len(top) > 1:
                ...
                return
            target_cid = top[0]
            pg["pending_target"] = target_cid
        _para_open_confirm(chat_id, target_cid)
    except Exception as e:
        _logger.warning("_para_resolve_day xatolik: %s", e)


def _para_open_confirm(chat_id, target_cid):
    """✅ Eng ko'p ovoz olgan juftlikni chiqarish-chiqarmaslikni yakuniy
    ochiq "Ha/Yo'q" ovoz berish orqali tasdiqlaydi (asosiy Hunter Mafia'dagi
    osish-tasdiqlash bosqichi bilan bir xil naqsh)."""
    try:
        with PARA_GAMES_LOCK:
            pg = PARA_GAMES.get(chat_id)
            if not pg:
            ...
            kb = types.InlineKeyboardMarkup()
            kb.row(
                types.InlineKeyboardButton("👍 Ha, chiqarish", callback_data=f"paradvc|{chat_id}|yes"),
                types.InlineKeyboardButton("👎 Yo'q, qoldirish", callback_data=f"paradvc|{chat_id}|no"),
            )
            bot.send_message(
                chat_id,
                f"⚖️ Eng ko'p ovoz to'plagan juftlik: {_para_couple_mention(target)}.\n\n"
                f"Ularni HAQIQATAN o'yindan chiqaramizmi?",
                reply_markup=kb,
            )
            t = threading.Timer(PARA_DAY_CONFIRM_SECONDS, lambda: _para_resolve_confirm(chat_id))
            t.daemon = True
            t.start()
            pg["timers"].append(t)
    except Exception as e:
        _logger.warning("_para_open_confirm xatolik: %s", e)


@bot.callback_query_handler(func=lambda c: c.data.startswith("paradvc|"))
def cb_para_confirm_vote(call):
    try:
        maybe_capture_owner(call.from_user)
        _, chat_id_s, choice = call.data.split("|")
        chat_id = int(chat_id_s)
        with PARA_GAMES_LOCK:
            pg = PARA_GAMES.get(chat_id)
            if not pg or pg.get("phase") != "dayconfirm":
                bot.answer_callback_query(call.id, "❌ Hozir tasdiqlash bosqichi emas.", show_alert=True)
                return
            uid = call.from_user.id
            voter_cid = pg["member_to_couple"].get(uid)
            if voter_cid is None or not pg["couples"][voter_cid]["alive"]:
                bot.answer_callback_query(call.id, "❌ Siz bu o'yinda emassiz yoki halok bo'lgansiz.", show_alert=True)
                return
            pg["confirm_votes"][uid] = choice
            _para_bump_activity(pg, uid, 1)
        bot.answer_callback_query(call.id, "✅ Ovozingiz qabul qilindi!")
    except Exception as e:
        _logger.warning("cb_para_confirm_vote xatolik: %s", e)
        try:
            bot.answer_callback_query(call.id, "⚠️ Xatolik yuz berdi.", show_alert=True)
        except Exception:
            pass
...
def _para_resolve_confirm(chat_id):
    try:
        with PARA_GAMES_LOCK:
            pg = PARA_GAMES.get(chat_id)
            if not pg:
                return
            target_cid = pg.get("pending_target")
            yes_n = sum(1 for v in pg["confirm_votes"].values() if v == "yes")
            no_n = sum(1 for v in pg["confirm_votes"].values() if v == "no")
            pg["pending_target"] = None
            pg["phase"] = "day"

            if target_cid is not None and yes_n > no_n and pg["couples"].get(target_cid, {}).get("alive"):
                _para_eliminate_couple(
                    chat_id, pg, target_cid,
                    f"⚖️ Ovoz berish natijasi ({yes_n} 👍 / {no_n} 👎) — aholi qaror qildi:",
                )
            else:
                bot.send_message(chat_id, f"⚖️ Ovoz berish natijasi ({yes_n} 👍 / {no_n} 👎): hech kim chiqarilmadi.")
        ...
        _logger.warning("_para_resolve_confirm xatolik: %s", e)


# ================================================================================
#  📜 QOIDALAR — /jamoa_qoida (Team Game) va /juftlik_qoida (Para Game)
#  Har ikkalasi ham endi asosiy Hunter Mafia o'yinidagi kabi: 🌙 tun (maxfiy/
#  anonim DM ovoz berish) → ☀️ kun (muhokama → ochiq tugmali ovoz berish →
#  "Ha/Yo'q" tasdiqlash) sikliga ega, va barcha e'lonlarda o'yinchi ismi
#  bosilganda profiliga o'tadigan (mention/nikneyn-teg) qilib ko'rsatiladi.
# ================================================================================
...
TEAM_GAME_RULES_TEXT = (
    ...
    "• Har bir o'yinchi 🐺 Bo'rilar yoki 🦅 Burgutlar tugmasini bosib jamoasini tanlaydi "
    "(istalgan payt boshqa jamoaga o'tishi mumkin).\n"
    "• /povtor_team — ro'yxat xabari yo'qolib qolsa, qayta chiqaradi (yozilganlar saqlanadi).\n"
    f"• /start_team — ro'yxatni yopadi. Ikkala jamoada kamida {TEAM_WAR_MIN_PER_SIDE} tadan a'zo bo'lsa, "
    "HAQIQIY 🌙 tun/☀️ kun urushi boshlanadi.\n\n"
    "<b>2) 🌙 Tun (maxfiy, anonim)</b>\n"
    f"• Har ikki jamoaning tirik a'zolariga shaxsiy xabarda (DM) \"kimni yo'q qilamiz?\" tugmalari yuboriladi.\n"
    f"• Kim qanday ovoz berganini guruhda hech kim bilmaydi — natija faqat umumiy shaklda e'lon qilinadi.\n"
    f"• Har jamoa eng ko'p ovoz olgan dushmanni yo'q qiladi. Vaqt: {TEAM_NIGHT_SECONDS} soniya.\n\n"
    "<b>3) ☀️ Kun (asosiy Hunter Mafia bilan bir xil naqsh)</b>\n"
    f"• Avval {TEAM_DAY_SECONDS} soniya ochiq muhokama.\n"
    "• So'ng guruhda BARCHA tirik o'yinchilar (ikkala jamoadan ham) tugma sifatida chiqadi — "
    f"har bir tirik o'yinchi ochiq ovoz beradi. Vaqt: {TEAM_DAY_VOTE_SECONDS} soniya.\n"
    "• Eng ko'p ovoz olgan o'yinchi \"👍 Ha, chiqarish / 👎 Yo'q, qoldirish\" tasdiqlash ovoziga chiqadi "
    f"(asosiy o'yindagi osish-tasdiqlash bilan bir xil). Vaqt: {TEAM_DAY_CONFIRM_SECONDS} soniya.\n"
    "• Ko'pchilik \"Ha\" desa — o'sha o'yinchi o'yindan chiqariladi.\n\n"
    "<b>4) Ismlar</b>\n"
    "• Har bir o'lim, g'alaba va ro'yxat xabarida o'yinchi ismi bosilsa uning profiliga o'tadigan "
    "qilib (nikneyn-teg / mention) ko'rsatiladi — xuddi asosiy Hunter Mafia o'yinidagi kabi.\n\n"
    "<b>5) G'alaba sharti</b>\n"
    "• Dushman jamoaning barcha a'zosi yo'q qilinsa — sizning jamoangiz g'olib!\n\n"
    "<b>6) Boshqaruv</b>\n"
    "• /stop_team — o'yinni istalgan bosqichda (ro'yxat, tun, kun) darhol butunlay bekor qiladi."
)

PARA_GAME_RULES_TEXT = (
    "💍 <b>Para o'yin (Nikohdagi juftliklar) — to'liq qoidalar</b>\n\n"
    "<b>1) Ro'yxatdan o'tish</b>\n"
    "• Admin /new_para (= /parastart) bilan ro'yxatni ochadi.\n"
    "• FAQAT botda RASMIY nikohdagi juftliklar \"💍 O'yinga qo'shilish\" tugmasini bosadi — "
    "ikkalasi BIRGA, bitta juftlik sifatida yoziladi. Nikohsiz bosgan foydalanuvchiga popup chiqadi.\n"
    "• /povtor_para — ro'yxat xabarini qayta chiqaradi (yozilgan juftliklar saqlanadi).\n"
    "• /start_para (= /paraforce) — ro'yxatni yopadi, har juftlikka BIRDAN rol beradi va birinchi tunni boshlaydi.\n\n"
    "<b>2) Rollar (juftlikka birgalikda beriladi)</b>\n"
    "• 🕶💍 Juftlik Mafiya — tunda dushman juftlikni yo'q qiladi.\n"
    "• 👨‍⚕️💍 Juftlik Doktor — tunda bir juftlikni himoya qiladi.\n"
    "• 🕵️‍♂️💍 Juftlik Komissar — tunda bir juftlikni tekshiradi, natijani DARHOL shaxsiy popup orqali biladi.\n"
    "• 🏠💍 Juftlik Tinch aholi — maxsus qobiliyatsiz, ovoz berish orqali kurashadi.\n\n"
    "<b>3) 💔 Bog'langan taqdir (eng muhim qoida)</b>\n"
    "• Juftlikning BIR a'zosi halok bo'lsa (tunda yoki ovoz berishda), ikkinchisi ham "
    "AVTOMATIK ravishda, DARHOL o'yindan chiqariladi.\n\n"
    f"<b>4) 🌙 Tun</b> — vaqt: {PARA_NIGHT_SECONDS} soniya, harakatlar shaxsiy xabarda (DM), maxfiy.\n\n"
    "<b>5) ☀️ Kun (asosiy Hunter Mafia bilan bir xil naqsh)</b>\n"
    "• Guruhda barcha tirik juftliklar tugma sifatida chiqadi — ochiq ovoz berish. "
    f"Vaqt: {PARA_DAY_VOTE_SECONDS} soniya.\n"
    "• Eng ko'p ovoz olgan juftlik \"👍 Ha, chiqarish / 👎 Yo'q, qoldirish\" tasdiqlash ovoziga chiqadi. "
    f"Vaqt: {PARA_DAY_CONFIRM_SECONDS} soniya.\n"
    "• Ko'pchilik \"Ha\" desa — o'sha juftlik (ikkalasi ham) o'yindan chiqariladi.\n\n"
    "<b>6) Ismlar</b>\n"
    "• Har bir juftlik ismi (ikkala a'zo ham) barcha e'lonlarda bosilsa profiliga o'tadigan "
    "qilib (nikneyn-teg / mention) ko'rsatiladi.\n\n"
    "<b>7) G'alaba sharti</b>\n"
    "• Barcha Mafiya juftliklar yo'q qilinsa — Tinch aholi juftliklar g'olib.\n"
    "• Mafiya juftliklar soni tirik qolgan Tinch aholi juftliklar soniga teng yoki ko'p bo'lib qolsa — Mafiya g'olib.\n\n"
    "<b>8) Boshqaruv</b>\n"
    "• /stop_para (= /parastop) — o'yinni istalgan bosqichda darhol butunlay bekor qiladi."
)
...
def build_join_text(game):
    ...
    if game.get("squad_mode"):
        ...
        # rollar ham maxfiy bo'lgani kabi).
        lines.append(f"🐺🦅 <b>Jamoalar tanlandi:</b> {wolf_n + eagle_n} kishi (kim qaysi jamoada — o'yin OXIRIGACHA maxfiy!)\n")
...
def group_text_guard(message):
    ...
    # 🐺🦅 Jamoaviy o'yin — asosiy Hunter Mafia bilan bir xil "anonim tun /
    # faqat tiriklar gapiradi" qoidasi: 🌙 tunda guruh xabarlari o'chiriladi
    # (harakatlar faqat DM orqali, anonim tarzda amalga oshadi), ☀️ kunduzda
    # esa faqat tirik ishtirokchilar yoza oladi.
    tg = TEAM_GAMES.get(chat_id)
    if tg:
        tphase = tg.get("phase")
        if tphase == "night":
    ...
    # 💍 Para o'yin — xuddi shu qoida: 🌙 tunda anonim (guruh xabarlari
    # o'chiriladi), ☀️ kunduzda faqat tirik juftlik a'zolari yoza oladi.
    pg = PARA_GAMES.get(chat_id)
    if pg:
        pphase = pg.get("phase")
        if pphase == "night":
            safe_delete(message)
            return
        if pphase in ("day", "dayconfirm"):
            uid = message.from_user.id
            if pg["member_to_couple"].get(uid) is None or not pg["couples"].get(pg["member_to_couple"][uid], {}).get("alive"):
                safe_delete(message)
            return
...
# ================================================================================
#  🧠 QULAYLIK #4: Theron AI (Gemini) sozlanganmi — bitta tugma bilan tekshirish
...
def cb_admin_ai_status(call):
    ...
    bot.answer_callback_query(call.id)
    if theron_available():
        model_line = f"🔧 Hozirgi ishlayotgan model: <code>{_GEMINI_WORKING_MODEL or 'hali sinalmagan'}</code>"
        text = (
            f"🧠 <b>Theron AI holati:</b> ✅ Sozlangan va ishlayapti.\n\n{model_line}\n"
        ...
        text = (
            "🧠 <b>Theron AI holati:</b> ❌ Sozlanmagan.\n\n"
            "GEMINI_API_KEY environment o'zgaruvchisi o'rnatilmagan. Uni hosting "
            "(Render va h.k.) sozlamalaridan qo'shing — https://aistudio.google.com/app/apikey "
            "orqali bepul olish mumkin."
        )
    bot.send_message(call.message.chat.id, text)
...
#
#  Foydalanuvchilar bazasi + barcha o'yin holatlari (GAMES/TEAM_GAMES/PARA_GAMES)
