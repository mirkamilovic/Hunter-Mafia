...
def _theron_raw_call(system_instruction, user_prompt, history=None, max_tokens=500, temperature=0.9):
    """Gemini API'ga xom (past darajadagi) so'rov. Har qanday xatolikda None
    qaytaradi — chaqiruvchi tomon buni albatta "AI hozircha javob berolmadi"
    kabi yumshoq xabarga aylantirishi kerak, hech qachon foydalanuvchiga xom
    Python xatoligini ko'rsatmaslik kerak."""
    ...
    try:
        ...
        _logger.warning("🧠 Theron AI (Gemini) so'rovida xatolik: %s", e)
        return None
...
def cmd_theron(message):
    ...
    try:
        ...
        _logger.warning("cmd_theron xatolik: %s", e)
        try:
...
def cmd_theron_hisobot(message):
    ...
    try:
        ...
        _logger.warning("cmd_theron_hisobot xatolik: %s", e)
        try:
