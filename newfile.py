...
def cmd_sell(message):
    ...
    parts = message.text.split()
    if len(parts) < 3:
        # Raqam/narx berilmagan — kod5.py'dagi kabi, birinchi navbatda inventarni
        # tartib raqamlari bilan ko'rsatamiz
        if not inv:
        ...
        lines.append("\n<i>Sotish uchun: </i><code>/sell &lt;raqam&gt; &lt;narx_HunterCoin&gt;</code>")
    ...
    with db_lock:
        cur.execute(
            "INSERT INTO market_listings (chat_id, seller_id, seller_name, item, price, active) VALUES (?,?,?,?,?,1)",
            (message.chat.id, uid, seller_name, item_to_sell, price),
        )
        conn.commit()
        listing_id = cur.lastrowid

    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton(f"🛒 Sotib olish ({price} 🪙)", callback_data=f"mbuy|{listing_id}"))
    bot.send_message(
        message.chat.id,
        f"📢 <b>Qora bozorda yangi e'lon!</b>\n\n"
        f"📦 Narsa: <b>{item_to_sell}</b>\n"
        f"💰 Narx: <b>{price}</b> 🪙 Hunter Coin\n"
...
def cmd_market(message):
    ...
    with db_lock:
        cur.execute(
            "SELECT id, item, price, seller_name FROM market_listings WHERE chat_id=? AND active=1 ORDER BY id DESC LIMIT 30",
            (message.chat.id,),
        )
        rows = cur.fetchall()
    if not rows:
        bot.send_message(
            ...
            "O'z narsangizni sotish uchun <code>/sell &lt;raqam&gt; &lt;narx&gt;</code> buyrug'idan foydalaning.",
        )
        return
    lines = ["🛒 <b>Qora bozordagi faol e'lonlar:</b>\n"]
    for listing_id, item, price, seller_name in rows:
        lines.append(f"🆔 <code>{listing_id}</code> | 📦 <b>{item}</b> — 💰 {price} 🪙 | 👤 {seller_name}")
...
def _show_market_buy_confirm(chat_id, message_id, listing_id, buyer_id):
    with db_lock:
        cur.execute("SELECT item, price, seller_id, seller_name FROM market_listings WHERE id=? AND active=1", (listing_id,))
        row = cur.fetchone()
    if not row:
        bot.send_message(chat_id, "❌ Bu buyum allaqachon sotilgan yoki e'lon topilmadi!")
        return
    item, price, seller_id, seller_name = row
    ...
    bot.send_message(chat_id, f"❓ <b>{item}</b>ni {price} 🪙 evaziga haqiqatan ham sotib olmoqchimisiz?", reply_markup=kb)
...
def cb_market_buy(call):
    ...
    try:
        bot.edit_message_text(
            f"❓ <b>{item}</b>ni {price} 🪙 evaziga haqiqatan ham sotib olmoqchimisiz?",
            call.message.chat.id, call.message.message_id, reply_markup=kb,
        )
    except Exception:
        bot.send_message(call.message.chat.id, f"❓ <b>{item}</b>ni {price} 🪙 evaziga haqiqatan ham sotib olmoqchimisiz?", reply_markup=kb)
...
def cb_market_confirm(call):
    ...
    with db_lock:
        cur.execute("SELECT item, price, seller_id, seller_name, active FROM market_listings WHERE id=?", (listing_id,))
        row = cur.fetchone()
        if not row or row[4] != 1:
            bot.answer_callback_query(call.id, "❌ Xatolik: e'lon topilmadi yoki allaqachon sotilgan!", show_alert=True)
            return
        item, price, seller_id, seller_name, _ = row
        if seller_id == buyer_id:
            bot.answer_callback_query(call.id, "❌ O'z narsangizni o'zingiz sotib ololmaysiz!", show_alert=True)
            return
        buyer = user_dict(buyer_id, call.from_user.first_name)
        if buyer["coin"] < price:
            bot.answer_callback_query(call.id, f"❌ Hunter Coin yetarli emas! Sizda {buyer['coin']} 🪙 bor, kerak: {price} 🪙.", show_alert=True)
            return
        cur.execute("UPDATE market_listings SET active=0 WHERE id=?", (listing_id,))
        conn.commit()

    add_balance(buyer_id, coin=-price)
    add_balance(seller_id, coin=price)
...
def cb_menu(call):
    ...
    if action == "roles":
        ...
        bot.send_message(
            call.message.chat.id,
            "🛒 <b>QORA BOZOR</b>\n\n"
            "Bu yerda o'yinchilar o'z inventaridagi buyumlarni Hunter Coin evaziga "
            "bir-biriga sotishlari mumkin.\n\n"
            "📦 <code>/sell</code> — inventaringizni raqamlangan holda ko'rish\n"
            "📦 <code>/sell &lt;raqam&gt; &lt;narx&gt;</code> — buyumni guruh bozoriga qo'yish\n"
            "🛒 <code>/market</code> — shu guruhdagi faol e'lonlarni ko'rish\n"
            "🛒 <code>/buy &lt;ID&gt;</code> — e'londagi buyumni sotib olish\n\n"
            "<i>Eslatma: savdo har bir guruhning o'z ichida amalga oshiriladi.</i>",
        )
        bot.answer_callback_query(call.id)
