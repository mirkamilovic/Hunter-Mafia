...
FAQ_KB = [
    ...
    {"keywords": ["hunter coin", "coin nima", "coin qanday"],
     "answer": "🪙 Hunter Coin — botning ichki ONLAYN VALYUTASI: kursi vaqt bilan tebranib turadi (<code>/kurs</code> orqali kuzating), do'kondan buyum olish uchun ishlatiladi, Olmosga ayirboshlanadi (<code>/olmos_coin</code>) va boshqa foydalanuvchiga o'tkazsa ham bo'ladi (<code>/coinsovga</code>). Telegram Stars orqali <code>/birja</code> menyusidan sotib olasiz."},
...
ROLES_INFO = {
    ...
    "Kezuvchi 💋": "Kechasi biron o'yinchining oldiga borib, uning tungi qobiliyatidan foydalanishiga to'sqinlik qiladi.",
...
EXTRA_ROLE_PROMPTS = {
    ...
    "fohisha":    ("💋 Kimning oldiga borib, uni band qilamiz?", "💋 <i>Kezuvchi birovning uyiga yo'l oldi...</i>"),
...
NIGHT_FLAVOR_TEXTS = {
    ...
    "Kezuvchi 💋": "💋 Shahar ko'chalarida Kezuvchi kezib yurdi...",
...
VISIT_PHRASE = {
    ...
    "Kezuvchi 💋": "Aytishlaricha, unikiga tunda Kezuvchi tashrif buyurgan...",
    "Sadoqatli yordamchi 🤝": "Aytishlaricha, unikiga Sadoqatli yordamchi kelgan...",
    "O'g'ri 🥷": "Aytishlaricha, unikiga tunda O'g'ri kirib chiqqan...",
    ...
    "Arvoh 👻": "Aytishlaricha, unikiga Arvoh sirli imo-ishora bilan kelgan...",
...
def cb_buy(call):
    ...
    if mode == "shield":
        ...
        # 🔧 x2/x3 multiplikatorlar endi ABADIY emas — har xarid faqat
        # MULTIPLIER_DURATION_SECONDS (3 kun) davomida amal qiladi, muddat
        # tugagach avtomatik x1'ga qaytadi (qayta xarid qilib uzaytirish kerak).
        ch = get_charges(uid)
...
def cmd_hc_kurs(message):
    ...
    bot.send_message(
        ...
        "💱 Ayirboshlash: /olmos_coin (💎↔🪙)  •  /coinsovga (🪙 sovg'a)",
    )


# --------------------------------------------------------------------------------
#  💵↔🪙 DOLLAR ↔ HUNTER COIN AYIRBOSHLASH — O'CHIRILDI
#  (Dollar endi na Olmosga, na Hunter Coinga ayirboshlanmaydi. Eski xabarlarda
#  qolib ketgan tugmalar/buyruqlar bosilsa ham, foydalanuvchiga shunchaki
#  funksiya o'chirilgani haqida xabar beriladi — xatolik chiqmaydi.)
# --------------------------------------------------------------------------------

_DOLLAR_EXCHANGE_DISABLED_TEXT = "🚫 Dollarni Olmosga yoki Hunter Coinga ayirboshlash funksiyasi o'chirilgan."
...
def cmd_dollar_coin(message):
    maybe_capture_owner(message.from_user)
    safe_delete(message)
    bot.send_message(message.chat.id, _DOLLAR_EXCHANGE_DISABLED_TEXT)


@bot.callback_query_handler(func=lambda c: c.data == "open_dollar_to_coin")
def cb_open_dollar_to_coin(call):
    maybe_capture_owner(call.from_user)
    bot.answer_callback_query(call.id, _DOLLAR_EXCHANGE_DISABLED_TEXT, show_alert=True)
...
_HELP_CATEGORIES = [
    ...
    ("🛒 Do'kon, bozor va ELITE", ["almazdokon", "dollar_dokon", "olmos_coin", "kurs", "sell", "buy",
