...
def cb_menu(call):
    ...
    if action == "roles":
        ...
        text = (
            ...
            "/bazani_tikla — zaxira nusxadan (.json) bazani tiklash (bot egasi)"
        )
...
def cb_admin_panel(call):
    ...
    kb.add(types.InlineKeyboardButton("🔄 Bazani tiklash (restore)", callback_data="admin|restore_info"))
    bot.send_message(
        call.message.chat.id,
...
@bot.callback_query_handler(func=lambda c: c.data == "admin|restore_info")
def cb_admin_restore_info(call):
    ...
    bot.send_message(
        call.message.chat.id,
        "🔄 <b>Bazani tiklash:</b>\n\n"
        "<code>/bazani_tikla</code> deb yozing, so'ng bot so'ragan joyga "
        "avvalgi zaxira nusxa (.json) faylni yuboring.",
    )


@bot.message_handler(commands=["bazani_tikla"])
def cmd_bazani_tikla(message):
    """Avval /admin panelidan olingan zaxira (backup) JSON faylni bazaga
    qaytarib qo'yadi. Faqat bot egasi ishlata oladi."""
    maybe_capture_owner(message.from_user)
    safe_delete(message)
    if not is_owner(message.from_user.id):
        bot.send_message(message.chat.id, "⛔ Bu buyruq faqat bot egasi uchun.")
    ...
    msg = bot.send_message(
        ...
        "⚠️ Diqqat: bu amal joriy bazadagi mos foydalanuvchilarning ma'lumotini "
        "backup fayldagi qiymatlar bilan <b>almashtiradi</b> (ustma-ust yozadi). "
        "Backup faylda yo'q foydalanuvchilarga tegmaydi.",
    )
    bot.register_next_step_handler(msg, _process_restore_upload)


def _process_restore_upload(message):
    if not is_owner(message.from_user.id):
        return
    if not message.document:
        bot.send_message(message.chat.id, "❌ Fayl topilmadi (fayl sifatida yuborilmadi). Qaytadan /bazani_tikla deb urinib ko'ring.")
        return
    try:
        ...
        if not isinstance(data, list):
            raise ValueError("Fayl formati noto'g'ri — JSON ro'yxat (list) bo'lishi kerak.")
        ...
        for entry in data:
            if not isinstance(entry, dict) or entry.get("user_id") is None:
                ...
                continue
            uid = entry["user_id"]
            # Faqat backup faylida haqiqatan mavjud bo'lgan ustunlarni yozamiz —
            # eski (masalan married_at qo'shilishidan oldingi) backuplarda yo'q
            # ustunlar shunchaki o'tkazib yuboriladi, xatolik bermaydi.
            fields = {col: entry[col] for col in USER_COLS if col != "user_id" and col in entry and entry[col] is not None}
            with db_lock:
                ...
                if exists:
                    ...
                    cur.execute(f"INSERT INTO users ({', '.join(cols)}) VALUES ({placeholders})", vals)
                conn.commit()
            restored += 1

        bot.send_message(
            message.chat.id,
            f"✅ Tiklash yakunlandi!\n👥 Tiklangan/yangilangan: {restored} ta\n⚠️ O'tkazib yuborilgan (noto'g'ri format): {skipped} ta",
        )
    except Exception as e:
