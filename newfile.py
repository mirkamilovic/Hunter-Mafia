...
TEAM_DAY_SECONDS = 15       # ☀️ natija e'lon qilingandan keyingi tanaffus
    ...
    try:
        with TEAM_GAMES_LOCK:
            ...
            if winner_team is None:
                ...
                names = ", ".join(tg["alive"][winner_team].values()) or "—"
                bot.send_message(
                    chat_id,
    ...
    try:
        with TEAM_GAMES_LOCK:
            ...
            for team, target_uid in attacks.items():
                ...
                if victim_name:
                    result_lines.append(f"☠️ {_team_label(team)} — {_team_label(enemy)} safidan <b>{victim_name}</b>ni yo'q qildi!")
...
def _team_start_day(chat_id):
    """☀️ Qisqa tanaffus — hozirgi holatni e'lon qiladi, so'ng navbatdagi
    tunni boshlaydi."""
    try:
        with TEAM_GAMES_LOCK:
            tg = TEAM_GAMES.get(chat_id)
            ...
            bot.send_message(
                ...
                f"⏳ {TEAM_DAY_SECONDS} soniyadan so'ng navbatdagi tun boshlanadi...",
            )
            t = threading.Timer(TEAM_DAY_SECONDS, lambda: _team_start_night(chat_id))
            t.daemon = True
            t.start()
            tg["timers"].append(t)
    except Exception as e:
