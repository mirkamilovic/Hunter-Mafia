...
def announce_item_use(chat_id, user_id, item_label, target_id=None, effect_text=""):
    ...
    qilinmasdan — anonim tarzda e'lon qilinadi. (Ishlatgan kishiga natija haqidagi DM
    odatda chaqiruvchi joyda alohida yuboriladi, shu funksiya faqat guruh e'loni uchun.)"""
    try:
        bot.send_message(chat_id, f"🎒 <i>Kimdir do'kondan olingan maxfiy buyumini ishlatdi...</i> ({item_label})")
    except Exception:
        pass
...
def start_day(chat_id):
    ...
    game["day_msg_count"] = {}
    game["day_first_speaker"] = None
    game["day_said_phrases"] = {}
    game["vote_order"] = []
...
def track_mission_activity(game, uid, text):
    """Kunduzgi xabarlar asosida maxfiy missiyalarni tekshirish uchun kerakli
    statistikani to'playdi (birinchi gapiruvchi, xabarlar soni, aniq iboralar)."""
    game.setdefault("day_msg_count", {})
    game["day_msg_count"][uid] = game["day_msg_count"].get(uid, 0) + 1
    if game.get("day_first_speaker") is None:
        game["day_first_speaker"] = uid
    low = text.lower()
    said = game.setdefault("day_said_phrases", {}).setdefault(uid, set())
