...
class _SafeExceptionHandler(telebot.ExceptionHandler):
    ...
    🔧 TUZATILDI (#5, keyin qayta ishlab chiqildi): minglab foydalanuvchi bo'lganda
    ODATIY, KUTILGAN xatolar (foydalanuvchi botni bloklab qo'ygan, eski tugma
    bosilgan va h.k.) juda tez-tez chiqadi va bularning HAR BIRI uchun sizga
    xabar yuborish — o'zi SPAM bo'lib qoladi. Shuning uchun endi:
      1) "Kutilgan/zararsiz" xatolar (pastdagi BENIGN_ERROR_PATTERNS) uchun
         UMUMAN xabar YUBORILMAYDI — faqat log fayliga yoziladi.
      2) Qolgan (haqiqiy, kutilmagan) xatolar ERROR TURI bo'yicha guruhlanadi
         (aniq user_id/chat_id farqi e'tiborga olinmaydi) — bir xil turdagi
         xato 50 marta chiqsa ham, sizga faqat BITTASI, "necha marta takrorlandi"
         hisobi bilan yuboriladi.
      3) /xato_bildir buyrug'i orqali bu xabarlarni butunlay yoqib/o'chirib
         qo'yish mumkin (standart holat: YOQILGAN)."""
    ...
    def handle(self, exception):
        ...
        try:
            ...
            if get_setting("error_notify_enabled") == "0":
                return True  # owner /xato_bildir orqali butunlay o'chirib qo'ygan

            # 1) Odatiy, zararsiz xatolarni SHOVQIN qilmasdan jim o'tkazib yuboramiz
            for pat in BENIGN_ERROR_PATTERNS:
                if pat in err_text:
                    return True

            # 2) 409 Conflict — alohida, tushunarli maslahat bilan (5 daqiqada bir marta)
            if "409" in err_text and ("Conflict" in err_text or "getUpdates" in err_text):
                key = "errnotify_409_conflict"
                last = _ERROR_NOTIFY_CACHE.get(key, 0)
                if time.time() - last > 300:
                    _ERROR_NOTIFY_CACHE[key] = time.time()
                    bot.send_message(
                        OWNER_ID_FROM_ENV,
                        "⚠️ <b>Diqqat: bot ikki joyda bir vaqtda ishga tushirilgan!</b>\n\n"
                        "Telegram bitta bot tokeniga faqat BITTA faol nusxaga ruxsat beradi. "
                        "Iltimos, serveringizda botning ESKI jarayonini to'liq to'xtating, "
                        "so'ng botni FAQAT BIR MARTA qayta ishga tushiring.",
                    )
                return True

            # 3) Qolgan xatolarni ERROR TURI bo'yicha guruhlab, kamdan-kam (10 daqiqada
            #    1 marta har bir tur uchun) va "necha marta takrorlandi" bilan yuboramiz —
            #    bu minglab foydalanuvchi bo'lganda ham sizni g'arq qilmaydi.
            err_type = type(exception).__name__
            key = f"errnotify_type_{err_type}"
            _ERROR_COUNT_CACHE[key] = _ERROR_COUNT_CACHE.get(key, 0) + 1
            last = _ERROR_NOTIFY_CACHE.get(key, 0)
            if time.time() - last > 600:  # 10 daqiqada bir marta shu TUR uchun
                count = _ERROR_COUNT_CACHE.pop(key, 1)
                _ERROR_NOTIFY_CACHE[key] = time.time()
                bot.send_message(
                    OWNER_ID_FROM_ENV,
                    f"🚨 <b>Botda xato yuz berdi!</b>\n"
                    f"🕐 Vaqt: {now}\n"
                    f"🔁 So'nggi 10 daqiqada bu TUR ({err_type}) — <b>{count}</b> marta chiqdi\n"f"📋 Namuna: <code>{err_text[:400]}</code>",
                )
        except Exception:
...
# 🔧 Minglab foydalanuvchida MUTLAQO ODATIY (kod xatosi emas) Telegram javoblari —
# bular uchun owner'ga UMUMAN xabar yuborilmaydi, faqat log fayliga yoziladi:
BENIGN_ERROR_PATTERNS = [
    "bot was blocked by the user",       # foydalanuvchi botni bloklagan
    "user is deactivated",               # foydalanuvchi Telegram akkauntini o'chirgan
    "chat not found",                    # chat/foydalanuvchi topilmadi
    "message is not modified",           # xabar matni o'zgarmagan holda tahrirlash urinishi
    "message to edit not found",         # tahrirlanadigan xabar allaqachon o'chirilgan
    "query is too old",                  # eski (muddati o'tgan) tugma bosilgan
    "message can't be deleted",          # o'chirib bo'lmaydigan (juda eski) xabar
    "have no rights to send a message",  # bot guruhda cheklangan
    "PEER_ID_INVALID",                   # foydalanuvchi botga /start bosmagan
    "Too Many Requests",                 # Telegram flood-limit — o'zi qayta uriladi
]
...
# ================================================================================
#  🛡 SPAM/FLOOD HIMOYASI (#4) — XAVFSIZ VERSIYA
#  🔴 MUHIM TUZATISH: avvalgi versiya `bot.middleware_handler()` dan foydalangan
#  edi — bu SIZNING pyTelegramBotAPI kutubxona versiyangizda ichki xatoga sabab
#  bo'lib, HAR BIR buyruqni (jumladan /start'ni ham) butunlay ishlamay qo'ygan
#  edi ("'TeleBot' object has no attribute 'typed_middleware_handlers'").
#  registratsiya vaqtidagi try/except buni USHLAB QOLOLMAGAN edi, chunki xato
#  faqat HAR BIR XABAR kelganda, botning ICHKI dispatch mexanizmida chiqqan.
#  Endi middleware BUTUNLAY OLIB TASHLANDI — o'rniga maybe_capture_owner()
#  ichiga (deyarli barcha handlerlarning birinchi qatorida chaqiriladi) YUMSHOQ,
#  hech qanday ichki kutubxona mexanizmiga bog'liq bo'lmagan tezlik nazorati
#  qo'shildi. Bu usul umuman hech qanday xavf tug'dirmaydi.
# ================================================================================

RATE_LIMIT_WINDOW = 2.0      # soniya
RATE_LIMIT_MAX_MSGS = 5      # shu oyna ichida ruxsat etilgan maksimal xabar soni
_RATE_LIMIT_LOG = {}         # uid -> [timestamp, timestamp, ...]


def is_rate_limited(uid):
    """🛡 #4: foydalanuvchi so'nggi RATE_LIMIT_WINDOW soniyada RATE_LIMIT_MAX_MSGS
    dan ko'proq marta buyruq/tugma yuborganmi — tekshiradi. Bot egasi HECH QACHON
    cheklanmaydi. Bu funksiya HECH QANDAY kutubxona-ichki mexanizmga tegmaydi —
    faqat oddiy Python lug'ati bilan hisoblaydi, shuning uchun butunlay xavfsiz."""
    try:
        if is_owner(uid):
            return False
        now = time.time()
        ...
        log.append(now)
        return len(log) > RATE_LIMIT_MAX_MSGS
    except Exception:
        return False
...
OWNER_EXTRA_COMMANDS = [
    ...
    ("xato_bildir", "🚨 Xato xabarlarini yoqish/o'chirish"),
]
...
@bot.message_handler(commands=["xato_bildir"])
def cmd_xato_bildir(message):
    """🚨 FAQAT bot egasi — kutilmagan xatolar haqida Telegram orqali avtomatik
    xabar kelishini yoqadi/o'chiradi. Standart holat: YOQILGAN. Minglab
    foydalanuvchi bo'lganda ham xabarlar allaqachon guruhlangan/filtrlangan
    (BENIGN_ERROR_PATTERNS + turi bo'yicha 10 daqiqalik cheklov), lekin xohlasangiz
    buni butunlay o'chirib qo'yishingiz ham mumkin."""
    maybe_capture_owner(message.from_user)
    safe_delete(message)
    if not is_owner(message.from_user.id):
        return
    current = get_setting("error_notify_enabled")
    new_val = "0" if current != "0" else "1"
    set_setting("error_notify_enabled", new_val)
    if new_val == "0":
        bot.send_message(message.chat.id, "🔕 Xato haqidagi avtomatik xabarlar O'CHIRILDI. Yana yoqish uchun /xato_bildir deb yozing.")
    else:
        bot.send_message(message.chat.id, "🔔 Xato haqidagi avtomatik xabarlar YOQILDI (allaqachon filtrlangan — spam qilmaydi).")
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # 🔧 YANGI: 409 Conflict (ikkita nusxa bir vaqtda ishlayotgani) uchun maxsus,
            # UZUNROQ kutish — aks holda ikkala nusxa ham 5 soniyada bir urinib, bir-biriga
            # xalaqit berishda davom etaveradi ("thrashing"). Boshqa xatolar uchun eski 5 soniya qoladi.
            err_str = str(e)
            if "409" in err_str and ("Conflict" in err_str or "getUpdates" in err_str):
                _logger.warning("⚠️ 409 Conflict: boshqa bot nusxasi ishlayapti. 30 soniya kutamiz...")
                time.sleep(30)
            else:
