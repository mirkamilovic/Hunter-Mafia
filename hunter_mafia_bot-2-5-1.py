...
def do_join(chat_id, tg_user, squad=None, via_couple=False):
    ...
    if game.get("couple_mode") and not via_couple:
        return "need_couple"
    uid = tg_user.id
    # 🔒 Bir vaqtning o'zida faqat BITTA guruhda o'ynash mumkin — agar
    # foydalanuvchi boshqa biror guruhdagi FAOL o'yinda (hali tugamagan)
    # ishtirokchi bo'lsa, bu yerga (hatto shu lobbiga ham) qo'shila olmaydi.
    # Bu — bir kishi ikkita guruhda parallel o'ynayotganda tungi/kunduzgi
    # DM'lar aralashib ketishining oldini oladi.
    for other_chat_id, other_game in GAMES.items():
        if other_chat_id != chat_id and uid in other_game.get("players", {}):
            return "other_game"
